-- ============================================================
-- M-MAINT PART B — PROOFS P3 / P4, CORRECTIVE RUN.
-- Identical assertions to 20260917210349_c23b6c76; the ONLY change is that the
-- acting operator is RESOLVED from this database (a global super_admin) instead
-- of a literal uuid, which belongs to one environment only and made the first
-- door answer "permission denied". Append-only: the earlier file stands.
...
BEGIN
  ...
  END IF;
  IF NOT (public.has_permission(v_uid, 'categories', 'create')
          AND public.has_permission(v_uid, 'categories', 'import')
          AND public.has_permission(v_uid, 'categories', 'update')) THEN
    RAISE EXCEPTION 'PROOF BLOCKED — super_admin % lacks the category rights', v_uid;
  END IF;
  ...
  RAISE NOTICE 'P3 and P4 PASSED (operator resolved from this database)';
END $proof$;

INSERT INTO public.migration_marks(version) VALUES ('20260917214500') ON CONFLICT DO NOTHING;