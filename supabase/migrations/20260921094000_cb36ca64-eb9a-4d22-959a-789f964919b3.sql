-- =====================================================================
-- M-SHAPE CORRECTIVE / INC-255 — A PROOF NEVER ANCHORS ON A REAL CATALOG ROW.
--
-- WHY THIS FILE EXISTS. 20260921092130_872f21f4-…sql landed the M-SHAPE planner
-- guard (INC-254) with proofs anchored on the LIVE `voltage` definition. Prod
-- holds that row; staging does not, so the file raised
--   ERROR: P0001: PROOF setup: voltage definition absent
-- and, migration files being immutable, it can never apply there (INC-255 —
-- INC-222's law restated: a proof builds its own scratch rows and deletes them;
-- it never depends on real reference data).
--
-- WHAT THIS FILE DOES.
--   1. Re-declares public.attr_import_plan WHOLE (INC-183), byte-IDENTICAL to
--      the body in 872f21f4 (md5 of the declaration + ACL block:
--      1f5ac0378f07ab807fde6c4759dc74d9). No behaviour change whatsoever: on
--      prod this is a no-op re-declaration; on staging it is the landing.
--   2. Proves the guard with SCRATCH ROWS ONLY (`e2e-mshape-*`), deleted in the
--      same file, with a residue read-back.
--   3. HEALS THE LEDGER: records mark 20260922090000 (872f21f4's own mark)
--      ON CONFLICT DO NOTHING — prod already holds it, staging does not.
--   4. Records its own mark 20260922094000, above the current maximum
--      20260922093000.
--
-- NO VALUE RULE IS INTRODUCED HERE (unchanged from INC-254's correction): no
-- option-value charset exists anywhere, and the live `17+` proves one would
-- refuse the catalog's own state.
-- =====================================================================
...
-- =====================================================================
-- PROOFS — SCRATCH ROWS ONLY (INC-255). Every row this file reads it also
-- creates; nothing anchors on a real catalog definition. The scratch rows are
-- deleted in the same block and a residue read-back follows.
-- =====================================================================
DO $proof$
DECLARE
  v_att   public.attributes%ROWTYPE;
  ...
  v_row   jsonb;
  v_n     int;
  v_keys  text[] := ARRAY['e2e-mshape-volt','e2e-mshape-depth','e2e-mshape-note'];
BEGIN
  -- SCRATCH SETUP. A select definition holding exactly the digit-led shapes the
  -- operator's export writes (`220v`, `110v`, `380v_three_phase` with aliases,
  -- `17+`), plus a number and a text definition for the round-trip walk.
  DELETE FROM public.attributes WHERE attr_key = ANY (v_keys);
  INSERT INTO public.attributes (attr_key, name_en, name_am, attr_type, options)
  VALUES ('e2e-mshape-volt', 'E2E M-SHAPE voltage', 'ኢ2ኢ ቮልቴጅ', 'single_select',
          '[{"value":"220v","label_en":"220 V"},
             {"value":"110v","label_en":"110 V"},
             {"value":"380v_three_phase","label_en":"380 V three-phase","aliases":["Three-Phase","380V"]},
             {"value":"17+","label_en":"17 and over"},
             {"value":"other","label_en":"Other","active":true}]'::jsonb);
  INSERT INTO public.attributes (attr_key, name_en, attr_type, unit)
  VALUES ('e2e-mshape-depth', 'E2E M-SHAPE depth', 'number', 'mm');
  INSERT INTO public.attributes (attr_key, name_en, attr_type)
  VALUES ('e2e-mshape-note', 'E2E M-SHAPE note', 'text');

  SELECT * INTO v_att FROM public.attributes WHERE attr_key = 'e2e-mshape-volt';
  IF v_att.id IS NULL THEN
    RAISE EXCEPTION 'PROOF setup: scratch definition was not created';
  END IF;

  -- PROOF 1 — THE OPERATOR'S OWN FILE, on a SCRATCH row. The stored option list
  -- re-imported byte-identical plans as UNCHANGED, with no refusal at all.
  ...
  IF jsonb_array_length(v_plan->'refusals') <> 0 THEN
    RAISE EXCEPTION 'PROOF 1 failed: byte-identical scratch row refused %', v_plan->'refusals';
  END IF;
  IF (v_plan->'definitions'->0->>'change') IS DISTINCT FROM 'none' THEN
    RAISE EXCEPTION 'PROOF 1 failed: scratch row planned as %', v_plan->'definitions'->0->>'change';
  END IF;
  RAISE NOTICE 'PROOF 1 ok — byte-identical scratch option cell: change=none, refusals=0';

  -- PROOF 2 — A NEW DIGIT-LED OPTION VALUE PASSES. Nothing refuses `128gb`.
  v_plan := public.attr_import_plan(
    jsonb_build_array(jsonb_build_object(
      'row', 1, 'attribute_key', 'e2e-mshape-probe', 'action', 'upsert',
      'label_en', 'E2E M-SHAPE probe', 'type', 'single_select',
  ...
  -- The same scratch row with ONE extra key in ONE option is no longer
  ...
  -- PROOF 5 — EVERY SCRATCH DEFINITION ROUND-TRIPS, each re-imported exactly as
  -- the export writes it (dependency column included). E6: the empty set is
  -- named — all three scratch rows must be walked.
  v_n := 0;
  FOR v_att IN SELECT * FROM public.attributes
                WHERE attr_key = ANY (v_keys) ORDER BY attr_key LOOP
    v_plan := public.attr_import_plan(
      jsonb_build_array(jsonb_build_object(
        'row', 1, 'attribute_key', v_att.attr_key, 'action', 'upsert',
        'label_en', btrim(COALESCE(v_att.name_en, '')),
    ...
    END IF;
    IF (v_plan->'definitions'->0->>'change') IS DISTINCT FROM 'none' THEN
      RAISE EXCEPTION 'PROOF 5 failed: % planned as %',
        v_att.attr_key, v_plan->'definitions'->0->>'change';
    END IF;
    v_n := v_n + 1;
  END LOOP;
  IF v_n <> 3 THEN
    RAISE EXCEPTION 'PROOF 5 failed: walked % scratch definitions, expected 3', v_n;
  END IF;
  RAISE NOTICE 'PROOF 5 ok — % scratch definitions re-import with zero refusals', v_n;

  -- CLEANUP — the scratch rows leave no trace.
  DELETE FROM public.attributes WHERE attr_key = ANY (v_keys);
  SELECT count(*) INTO v_n FROM public.attributes WHERE attr_key = ANY (v_keys);
  IF v_n <> 0 THEN
...
-- READ-BACKS — the definition carries the guard, and the ACL is intact.
DO $readback$
DECLARE r record;
BEGIN
...
-- LEDGER HEAL (DEC-022) — 872f21f4's own mark, recorded where it is missing.
INSERT INTO public.migration_marks (version)
VALUES ('20260922090000') ON CONFLICT DO NOTHING;

-- This file's own mark, above the current maximum 20260922093000.
INSERT INTO public.migration_marks (version)
VALUES ('20260922094000') ON CONFLICT DO NOTHING;