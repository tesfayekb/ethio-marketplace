-- =====================================================================
-- M-ORDER — THE LINKS FILE CARRIES `display_order` · PART 1 of 2
--   the planner: parse, judge, diff, emit.
--
...
--   attr_option_norm_v2            20260920090257_7b9dc9eb-427e-4883-a526-7f183bec1c70.sql (M-FACTS)
--   get_posting_schema             20260919134523_67ef15da-8a1e-4d34-8e9a-2a3f37a2a56a.sql
--
-- WHAT THE CENSUS FOUND (stated, not assumed):
--   * M-FACTS (7b9dc9eb) re-declared attr_option_norm_v2 ONLY — NOT the
--     planner. The planner's latest body is therefore 6628d97b's, re-declared
--     WHOLE below.
--   * THE PLANNER never read `display_order`: the cell was neither parsed nor
--     diffed nor emitted. THE ONE FIX HERE.
--   * THE COMMIT's UPDATE path ALREADY writes
--       display_order = COALESCE(NULLIF(v_item->>'display_order','')::int,
--                                l.display_order)
--     so a planned value lands and a blank keeps the stored one; its ADD path
--     still forces max+1 — corrected in PART 2.
--   * THE REVISION rows ALREADY capture display_order in prev and post
--     (all three jsonb_build_object captures name it).
--   * THE UNDO ALREADY restores it (`display_order = EXCLUDED.display_order`
--     from v_rev.prev). Nothing to re-declare.
--   * THE EXPORT does NOT echo it — corrected in PART 2.
--   * THE POSTING READ already orders by it: get_posting_schema joins
--     effective_category_links e ... ORDER BY e.display_order, a.attr_key and
--     builds the details array with jsonb_agg(... ORDER BY ord, key) where
--     ord = e.display_order. CONFIRMED BY GREP; no re-declaration needed, and
--     the TIE-BREAK (attr_key) is therefore untouched.
--
...
-- =====================================================================
-- IN-FILE PROOF P1 — the PLANNER alone (pure, no commit): the cell parses,
-- refuses a non-integer, and a display_order-only change is a `change`.
-- INC-222 scratch identities: NO user id is involved in a plan (the planner
-- is STABLE SECURITY DEFINER and reads only taxonomy), so this proof needs no
-- session; the scratch taxonomy is created and removed here.
-- =====================================================================
DO $proof1$
DECLARE
  v_cat   uuid;
...
BEGIN
  ...
  VALUES ('E2E MOrder Plan Leaf', 'e2e-morder-plan-leaf', true, true, true, 9994)
  RETURNING id INTO v_cat;

  INSERT INTO public.attributes (attr_key, name_en, attr_type)
  VALUES ('e2e_morder_plan_a', 'E2E MOrder Plan A', 'text')
  RETURNING id INTO v_a1;

  INSERT INTO public.category_attribute_links
    (category_id, attribute_id, is_required, is_filterable, display_order)
  VALUES (v_cat, v_a1, false, false, 1);

  -- (a) an ABSENT cell changes nothing
  v_plan := public.attr_import_plan('[]'::jsonb, jsonb_build_array(jsonb_build_object(
      'row', 1, 'category_slug', 'e2e-morder-plan-leaf',
      'attribute_key', 'e2e_morder_plan_a',
      'is_required', 'false', 'is_filterable', 'false')), NULL);
  IF v_plan->'links'->0->>'change' <> 'none' THEN
    RAISE EXCEPTION 'P1 FAILED (absent) — an absent display_order planned as %',
      v_plan->'links'->0->>'change';
  END IF;

  -- (b) a BLANK cell changes nothing, and carries nothing forward
  v_plan := public.attr_import_plan('[]'::jsonb, jsonb_build_array(jsonb_build_object(
      'row', 1, 'category_slug', 'e2e-morder-plan-leaf',
      'attribute_key', 'e2e_morder_plan_a',
      'is_required', 'false', 'is_filterable', 'false',
      'display_order', '')), NULL);
  v_link := v_plan->'links'->0;
  IF v_link->>'change' <> 'none'
     OR (v_link->>'display_order_set')::boolean IS DISTINCT FROM false
     OR COALESCE(v_link->>'display_order','x') <> '' THEN
    RAISE EXCEPTION 'P1 FAILED (blank) — a blank display_order is not silent: %', v_link;
  END IF;

  -- (c) the SAME value is still unchanged
  v_plan := public.attr_import_plan('[]'::jsonb, jsonb_build_array(jsonb_build_object(
      'row', 1, 'category_slug', 'e2e-morder-plan-leaf',
      'attribute_key', 'e2e_morder_plan_a',
      'is_required', 'false', 'is_filterable', 'false',
  ...
  IF v_plan->'links'->0->>'change' <> 'none' THEN
    RAISE EXCEPTION 'P1 FAILED (same) — an identical display_order planned as %',
      v_plan->'links'->0->>'change';
  END IF;

  -- (d) a DIFFERENT value is a field change, named in the plan entry
  v_plan := public.attr_import_plan('[]'::jsonb, jsonb_build_array(jsonb_build_object(
      'row', 1, 'category_slug', 'e2e-morder-plan-leaf',
      'attribute_key', 'e2e_morder_plan_a',
      'is_required', 'false', 'is_filterable', 'false',
      'display_order', '7')), NULL);
  v_link := v_plan->'links'->0;
  IF v_link->>'change' <> 'change' THEN
    RAISE EXCEPTION 'P1 FAILED (diff) — a display_order-only change planned as %',
      v_link->>'change';
  END IF;
  IF (v_link->>'display_order') <> '7'
     OR (v_link->>'display_order_set')::boolean IS DISTINCT FROM true THEN
    RAISE EXCEPTION 'P1 FAILED (diff) — the plan entry does not name display_order: %', v_link;
  END IF;
  IF (v_plan->'counts'->>'changes')::int <> 1 THEN
    RAISE EXCEPTION 'P1 FAILED (counts) — %', v_plan->'counts';
  END IF;
  ...
  v_plan := public.attr_import_plan('[]'::jsonb, jsonb_build_array(jsonb_build_object(
      ...
      'attribute_key', 'e2e_morder_plan_a',
      'is_required', 'false', 'is_filterable', 'false',
      'display_order', '0')), NULL);
  IF jsonb_array_length(v_plan->'refusals') <> 0
     OR v_plan->'links'->0->>'change' <> 'change' THEN
    RAISE EXCEPTION 'P1 FAILED (zero) — 0 was not accepted: %', v_plan;
  END IF;

  -- (f) a NON-INTEGER and a NEGATIVE refuse, naming the value
  FOREACH v_badtxt IN ARRAY ARRAY['two', '-1', '1.5']
  LOOP
    v_plan := public.attr_import_plan('[]'::jsonb, jsonb_build_array(jsonb_build_object(
        'row', 3, 'category_slug', 'e2e-morder-plan-leaf',
        'attribute_key', 'e2e_morder_plan_a',
        'is_required', 'false', 'is_filterable', 'false',
        'display_order', v_badtxt)), NULL);
    IF jsonb_array_length(v_plan->'refusals') <> 1
       OR v_plan->'refusals'->0->>'reason' <> 'badDisplayOrder'
       OR v_plan->'refusals'->0->>'value' <> v_badtxt
       OR (v_plan->'refusals'->0->>'row')::int <> 3 THEN
      RAISE EXCEPTION 'P1 FAILED (refusal) — % gave %', v_badtxt, v_plan->'refusals';
    END IF;
    -- a refused row leaves NO link entry behind (F5: no trace)
    IF jsonb_array_length(v_plan->'links') <> 0 THEN
      RAISE EXCEPTION 'P1 FAILED (refusal) — a refused row still planned a link: %',
        v_plan->'links';
    END IF;
  END LOOP;
  ...
  DELETE FROM public.attributes WHERE id = v_a1;
  DELETE FROM public.categories WHERE id = v_cat;

  RAISE NOTICE 'P1 PASSED — display_order parses, diffs, refuses and stays silent when blank.';
END $proof1$;
...
BEGIN
  ...
  IF (SELECT p.prosrc FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
       WHERE n.nspname = 'public' AND p.proname = 'attr_import_plan')
     NOT LIKE '%badDisplayOrder%' THEN
    RAISE EXCEPTION 'READ-BACK FAILED — the planner does not judge display_order';
  END IF;
  IF (SELECT p.prosrc FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
       WHERE n.nspname = 'public' AND p.proname = 'attr_import_plan')
     NOT LIKE '%display_order_set%' THEN
    RAISE EXCEPTION 'READ-BACK FAILED — the planner does not emit display_order';
  END IF;
  ...
  RAISE NOTICE 'READ-BACK OK — display_order is inside the links diff.';
END $readback$;

-- --------------------------------------------------------------- THE MARK
INSERT INTO public.migration_marks (version)
VALUES ('20260920120000')
