-- MIGRATION MARK 20260924020000 — D37-1 catalog finder
-- pg_trgm was enabled by 20260903033520. unaccent is intentionally not added.
...
CREATE TABLE public.catalog_find_index (
...
CREATE INDEX catalog_find_index_term_trgm ON public.catalog_find_index USING gin (term_norm public.gin_trgm_ops);
CREATE INDEX catalog_find_index_lang_kind ON public.catalog_find_index (lang, kind);
CREATE INDEX catalog_find_index_term_eq ON public.catalog_find_index (term_norm text_pattern_ops);

CREATE TABLE public.catalog_find_terms (term_norm text PRIMARY KEY);
...
CREATE INDEX catalog_find_terms_trgm ON public.catalog_find_terms USING gin (term_norm public.gin_trgm_ops);
...
BEGIN
 INSERT INTO public.categories(name_en,name_am,slug,is_active,allow_listings) VALUES ('Treadmill','ትሬድሚል','e2e-cf-treadmill',true,true) RETURNING id INTO v_fit;
 INSERT INTO public.categories(name_en,name_am,slug,is_active,allow_listings) VALUES ('Traditional & Natural','ባህላዊና ተፈጥሯዊ','e2e-cf-traditional-natural',true,true) RETURNING id INTO v_trad;
 INSERT INTO public.categories(name_en,name_am,slug,is_active,allow_listings) VALUES ('Cars','መኪናዎች','e2e-cf-cars',true,true) RETURNING id INTO v_cars;
 INSERT INTO public.attributes(attr_key,name_en,attr_type,options) VALUES ('e2e_cf_make','Make','single_select','[{"value":"toyota","label_en":"Toyota","label_am":"ቶዮታ"}]') RETURNING id INTO v_make;
 INSERT INTO public.attributes(attr_key,name_en,attr_type,options,depends_on) VALUES ('e2e_cf_model','Model','single_select','[{"value":"corolla","label_en":"Corolla","parent":"toyota"}]',v_make) RETURNING id INTO v_model;
 INSERT INTO public.attributes(attr_key,name_en,attr_type,options) VALUES ('e2e_cf_transmission','Transmission','single_select','[{"value":"automatic","label_en":"Automatic"}]') RETURNING id INTO v_trans;
 INSERT INTO public.attributes(attr_key,name_en,attr_type,options) VALUES ('e2e_cf_cosmetic','Cosmetic','single_select','[{"value":"kohl","label_en":"Kohl","label_am":"ኩል","aliases":["ኩሕል"]}]') RETURNING id INTO v_cos;
 INSERT INTO public.attributes(attr_key,name_en,attr_type,options) VALUES ('e2e_cf_power','Power Source','single_select','[{"value":"electric","label_en":"Electric"}]') RETURNING id INTO v_ptype;
 ...
 SELECT count(*) INTO v_n FROM public.catalog_find('ኩል','am') f WHERE f.leaf_id=v_trad AND f.matches @> '[{"key":"e2e_cf_cosmetic","value":"kohl"}]'; IF v_n<>1 THEN RAISE EXCEPTION 'PROOF 2 failed'; END IF;
 SELECT count(*) INTO v_n FROM public.catalog_find('automatic toyota','en') f WHERE f.leaf_id=v_cars AND f.matches @> '[{"key":"e2e_cf_make","value":"toyota"},{"key":"e2e_cf_transmission","value":"automatic"}]'; IF v_n<>1 THEN RAISE EXCEPTION 'PROOF 3 failed'; END IF;
 ...
 SELECT count(*),coalesce(octet_length(jsonb_agg(to_jsonb(f))::text),0) INTO v_n,v_bytes FROM public.catalog_find('automatic toyota','en') f; IF v_n>8 OR v_bytes>2048 THEN RAISE EXCEPTION 'PROOF 5 failed'; END IF;
 ...
 DELETE FROM public.attributes WHERE attr_key='e2e_cf_model'; DELETE FROM public.attributes WHERE attr_key IN ('e2e_cf_make','e2e_cf_transmission','e2e_cf_cosmetic','e2e_cf_power'); DELETE FROM public.categories WHERE id IN (v_fit,v_trad,v_cars);
