...
--   4. Own mark 20260922120000, above the current maximum 20260922110000.
--
...
BEGIN
  ...
  -- PROOF 3 — A SWATCH-ONLY EDIT IS ONE CHANGE, NOT A REFUSAL.
  v_plan := public.attr_import_plan(
    jsonb_build_array(jsonb_build_object(
      'row', 1, 'attribute_key', v_key, 'action', 'upsert',
      'label_en', 'INC-265 scratch colour', 'type', 'single_select', 'depends_on', '',
  ...
  END IF;
  IF (v_plan->'definitions'->0->>'change') IS DISTINCT FROM 'update' THEN
...
-- This file's own mark, above the current maximum 20260922110000.
