-- IE-1 — ATTRIBUTE EXPORT. Additive, read-only, gated.
-- CENSUS: the C3b normalized model (attributes + category_attribute_links)
-- has NO effective/inherited reader — `get_category_attributes` still reads the
-- legacy per-category `category_attributes` table, and
-- `admin_list_category_attribute_links` is one category's DIRECT links only.
-- The export needs own ∪ ancestors with nearest-link-wins, so the scope's
-- conditional applies: exactly one additive gated RPC, returning both CSV row
-- sets in one call so the route makes a single trip.
...
DO $$
DECLARE v_sig text := 'public.admin_export_attributes()';
BEGIN
...
INSERT INTO public.migration_marks(version) VALUES ('20260907070000') ON CONFLICT DO NOTHING;