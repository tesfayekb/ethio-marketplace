...
--
-- WHY. D28/M-SWATCH allows a two-tone swatch written as two hexes separated by
-- a pipe ("#000000|#8B5A2B"). The options cell also separates its RECORDS with
-- a pipe, and every parser split on a bare pipe — so a two-tone record was cut
-- in half, the fragment was not valid JSON, and the import answered
-- `malformedOptions` ("The options could not be read") for a cell the door
-- itself allows. The same cut hit any label carrying a pipe.
--
-- WHAT. One splitter, in SQL as in the gate (`splitOptionSegments`,
-- src/server/imports/gate.ts): walk the cell, track JSON string state and brace
-- depth, separate only at a pipe that sits OUTSIDE every record. A legacy
-- `value=label` segment still separates exactly as before (depth 0).
--   1. NEW public.attr_split_option_cell(text) — the one boundary reader.
--   2. public.attr_import_plan re-declared WHOLE (INC-183), byte-identical to
--      20260921094000 except the single FOREACH source, which now reads the
--      boundary instead of `string_to_array(v_raw, '|')`.
--   3. Scratch-row proofs only (INC-222), deleted in this same file.
--   4. Own mark 20260922130000; the never-applied 20260922120000 (identical SQL,
--      one mis-worded proof) is healed in the same ledger statement block.
--
-- INC-266 — WHY THIS FILE EXISTS. 20260922120000 asserted the planner's verdict
-- vocabulary as 'update'; the planner's own word for a changed definition is
-- 'change' (see v_change := 'change'). The proof was wrong, not the fix, and a
-- tool-written migration cannot be edited, so the landing is restated here.
--
-- The two-tone CONTRACT is unchanged: a swatch is one hex, two hexes separated
-- by `|`, or `pattern:<…>`, judged by `attr_option_shape` as before. The export
-- is unchanged too: it writes whole JSON records joined with `|`, which is
-- exactly what this reader separates.
-- =====================================================================
...
BEGIN
  ...
  FOR i IN 1..length(p_cell) LOOP
    v_ch := substr(p_cell, i, 1);
    IF v_str THEN
      v_cur := v_cur || v_ch;
      IF v_esc THEN
        v_esc := false;
...
-- PROOFS — SCRATCH ROWS ONLY (INC-222): this file builds every row it reads and
-- deletes them before it ends.
DO $proof$
DECLARE
  v_key   text := 'e2e-inc265-colour';
  v_opts  jsonb;
  v_plan  jsonb;
  v_parts text[];
  v_n     int;
  v_att   public.attributes;
BEGIN
  -- PROOF 1 — THE BOUNDARY READER. A two-tone swatch and a piped label stay
  -- whole; records still separate; a legacy `value=label` pair still splits.
  ...
  IF array_length(v_parts, 1) <> 2 THEN
    RAISE EXCEPTION 'PROOF 1 failed: split into % segments, expected 2 (%)',
      array_length(v_parts, 1), v_parts;
  END IF;
  IF public.attr_json_or_null(btrim(v_parts[1])) IS NULL
     OR public.attr_json_or_null(btrim(v_parts[2])) IS NULL THEN
    RAISE EXCEPTION 'PROOF 1 failed: a segment is not valid JSON %', v_parts;
  END IF;
  v_parts := public.attr_split_option_cell('a=A|b=B');
  IF array_length(v_parts, 1) <> 2 THEN
    RAISE EXCEPTION 'PROOF 1 failed: legacy pairs split into %', v_parts;
  END IF;
  RAISE NOTICE 'PROOF 1 ok — two-tone and piped label stay whole; legacy pairs still split';

  -- Setup: a scratch colour definition carrying a two-tone swatch and a label
  -- with a pipe in it.
  DELETE FROM public.attributes WHERE attr_key = v_key;
  v_opts := jsonb_build_array(
    jsonb_build_object('value','black_tan','label_en','Black | Tan','swatch','#000000|#8B5A2B'),
    jsonb_build_object('value','white','label_en','White','swatch','#FFFFFF'));
  INSERT INTO public.attributes (attr_key, name_en, attr_type, options)
  VALUES (v_key, 'INC-265 scratch colour', 'single_select', v_opts);

  -- PROOF 2 — THE EXPORT'S OWN CELL RE-IMPORTS AS UNCHANGED. The cell is built
  -- exactly as `admin_export_attributes` writes it: whole records joined by '|'.
  v_plan := public.attr_import_plan(
    jsonb_build_array(jsonb_build_object(
      'row', 1, 'attribute_key', v_key, 'action', 'upsert',
      'label_en', 'INC-265 scratch colour', 'type', 'single_select', 'depends_on', '',
      'options', (SELECT string_agg(o.x, '|' ORDER BY o.ord)
                    FROM jsonb_array_elements_text(v_opts) WITH ORDINALITY AS o(x, ord)))),
    '[]'::jsonb, NULL);
  IF jsonb_array_length(v_plan->'refusals') <> 0 THEN
    RAISE EXCEPTION 'PROOF 2 failed: a two-tone cell refused %', v_plan->'refusals';
  END IF;
  IF (v_plan->'definitions'->0->>'change') IS DISTINCT FROM 'none' THEN
    RAISE EXCEPTION 'PROOF 2 failed: byte-identical cell planned as %',
      v_plan->'definitions'->0->>'change';
  END IF;
  RAISE NOTICE 'PROOF 2 ok — a two-tone swatch round-trips as change=none, 0 refusals';

  -- PROOF 3 — A SWATCH-ONLY EDIT IS ONE CHANGE, NOT A REFUSAL. INC-266: the
  -- planner's own word for a changed definition is 'change' (v_change), which is
  -- what the counts read as `changes`.
  v_plan := public.attr_import_plan(
    jsonb_build_array(jsonb_build_object(
      'row', 1, 'attribute_key', v_key, 'action', 'upsert',
      'label_en', 'INC-265 scratch colour', 'type', 'single_select', 'depends_on', '',
      'options', '{"value":"black_tan","label_en":"Black | Tan","swatch":"#000000|#FFFFFF"}'
                 || '|{"value":"white","label_en":"White","swatch":"#FFFFFF"}')),
    '[]'::jsonb, NULL);
  IF jsonb_array_length(v_plan->'refusals') <> 0 THEN
    RAISE EXCEPTION 'PROOF 3 failed: a swatch-only edit refused %', v_plan->'refusals';
  END IF;
  IF (v_plan->'definitions'->0->>'change') IS DISTINCT FROM 'change' THEN
    RAISE EXCEPTION 'PROOF 3 failed: swatch-only edit planned as %',
      v_plan->'definitions'->0->>'change';
  END IF;
  IF (v_plan->'counts'->>'changes') IS DISTINCT FROM '1' THEN
    RAISE EXCEPTION 'PROOF 3 failed: counts.changes = %', v_plan->'counts'->>'changes';
  END IF;
  RAISE NOTICE 'PROOF 3 ok — "#000000|#FFFFFF" plans changed 1 · refused 0';

  -- PROOF 4 — THE SHAPE LAW STILL BITES. An unknown key on a CHANGED cell is
  -- refused by name, and a malformed record is still unreadable.
  v_plan := public.attr_import_plan(
    jsonb_build_array(jsonb_build_object(
      'row', 1, 'attribute_key', v_key, 'action', 'upsert',
      'label_en', 'INC-265 scratch colour', 'type', 'single_select', 'depends_on', '',
      'options', '{"value":"black_tan","nope":"x"}')),
    '[]'::jsonb, NULL);
  IF (v_plan->'refusals'->0->>'reason') IS DISTINCT FROM 'badOption' THEN
    RAISE EXCEPTION 'PROOF 4 failed: unknown option key escaped %', v_plan->'refusals';
  END IF;
  v_plan := public.attr_import_plan(
    jsonb_build_array(jsonb_build_object(
      'row', 1, 'attribute_key', v_key, 'action', 'upsert',
      'label_en', 'INC-265 scratch colour', 'type', 'single_select', 'depends_on', '',
      'options', '{"value":"black_tan"')),
    '[]'::jsonb, NULL);
  IF (v_plan->'refusals'->0->>'reason') IS DISTINCT FROM 'malformedOptions' THEN
    RAISE EXCEPTION 'PROOF 4 failed: a truncated record was read anyway %', v_plan->'refusals';
  END IF;
  RAISE NOTICE 'PROOF 4 ok — unknown key and truncated record still refuse by name';

  -- PROOF 5 — the scratch definition is walked once (E6: the set is named).
  v_n := 0;
  FOR v_att IN SELECT * FROM public.attributes WHERE attr_key = v_key LOOP
    v_n := v_n + 1;
  END LOOP;
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'PROOF 5 failed: walked % scratch definitions, expected 1', v_n;
  END IF;

  -- CLEANUP — no residue.
  DELETE FROM public.attributes WHERE attr_key = v_key;
  SELECT count(*) INTO v_n FROM public.attributes WHERE attr_key = v_key;
  IF v_n <> 0 THEN
...
-- READ-BACKS — both definitions carry the boundary reader, and the ACLs hold.
DO $readback$
DECLARE r record;
BEGIN
  SELECT p.provolatile AS vol, p.prosecdef AS secdef,
         (position('attr_split_option_cell' in p.prosrc) > 0) AS has_reader,
         (position('string_to_array(v_raw' in p.prosrc) = 0) AS no_bare_split
    INTO r
    FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = 'attr_import_plan';
  IF NOT r.has_reader OR NOT r.no_bare_split OR r.vol <> 's' OR NOT r.secdef THEN
    RAISE EXCEPTION 'READ-BACK failed: reader=% bare=% volatility=% secdef=%',
      r.has_reader, r.no_bare_split, r.vol, r.secdef;
  END IF;
  RAISE NOTICE 'READ-BACK attr_import_plan: boundary reader present, no bare split, STABLE SECURITY DEFINER';

  FOR r IN
    SELECT p.proname, unnest(p.proacl)::text AS ace
      FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
     WHERE n.nspname = 'public'
       AND p.proname IN ('attr_import_plan', 'attr_split_option_cell')
  LOOP
    RAISE NOTICE 'READ-BACK acl % %', r.proname, r.ace;
  END LOOP;
END $readback$;

-- This file's own mark, plus the ledger heal for the never-applied
-- 20260922120000 (identical SQL, one mis-worded proof — INC-266).
INSERT INTO public.migration_marks (version)
VALUES ('20260922120000') ON CONFLICT DO NOTHING;
INSERT INTO public.migration_marks (version)
VALUES ('20260922130000') ON CONFLICT DO NOTHING;