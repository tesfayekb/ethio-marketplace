-- IE-5 (corrective) — THE PLANNER, WRITTEN OUT IN FULL.
--
-- The IE-5 landing patched `attr_import_plan` surgically. The anchor text is
-- not identical on every copy of the database (staging refused with
-- "the planner declaration block did not match"), so the corrective declares
-- the WHOLE body — the post-INH-1 planner plus the IE-5 survivor rule — and is
-- therefore text-independent and idempotent everywhere.
--
-- LAW (unchanged): a definition delete is refused only if a link to it SURVIVES
-- the same file's unlinks; the refusal still NAMES the categories it judged.
--
...
-- PROOF, in-file: the rewrite carries the survivor rule.
DO $ie5p$
DECLARE
  v_src text;
BEGIN
  ...
  IF v_src IS NULL OR position('ANY (v_unlinks)' IN v_src) = 0 THEN
    RAISE EXCEPTION 'IE-5 corrective: the planner does not carry the survivor rule';
  END IF;
  IF position('cat_primary_parent(c.id)' IN v_src) = 0 THEN
    RAISE EXCEPTION 'IE-5 corrective: the INH-1 primary-lineage walk was lost';
  END IF;
  RAISE NOTICE 'IE-5 corrective: planner carries the survivor rule and the primary-lineage walk';
END $ie5p$;
...
BEGIN
  ...
  IF v_acl IS NULL OR position('authenticated=X' IN v_acl) = 0 THEN
    RAISE EXCEPTION 'IE-5 ACL: attr_import_plan is not executable by authenticated (%)', v_acl;
  END IF;
  IF position('anon=X' IN v_acl) > 0 THEN
    RAISE EXCEPTION 'IE-5 ACL: attr_import_plan still carries an anon execute grant (%)', v_acl;
  END IF;
  RAISE NOTICE 'IE-5 ACL attr_import_plan: %', v_acl;
END $acl$;

INSERT INTO public.migration_marks(version) VALUES ('20260909041000') ON CONFLICT DO NOTHING;