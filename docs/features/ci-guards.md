...
Part 2 (2026-09-25): green runs carry the sections. The reporter loads every source and the context index ABOVE the green branch, so `renderGreen` (E2E_GREEN=1) renders "- Flaky (passed on retry, DEC-030, non-gating): <n>" after the post-test warnings line, then the same Flake ledger and Flaky bodies sections the red path renders (one shared `flakeLedgerSection`, no second copy). The green verdict carries the real count, `flaky=<n>`. The ledger-only pass (E2E_FLAKE_ONLY=1) is unchanged. Self-test: the flipped capture renders "passed", the ledger, the bodies and the flipped first line in that order; a clean green renders "non-gating): 0" and no section heading.
