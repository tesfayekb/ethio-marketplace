...
- INC-097c — service_role bypasses RLS but NOT table grants — pre-law tables denied even correct service-client fixture reads (TR-14). E1 extended explicitly: every table ships GRANT ALL TO service_role alongside its RLS policies; the pre-law era is swept and the in-migration census proves totality. Addendum (b): the first sweep census predicated on ABSENCE of any grant row and skipped four partially-granted tables (countries, locations, profiles, user_directory — reference-era SELECT-only grants); the corrective census keys on missing PRIVILEGES (has_table_privilege per operation) and the live read-back proves zero gaps across all 21 public tables. CLASS RULE: grant totality is proven per-privilege, never per-row-existence.
