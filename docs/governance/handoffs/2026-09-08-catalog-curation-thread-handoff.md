...
## 9. Addendum 2026-09-13 — manufacturer specifications (`allowed`), unit spellings, and the follow-ups

`allowed` — an option may constrain a sibling SELECT attribute's values: `"allowed": {"<select attribute_key>": ["value", …]}` (≤5 targets, ≤50 values each), written only when non-empty and only on the option that owns the fact. The target must be a single- or multi-select definition linked in at least one category where the owner is linked (the restriction applies wherever both are present — DEC-057b); never the owner, its parent or a definition that depends on it; every value must exist in the target's list (a file may create the target in the same import). Use it for manufacturer facts you are sure of: a phone model's storage tiers, a series' RAM tiers, a car model's fuel and transmission sets; leave it off where the market carries variants. Numeric facts (engine cc, year) stay in `bounds`. At posting, sets fold by intersection; an empty intersection is refused by name, so do not author conflicting sets on a brand and its model.
...
Pass mechanics (2026-09-14): a pass that links into a category whose primary lineage lies outside the vertical (a secondary-parent child such as Fashion's Kids' Clothing) imports its attributes with the category filter set to All categories — a filtered import refuses those rows by design. A pass with unlinks imports definitions and links together — a definitions-only import refuses every deletion whose links still exist. A retire on an already-inactive category previews as unchanged. Categories files are built only from a categories export, never inferred from links — a slug may exist with no links.

Attestation booleans (2026-09-14): a boolean that records a seller's claim (halal, fasting-friendly, certified, organic, authentic, imei_registered) is labelled "(seller's statement)", carries help text saying "tick only if…; unticked makes no claim; the platform does not verify it", is never on a card, and is linked only where the question is real — never at a root. A root's slug is derived from its English name at creation; name it for the slug you want and rename by file.

Pass mechanics, second set (2026-09-15): retiring and reactivating a category need action = retire / reactivate — an is_active cell alone is not applied; until INC-199 lands, an action row's other cell changes are also not applied, so put renames and secondary parents on a separate change row or set them in the console. A root cannot be created by the console with a chosen slug (the slug derives from the English name — name it for the slug you want and rename by file) and cannot be deleted by file (INC-198). Every root has now been curated; the sweep runs on the same loop: review note → rulings → files → counts line → import → walk → Amharic.
