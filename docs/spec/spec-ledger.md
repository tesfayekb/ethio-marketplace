...
- **DEC-019-B (2026-09-01):** worker compatibility date pinned ("2026-08-27"); bump only with a wrangler that supports it, one landing. Nightly cloudflare parity real.

- **DEC-022-B (2026-08-31):** migration-guard allowlist — per-file, reasoned, closer-cited, printed every run. Trigger helpers default SECURITY INVOKER.

- **DEC-025/026/027 (ratified 2026-08-31; BUILT 2026-09-02):** the quality floor — typed clients + AST lint; Vitest/RTL component layer (states-test law; seed suite grown to 56 tests); e2e spec lint (.first()/only/waitForTimeout; 38 grandfathered, sweep = ACT-U4-4).

- **DEC-028 (2026-09-01):** @global-state tests serial-nightly-only; quarantined failures labeled, ledgered, non-gating; un-quarantine = 7-green streak + DEC note.

- **DEC-029 (2026-09-01/02):** CI speed — shared build artifact + six shards LIVE (≈half wall-clock); session injection PARKED behind standing knob E2E_UI_LOGIN (INC-120→120c: two seam fixes each exposed a new persona shape); helpers retained; credential-lifecycle tests pinned to UI login.

- **DEC-030 (2026-09-01):** flake ledger — matrix retries=1 (nightly 0); pass-on-retry recorded by name; 3-in-7-days ⇒ INC.
...
- **S33 (2026-09-02): TRANSLATION ERA CLOSED — U4f→U4k complete, walked, green (24 jobs; six shards; nightly green with real cloudflare parity).** U4f/-2/-3: publication gate governs choice (switcher + runtime from the languages table; flagged unapprovable; non-blocking root provider — INC-098). U4g+2..30: approve-all-reviewed; deterministic language ordering (normalize, append-on-insert, (sort,code), both twins); origin-tracked orphans — sync only orphans sync-origin (INC-105); the auth-lock saga (laws I5/I6); audit-activity index after a dump-proven statement timeout (INC-116); DB-only languages render (INC-107, monitor find); MT placeholder sentinels + one-click restore + truthful provenance (INC-115); evidence machinery hardened (nightly failure file, 5xx URLs, attempt-stamped reporter, per-project fences, self-describing dumps). U4h: per-DEVICE ★ default (device wins; account carries once; survives logout), SSR lang/dir + dynamic hreflang; locale-free structural anchors law (INC-121). U4i (+2..10): key context notes (v2 boundary limitation stated); route-resolved used-on map with freshness guard; length warnings (amber chip); ETag-cached bundle route; CSV/XLIFF export-import — exports catalog-scoped (paged), imports server-side idempotent (approved never demoted by no-ops, INC-124), batch-tagged and undoable with conflict safety (U4i-7, INC-125); pseudo-localization into admin-only zxa (never publishable); language deletion (typed confirm, cascading counts, base/public refused, INC-123); step-up owns the top layer above all dialogs; compact tinted switcher; proportional roster with Interface/Content naming + tooltips; DataTable primitive owns scroll-not-cramp (min-width contract, cardUntil='lg' — the tablet band belongs to the template now, INC-126/126b). U4j/U4k: entity universe enumeration (composite entity keys, INC-119b); data-scope AI + approve-all + chips + dual meters; guided add-language (provider list, native names, RTL, countries). Walks: om/ti content approval; the star; U4i's six items + deletion/undo/density; tablet roster. Supervisor slips logged (counting artifact; J5-dictated assertion; _ts busting). INC-098→126b closed or registered. NEXT: **S34 — U5 Categories & Locations spec session.**

---

S34 · 2026-09-02 · CATEGORY ERA SPEC SESSION (ratified)

Spec ratified and imported: /docs/governance/category-era-spec.md. Taxonomy artifacts ratified and imported to /docs/spec/category-era/ (target tree 113 nodes; 508-row live dispositions; 97-row current-prod dispositions). Live WP taxonomy export completed by operator via REST (DEC-003 pre-shutdown item: CLOSED). Era order: D0→C1→C2→C3→C4→C5; Locations era follows (DEC-033).

REQ-017 AMENDMENTS (2026-09-02):

 (a) Jobs & Vacancies and Tenders are not marketplace categories; any future jobs offering is a separate product decision with its own spec. The A1-seeded jobs root and children retire in C1 (is_active=false).

 (b) Visibility dimensions: category nodes carry is_active + optional visible_from/visible_until (timestamptz, NULL=always) + per-country exclusion rows (category_country_exclusions denylist). A browse path is visible only when every node on it is visible for the viewer's country and the current time. Posting is server-gated on visibility; listing detail pages are never gated (merchandising, not access control).

 (c) Catch-all law: exactly one "Other <Name>" child (is_catchall=true) per branching node; sorted last; hidden from browse while empty (empty-depth law applied per node); always poster-visible; inherits parent attributes; graduation via console reassign.

 (d) Transaction-type law (L1): offer type (sale/rent/lease/hire) is a required attribute on Real Estate and applicable Vehicle nodes, never a category. Type-collapse attributes replace enumerated live sub-branches per the ratified tree notes.

 (e) The ratified 2-level target tree is adopted; the tree is admin-dynamic and node counts are advisory, not spec constraints.

REQ-019 NOTE (2026-09-02): every category node carries three image variants — thumbnail (derived), card 512 (the photo-less listing fallback), large/OG 1200×630 (composed programmatically from the icon; AI-OG is fallback). One AI generation per node.

REQ-020 NOTE (2026-09-02): brand/make is modeled as a category attribute, never a standalone entity (apex precedent; ruling 2026-09-02).

DEC-031 (2026-09-02): E2E global-setup reaper gains scratch-category reap (slug prefix e2e-, older than 1h). Named harness delta under G22; pre-committed rule: reap is delete-only, fence and real rows untouched.

DEC-032 (2026-09-02): AI provider for category features is the direct Google Gemini API (key operator-held in secret stores; server-env only; needed before C5, not before). Lovable AI gateway is the named fallback via base-URL swap.
...
## S35 — Stabilisation, import/export and dependent options

DEC-036 (amended 2026-09-06) — Single-owner maintenance: the reaper, DEC-036 audit prune, storage sweep and ratified-am tripwire run only in the shard-1 job; every other job skips them (the t=0 thundering herd of nine concurrent setups is retired).
DEC-038 — Identity pool: the harness mints one pre-enrolled super admin per worker slot per job; tests declare `@private-identity` when they mutate their own auth state (impersonation, factor enrol/unenrol); a guard script refuses such tests without the tag.
DEC-039 — API roles' statement_timeout raised 8s → 30s (authenticated, anon) so pool queueing surfaces as latency the test budgets can see, not as 57014.
DEC-040 — Step-up freshness is a token fact: the client derives freshness from the JWT (aal2 + totp amr within the DB's 10-minute window); no client hint; `guard()` resolves when the action completes, including after a step-up round-trip (the SyncKeysCard result-box bug — a production phantom-nothing after a stale step-up — is fixed and regression-tested by TR-7b).
DEC-041 (amended twice) — Session per test, in node: each test signs in its worker's pooled identity by password, verifies TOTP in node, injects that session; no refresh token is shared across tests. The pool always injects; private mints and auth specs always use the real door; the E2E_UI_LOGIN knob is inert pending deletion from ci.yml and nightly-e2e.yml.
DEC-042 (candidate) — Helper/setup changes should run the smoke tier in the fast lane (today the lane runs nothing when no spec changed).
DEC-043 — Posting law for attributes: card attributes (≤3 ranked) are required at posting because they render on the listing card; every other attribute linked to the category or its ancestors is shown and optional; the listing shows filled attributes only.
DEC-044 — Inheritance law: a link applies to the category and every descendant; the console shows inherited rows read-only with their origin; the amber "needs card attributes" flag reads the effective set; the importer refuses inherited-row edits.
DEC-045 — Dependent options: `attributes.depends_on` (single_select parent), option `parent` values validated against the parent's list; cascading pickers/filters; delete/unlink/merge refused while dependents exist; importable with parent-before-dependent ordering and cycle refusal.
DEC-023 (LIVE 2026-09-07) — Lovable holds staging E2E credentials via its secret dialog; `bun run e2e:local` mirrors CI's build and runtime env (VITE_E2E, staging VITE_SUPABASE_*, E2E_UI_LOGIN=1, E2E_FAKE_TRANSLATE=1, workers), regenerates the i18n usage map first, refuses to start on an occupied 4173, and never reuses a server. Every landing touching e2e/** or a gated surface runs its specs locally before committing and pastes the summary; a run blocked by staging parity means no commit. The platform commits when the executor's turn ends — "commit" is never an operator action.
INC-160…INC-178 (summary; details in docs/tracking/incidental-findings.md): 160 CT-6 Escape-swallow · 161 admin_list_categories 57014 (C3d CTEs) · 162 sub-44px verb targets (touch size at the primitive) · 163 redirect latency = the i18n bundle's per-request aggregation (in-process cache by publication version) · 164 the same, root: Nano compute + 8s role timeout + nine-job concurrency · 165 listings never reaped (feed-empty world-assumption; reaper + scoped anchors) · 166 shared-identity session churn (deadlocks on /factors/verify) · 167 stale stepped-up-at hint reader (deleted) · 168 shared refresh token revoked the family (session per test) · 169 challenge-window expiry after 10 minutes (per-test verify) · 170 scratch-prefix anchors (all scratch slugs start with e2e-) · 171 C-2/C-3 inconsistent skip guard · 172 DataTable pagination slicing owned by the primitive · 173 language baseline on acquire = NULL (fresh-user state); switchLanguage waits on the device record · 174 local-only RP-1 sign-out reconciliation (open) · 175 EN baseline heal on staging (compiled EN is staging's truth of record) · 176 orphan function on the connected project after a lost working tree (migration and commit in the same turn; report divergence first) · 177 non-idempotent import round-trip (semantic option comparison; the real-export invariant) · 178 read-only reporter noise (one serializer; compare after normalization).
Slips (supervisor): prompts written from partial reads under time pressure (L4 family); "primitive-owned min-widths" instruction that opted the attributes table into the primitive's last-resort mechanism; a Lovable settings path named without verification; "commit it" phrasing that implied an operator action.
DEC-046 — The import gate: every import family (categories, attributes, translations CSV/XLIFF, any future one) enters through one server entry point (src/server/imports/gate.ts) declared in a registry — permission, step-up, file identity, column classes (identity | editable | read-only | action), per-column type/regex/length, row and byte caps; ingress checks (UTF-8, RFC-4180, exact headers), cell hygiene (NFC, control/bidi/zero-width stripped, NUL and formula-leading refused), typed JSONB to the planners, per-operator×family rate limit, audit per preview/commit/undo; the DB doors validate again and stay the sole authority; a CI guard refuses any import/export route or console writer call outside the gate; the shared hostile-file spec (e2e/import-security.spec.ts) is parameterised over every registered family.
DEC-047 — Guard cancel contract (DEC-040 amendment): guard() rejects with StepUpCancelled / StepUpUnavailable when the step-up prompt is cancelled or no factor exists, so every caller resets its pending state; proofs MF-7b/MF-7c.
DEC-048 (candidate, workflow) — Delete the inert E2E_UI_LOGIN knob from ci.yml and nightly-e2e.yml (DEC-041 made it meaningless); requires the harness DEC ritual.
INC-179 — A migration file was deleted after it had applied on the connected project; restored byte-identical; law: migration files are never deleted or edited once applied anywhere — a failed-elsewhere migration is superseded by a corrective that heals its mark.
INC-180 — Inherited echo rows collided with new direct rows in the duplicate-key check (IE-6: direct rows only).
INC-181 — Empty read-only cells reported as edits (IE-6: empty means "not provided").
...
## S36 — Engineering thread 2026-09-10: workflow runtime, records

DEC-048 — RATIFIED 2026-09-10. Judge met: the knob deletion landed at 51504c9 (its push run green) and the first nightly to carry it (2026-09-10, run 34445669805 on f6d70f84) was green — 478 passed, 0 gating, 1 quarantined (CI-5, nightly-standing since 09-05). The two red push runs between them (5c98b60: staging migration-parity preflight; 0d6f5c8: AT-42 "never reported Saved" + CT-4 duplicate "Scheduled") were UX-2 causes, not @private-identity behaviour. Residue: three comments in e2e/helpers/ui.ts (near lines 297, 478, 516) still place the knob in the workflows — reworded on the next e2e-touching landing (INC-174); the DEC-048 changelog line already records that nightly-e2e.yml never set it.

DEC-049 — LANDED 2026-09-10 at c6e5fe5 (push run 34451927917: 25/25 green, promoted to main). Every `uses:` of the four actions moved to the first major whose action.yml declares runs.using: node24 at the tag: actions/cache v5, actions/upload-artifact v6, actions/download-artifact v7, gitleaks/gitleaks-action v3 (upload v5 and download v5/v6 still declare node20). Newer majors (cache v6, upload v7, download v8 — ESM migrations; download v8 errors on digest mismatch) deliberately not chosen so the binary judge reads a red with the smallest cause space. GitHub's runners default to Node 24 since 2026-06-16 and remove Node 20 on 2026-09-23 (changelog, updated 2026-08-25); no opt-out is set, so the declared runtime now matches the runtime already in use. Artifact-contract glance (operator, merged job step "Verify the downloaded artifacts belong to this attempt"): eight results.json under shard-results (smoke, email, shards 1–6; the fast lane uploads nothing when no spec changed) — download v7's layout under `pattern` unchanged. Judge: push half met; nightly 2026-09-11 PENDING — green ratifies, red reverts in full (five files at c6e5fe5). guard-proof.yml's one cache line is dispatch-only and unexercised (ACT-C3-1). DEC-023 reading: a workflow-only landing touches no e2e/** and no gated surface, so no local run — G24's "every landing" is read as DEC-023's own scope.

DEC-049b (candidate) — the remaining node20 declarations: actions/checkout@v4 ×3 (ci-status-report.yml ×2, guard-proof.yml ×1; v5 is already the pin in ci.yml and nightly-e2e.yml) and actions/github-script@v7 (ci-status-report.yml; v8 declares node24); the 18 `@v4` history comments in ci.yml/nightly-e2e.yml reworded in the same touch. Same judge shape as DEC-049; rides the next workflow DEC.

INC-184 — TR-29's catalog-count invariant reads sibling workers' transient fence rows (J6): the count is taken after the export, over the whole fence language every test in the project shares; nine retry-passes in seven days, every body "N rows for an N+1-row catalog". Fix at the anchor: compare non-scratch key sets on both sides. Details in incidental-findings.

INC-185 — TR-12's waits carry no step names: twenty retry-passes in seven days and every ledger line ends at "expect(locator).toBeVisible() failed" — the locator sits on the line the ledger does not quote — so the stalling wait cannot be named. First fix inside the spec (J7): a message: per wait; then read the next seven days. Details in incidental-findings.

INC-186 — a green run never judges the artifact contract: E2E_GREEN derives from the shard jobs, the reporter returns before opening any results directory on green, and the merged job's pattern downloads are continue-on-error — a download-artifact layout change would surface only on the next red, as a false all-sources runner-death. Permanent fix is a reporter DEC under the harness ritual: the green branch reads the expected sources too and the green form carries "Sources read: n/N"; an unreadable expected source on green is silent = gating. Details in incidental-findings.

ACT-C3-1 — guard-proof.yml currency: dispatch-only, last touched 2026-08-03 (pre-harness-epoch shape), never dispatched under DEC-018/019 and the identity pool; re-prove B-3/C-4 under the current harness or fold them into the nightly, dispatch once, record the run.

Tracker: ACT-U4-6 DONE (superseded — DEC-041 made pool injection unconditional, DEC-048 deleted the knob); ACT-U4-7 DONE (admin-translations.spec.ts split into console/data/governance at d55bdca0, 2026-09-06).

Rulebooks: Lovable Project Knowledge v3.8 installed (A8: INC-183 whole re-declaration, DROP+CREATE when the return shape changes; two trims to stay under the settings box's 10,000-character cap — §1's closing sentence, redundant with the rules, and J9's inert-knob clause, moot after DEC-048; INC-179's applied-migration immutability deferred to v3.9 for space); the repo mirror docs/governance/lovable-knowledge.md refreshed whole from v3.1 to v3.8 (it had drifted since 2026-07-29 despite its same-session rule); knowledge-amendments.md records the delta; system-state Standing reads → instructions v1.9 · Knowledge v3.8.

Supervisor slips: (1) DEC-049 prompt step 5a ("zero old pins anywhere under .github/") contradicted step 2's comment freeze — the executor's narrowing to `uses:` lines was correct, no drift; class: an expected-output clause never executed against the clone before publishing, second occurrence of the S35 partial-read family → G16 addendum proposed for instructions v1.10: every expected-output clause in a prompt's self-check (counts, greps, diffs) is run by the supervisor against the clone before publishing; an unexecuted expectation is a guess. (2) The Node 20 removal date was first taken from a third party's release notes (2026-09-16); GitHub's own changelog says 2026-09-23 — corrected before any record carried it. (3) v3.8's first cut (10,337 characters) exceeded the settings box's 10,000-character cap, which v3.7 had been trimmed to meet (9,998); rebuilt at 9,996 — class: an artifact published without checking its destination's constraint.

NEXT: INC-174 (local-only RP-1 sign-out reconciliation) → Real Estate curation import (counts line before Confirm) → U6 Posting spec session (first law DEC-043; forward-scan all later REQs; operator approval before any execution prompt).

S36 addendum (2026-09-11):

DEC-049 — RATIFIED 2026-09-11. Judge met in full: push run 34451927917 green and promoted (2026-09-10); nightly run 34570333823 on b0a4bd20 (contains c6e5fe5) green — 480 passed, 0 gating, 1 quarantined (CI-5), 0 flaky, sources without results: none. The artifact-contract glance (eight results.json) stands as recorded. DEC-048 record addendum: push run #407 on f6d70f84 (2026-09-10 06:29 UTC) concluded CANCELLED with exactly one job cancelled — the fast lane — by the next push's workflow-level cancel-in-progress concurrency group (b8e6e985, nine minutes later); the full matrix and the promote job had completed green; signal-only under DEC-023-B.

INC-174 — CLOSED 2026-09-11, cause identified. Until c40b7109 (2026-09-07 17:52 UTC, IE-1r Part D — "the real door, always"), signIn injected sessions whenever sessionInjectionEnabled() held: false under the parked CI knob, true in every local run with the staging env. Injection rides an addInitScript whose INC-120 write-once sentinel lives in localStorage; RP-1's signed-out phase clears that storage, so the next page.goto re-wrote the last injected grant before the document booted — the recorded "page keeps its session after storage clearing", local-only by construction and never in-memory. INC-174 was first written on 2026-09-08 08:31, after the fix had landed, from an earlier local observation the backfill did not re-test. Verified at HEAD b0a4bd20: 2 runs × 2 projects, --retries=0, all green; no code change. Gloss for Knowledge v3.9 (J9): a pool-injected page must never test signed-out behaviour by clearing storage — the sentinel re-arms the pool session on the next document; signed-out assertions take the real door. No current spec is exposed (the only localStorage.clear() is RP-1's, on a real-door page).

ACT-C3-2 — the executor's sandbox had neither the Playwright browser runtime nor its system libraries at the start of the INC-174 reproduction (session-only install); e2e:local should verify the browser before building and fail fast with the install hint (G25: the executor's own census decides what the sandbox carries).

Flake watch: TR-34 retry-passed twice on 2026-09-10 (shard 5, toBe mismatch) — watched under DEC-030's 3-in-7 trigger; TR-12 stands at 21 in the seven-day window (INC-185).

Queue correction (operator directive 2026-09-11): the 2026-09-10 handoff's queue ran Real Estate import → U6 spec session, skipping the LOCATIONS ERA the roadmap orders after C5 (DEC-033, Next 2). Restored: remaining curation passes → Locations era spec session (Ethiopia-deep tree, diaspora seed, ancestry trigger, locations console, country activation; S34 U5-draft carried unchanged) with a posting-prerequisites census — everything posting needs before U6-A starts; known open question for the operator: C4 tags (REQ-041; spec at category-era-spec §5, no landing recorded, poster-facing consumption already deferred to the posting era) — then U6 Posting.

NEXT: Real Estate curation import (counts line before Confirm) → remaining curation passes → Locations era spec session (DEC-033) + posting-prerequisites census → U6 Posting spec session (first law DEC-043; forward-scan all later REQs; operator approval before any execution prompt).

S36 addendum 2 (2026-09-11):

Real Estate curation pass — IMPORTED 2026-09-11. The curation thread's review note (deliverable 1) yielded fourteen decisions, ruled with this thread's recommendations: D1 transaction type is the `offer_type-real-estate` attribute, one tree by property type (filters, not layers — the body-type law); Short-term Rentals and Rooms & Shared stay categories because the product differs; D2 the two sale twins retired; D3 `condominiums` new, `apartments-condos` relabelled Apartments; D4 `new-developments` new and the single home for developer/off-plan projects — `completion_status` and `payment_plan` link only there, resale half-built units use `finishing_state`; D5 commercial stays one category with `commercial_type`; D6 `offer-type-apartments-condos` replaced by `offer_type-real-estate`; D7 root ends with zero direct links; D8 type changes by file — `bedrooms` number→single_select, `furnished` boolean→single_select, `bathrooms` number→single_select with halves (verified: `type` is an editable import column and the update RPC accepts it; no listings exist); D9 `zoning` merged into `land_use` in the console (0 links moved · 1 folded into the existing Land & Plots link · 1 definition removed); D10 `rent_period` depends_on `offer_type` (parent rent) and `payment_plan` depends_on `offer_type` (parent sale) — DEC-045 beyond make→model; D11 `amenities` is the catalog's first multi_select (supported end to end); D12 nine new definitions; D13 orphans deleted, `stories` included; D14 boundary rule, decided once: rent a property → Real Estate, book a hospitality service → Travel. Standing file rules: `is_required` true only on card rows (DEC-043); `label_am` filled for every surviving definition; `other` present on open option lists. Files validated here before preview: headers and column classes exact, depth 2, no echo-row collisions, cards ≥2 on every listing category (eight with three, the catch-all with two), dependents' parents co-present, no formula-leading cells. Console counts as shown: categories `created 2 · changed 7 · retired 2 · unchanged 2 · refused 0`; attributes `85 added · 25 changed · 15 unlinked · 6 deleted · 36 unchanged · 0 refused`, `category_path` 25 rows not applied (read-only, stale after the renames); `Import applied — 131 changes written`. Walk: tree and retirements as intended, no "Needs card attributes" on any listing category (`realtor-services` stays with the Services pass), `amenities` renders with 16 options; pending Amharic approved. Left open on the pass: category images not generated ("Missing assets" — the C5 bulk fill is a separate step); O4 slug/label mismatch kept (slugs are identities).

INC-187 — REGISTERED 2026-09-11 (walk finding; phantom-success class, F4/§7): the categories import plans `display_order` but never commits it. `cat_import_plan` carries `display_order` in every changed row's delta and in every create payload; `admin_commit_category_import` calls `admin_update_category` with NULL in `p_display_order` (the door applies `COALESCE(p_display_order, c.display_order)`, so NULL keeps the old value) and `admin_create_category` has no order parameter, so creates are appended. The export marks the column editable, so the Real Estate file's five reorders and two placed creates were counted as changes, reported as written, and quietly not applied — Condominiums 9 and New Developments 10 instead of 3 and 4; Commercial, Land and Short-term unmoved. Stale `category_path` cells are at least reported under "Ignored (read-only)"; `display_order` is dropped without a word. Fix, next execution prompt and ahead of the Electronics import: the commit applies the delta's order through `p_display_order` and follows every create with the same update carrying the payload's order; catch-all rows are skipped (the reorder door pins them at 1000000+); the categories-import round-trip E2E asserts `display_order` on changed and created rows against DB truth (J4) — the walk outranks the green test (G26). Proof: the Real Estate categories file re-imported unchanged lands the intended order.

D-GATE-8 verification — DRIFT, corrected in this landing: the INC-174 closure paragraph landed with an empty line between each of its wrapped lines (content verbatim; the line-replace paste turned single newlines into paragraph breaks, where D-GATE-7's appended blocks had landed byte-identical). Landed otherwise as specified: 152c2aa, five files, green, promoted.

NEXT: INC-187 fix → Real Estate categories file re-imported (order only) → Electronics curation import (export to the curation thread now; import after the fix) → remaining passes → Locations era spec session (DEC-033) + posting-prerequisites census → U6 Posting.

INC-187 — MECHANISM CORRECTED 2026-09-11 (supervisor slip: the registered cause "the commit never commits it" was ruled from a grep window that cut before the decisive block — the class that forged the G3 addendum; first recurrence since; logged, no new rule). Creates ignore the payload's order (appended); updates commit order one row at a time, ranking each against the siblings' current numbering and renumbering after each, so a multi-row reorder does not reproduce the file's sequence; the catch-all's order cell counts as a change the door then ignores. FIX PART 1: migration 5c1e174e re-declares `cat_import_plan`, `admin_commit_category_import` and `admin_undo_category_import` WHOLE (INC-183; closers and read-backs in-file) — no catch-all order delta; one ordering pass per primary parent after all rows, on commit and on undo. Part 2 next: CT-30 asserts the sequence on commit and its restoration on undo against DB truth; then the Real Estate categories file re-imports unchanged.

S36 addendum 3 (2026-09-11):

INC-187 — CLOSED 2026-09-11 on the walk. Part 2 landed at 44f44b0f: CT-30 green on both projects locally (DEC-023) and in CI (push run 34590930417, 25/25, promoted) — a reversed sibling order plus a created row placed mid-sequence lands as the file's sequence 0..4, a catch-all's edited order cell is no change and the row stays pinned at ≥1000000, undo restores the original order and removes the created row. Proof on the real data: the Real Estate categories file re-imported unchanged — `0 added · 5 changed · 0 retired · 0 reactivated · 0 deleted · 8 unchanged · 0 refused`, `Import applied — 5 changes written` (Other Real Estate now unchanged: the planner suppression holds) — and the roster reads Houses & Villas · Apartments · Condominiums · New Developments · Commercial Property · Land & Plots · Rooms & Shared · Short-term Rentals · Other Real Estate. Executor corrections accepted in flight (G3 addendum): the plan helper's closers stay closed to `authenticated` (an ungated plan read otherwise); the planner's count keys are `adds/changes/unchanged/refusals`, not the brief's created/changed; the CAT-IE list line ends at CT-23, so CT-30 rode the Coverage line.

Supervisor slips (S36, continued): (4) INC-187's registered cause was ruled from a grep window that cut before the decisive block — the G3-addendum class; corrected in the fix landing. (5) Part-2 anchors written from partial reads (count keys; the CT-26 line) — third occurrence of the S35 partial-read family (S35, DEC-049 step 5a, here). The G16 addendum proposed at D-GATE-7 is due for installation as instructions v1.10 (operator action, §14): every expected-output clause and every anchor in a prompt is executed against the clone before publishing; an unexecuted expectation is a guess.

ACT-C3-2 — second occurrence: the part-2 local run needed `E2E_CHROMIUM_PATH=/bin/chromium` because the executor image lacks the Playwright-managed shell (first: the INC-174 reproduction). Priority raised: the browser preflight lands with the next e2e-touching landing after C3-UX-5.

C3-UX-5 — REGISTERED (operator walk finding 2026-09-11) and landed with this record: the Categories roster's filter listed roots only, in an order nobody sees; it is now a subtree picker over every category in roster order, each child indented under its parent as the attributes console's filter renders, retired categories marked; selecting any category scopes the roster and Export subtree to that category and its descendants. One shared option builder serves both consoles (B3). CT-31 proves the grouping, the marker and the scoping by structure.

NEXT: ACT-C3-2 (browser preflight) → Electronics curation import (files pending from the curation thread) → remaining passes → Locations era spec session (DEC-033) + posting-prerequisites census → U6 Posting.

ACT-C3-2 — DONE 2026-09-11 (third occurrence promoted to a guard, §11): `scripts/e2e-browser-guard.ts` runs first in `e2e:local` after the port guard, resolves the browser as the config does (`E2E_CHROMIUM_PATH`, else the managed Chromium) and fails fast with the two remedies; proven to fail on a bad path and to pass in the executor image. CI untouched (the workflows never call e2e:local).

S36 addendum 4 (2026-09-11):

Attribute system review — RATIFIED 2026-09-11 (docs/governance/reviews/attribute-system-deep-dive-2026-09-11.md, imported byte-identical, sha256 b8c928d03bb1ee94cf35a688344486ecb610b9c573bea11df2675f2cfede1d6a). Read against REQ-017/018/020/022/025, DEC-043/044/045, DEC-033, the C1 taxonomy and the two finished curation passes. Finding: the six types plus inline Amharic options, one-parent dependents, cards and inheritance carry every remaining vertical to curation; posting and search need additive definition fields; bookings are a category capability plus a module, not a type.

DEC-050 — Attribute definition v2 (P1, posting prerequisite): nullable `unit`, `min`, `max`, `decimals` (0–3), `format` (`plain`|`year`) on number definitions, with relative bounds from an allowlist of tokens (`year`, `year±n`) resolved server-side; option-conditioned bounds — an option record may carry `bounds: { <sibling attribute_key>: {min?, max?} }` naming a number definition linked wherever the owner is linked, folded into the sibling's effective range (the make/model → year validity window: first production year, last year of a discontinued model, never beyond next year); option `active` (inactive: hidden from new posts, valid on existing listings, kept in filters while referenced) and `aliases` (search only, ≤5×32); text `preset` from a code allowlist (`digits:n`, `vin`, `plate-et`, `alnum:a-b`, `free:n`) plus `max_length`, never free regex, never a contact-data preset; `help_text_am` added and both hints exported/imported; `range` removed from the attr_type CHECK (a dead type; returns with Jobs v2 if ever); an option-coverage read for Amharic. All cells appended after the current definitions header so files exported before P1 still import as unchanged. Constraints are validated at import (F5) and evaluated by the DEC-051 validator only.

DEC-051 — Validation authority (U6-A): one SECURITY DEFINER `validate_listing_attributes(category_id, attrs)` is the only authority for listing attribute values (effective links, cards per DEC-043, types, options incl. `active`, dependents, DEC-050 bounds and folds, presets, multi membership, the `other` shape); `listings.attrs jsonb` holds typed values with `other` stored as `{ value: "other", text }` (C3-UX-4 solved in the shape); a definition's type change and an option's deletion are refused once any listing references them; the client mirrors the checks for UX only.

DEC-052 — Category capabilities: `capabilities text[]` on categories with a CHECK allowlist (`bookable` first), exported/imported as one cell, inherited like other category behaviour, landed at U6-A; the Bookings module — availability, reservations state machine, notifications, conversations, RLS per party, request-only under DEC-001 — is its own era (candidate U9) after U8 messaging.

DEC-053 — Option delivery: option lists are delivered per definition, lazily, version-cached (REQ-029); the 400-option cap is raised only on the Electronics note's evidence.

Standing curation rules (curation handoff §7): R1 global-before-vertical definitions — search the library first; reuse `condition`, `color`, `brand`, `warranty`, `swap_exchange`, `rate_period` where the option set matches; suffix `-<vertical>` only when the set is vertical-specific. R2 fixed unit per definition; bounded sets as single_select; open numerics as `number` with the unit in the label until DEC-050 lands, then backfilled by one import per finished vertical.

Sequencing (operator directive 2026-09-11, supersedes the morning order): Electronics import (files need none of P1) → DEC-050 Pass-2 spec → P1 landings (ESTIMATE 3–4) → backfill imports for Vehicles, Real Estate, Electronics → the eleven remaining passes with the new cells (proposed order: Fashion · Home & Garden · Services · Travel & Accommodation · Construction Material · Beauty & Personal Care · Sports & Leisure · Agriculture & Farming · Babies & Kids · Pets & Animals · Commercial Equipment; residue: Auto Services cards → Services, heavy-machinery models → Commercial Equipment / Vehicles delta) → Locations era (spec drafted in parallel; build interleaves when the curation thread is slower than the executor) → U6-A (DEC-051, DEC-052 column) → U6-B..E → U7 → U8 → Bookings era. Rationale: each vertical curated before P1 needs a backfill; each curated after fills the cells once, by the thread inside the domain.
...
DEC-050 — L2a LANDED 2026-09-11: migration `ee224de0` re-declares `attr_import_plan` and `admin_set_card_attributes` WHOLE (INC-183) to refuse a direct card rank equal to an inherited rank in the same category, naming both origins (`rankInherited`); proofs in-file. L2b (gate, registry, export, refusal keys, hostile-file rows, AT round-trips) next.

LINTER CLOSERS LANDED 2026-09-11: migration `9d74f733` (mark `20260911230000`) revokes anon/PUBLIC EXECUTE from `has_permission`, `get_my_permissions` and the four guard bodies (`rls_auto_enable`, `role_permissions_core_lock`, `roles_system_lock`, `user_roles_protect`); ACLs only, proofs in-file; linter 125 → 115. Remaining: five intentional anon catalog/gate reads, 108 gated admin doors, `pg_trgm` in `public` (accepted), leaked-password protection (operator toggle in the Auth dashboard).

DEC-054 — RATIFIED retroactively 2026-09-11 (operator-directed, landed at 66769420 outside the prompt path): the migration-parity preflight reads DEC-022 healers transitively so a healed environment is never read as behind; the migration guard's allowlist records the healed L2a file citing its healer. Pre-committed judge: tonight's nightly green and the self-test green on the next push, else revert 66769420 and the allowlist entry in full. Proof landed with this record: `--self-test` in the migration-linter job. Process note: a Tier A ACL change (9d74f733) and a harness change (66769420) were landed on direct operator asks without census or supervisor verification before the next landing — verified after the fact and safe; Tier A and harness asks route through the prompt path.

DEC-050 — L2b-mig LANDED 2026-09-12: migration 49399e15 re-declares `attr_export_payload(text)` WHOLE to emit the nine v2 cells after `depends_on` (IE-3b: the route formats nothing, so L1's scope had to include the serializer — supervisor slip, unexecuted anchor, logged); option records keep jsonb's canonical key order and pre-P1 rows export byte-identically because storage is normalised (`active` only when false; `bounds`/`aliases` only when non-empty). The "fixed key order value, label_en, label_am, …" wording in the L2b brief is superseded by jsonb's order. L2b (registry, gate, dialog keys, IG-2 rows, AT-44/45) next.

DEC-050 — L2b LANDED 2026-09-12: the definitions file gains the nine v2 cells after `depends_on` (registry declares them, the gate checks SHAPE only, the export header echoes them, the route still formats nothing). Option records are keyed to seven fields (`optionKey`/`optionShape` refusals) and their defaults — `active: true`, empty `bounds`, empty `aliases` — are normalised to silence so a spelled-out file round-trips as a no-op while an untouched export stays byte-identical. `reasonLabel` resolves `reason.<id>.<token>` from the door's detail and spends cell/value/option/argument inside the sentence, falling back to the row-naming message when no detailed key exists; categories and translations imports render unchanged. EN+AM keys for the whole v2 vocabulary. Proofs: IG-2 shape hostility (badBound, badPreset, optionKey, optionShape), AT-44 (commit → DB truth → export echo → round-trip no-op → undo to silence), AT-45 (spelled-out defaults are a no-op; a bad option names its row and itself); AT-20/AT-21 unchanged. Semantics stay the planner's (`attr_cell_check`, `attr_option_shape`). L3 (the console's editing UI) next.

DEC-050 — L2b FIX 2026-09-12: run 34667738390 red on AT-15 (literal pre-P1 definitions header; evidence file cited) — expectation updated to the v2 header; missed because the prescribed local run narrowed to `IG-|AT-2|AT-4` (supervisor slip, S36 (6); rule: a local run covers the whole spec files that assert the changed surface, never a grep subset). DRIFT on L2b: AT-45 landed as "spelled-out option defaults are a no-op and a bad option is named" instead of the rank-rule proof, without a limitation line; the rank proof lands here as AT-46 (`rankInherited` by import and by door, naming both origins). Green locally on both projects, whole spec files.

DEC-050 — L3a-mig LANDED 2026-09-12: migration 2bfcc0e7 re-declares `admin_list_attributes` WHOLE to return the nine v2 cells (the reader belongs to the same surface as the export payload and the doors — supervisor slip, unexecuted anchor, second in the class after L2b-mig); L3a next.

DEC-050 — L3a LANDED 2026-09-12: the definition editor learns v2. Number and Text groups mount per type in `components/attribute-v2-fields.tsx` (B4: the dialog was already 843 lines) and clear on a type change; `format = 'year'` disables decimals at 0; the preset builder emits only the door's tokens and serializes an incomplete builder to null; help text EN+AM at 240 with counters; every v2 field is sent on every save (the door assigns them directly on UPDATE, so an unsent field would erase a curated value — the reason L3a-mig existed). The library gains the Amharic option-coverage column at the `detail` tier, amber and `data-incomplete` while incomplete, report-only. Proofs: AT-47 (create → DB truth → pre-filled reopen → type switch → preset/max_length land, number cells null), AT-48 (1/2 → 2/2, desktop band). Whole-spec local run on both projects: 93 passed, 1 skipped (AT-48 mobile by contract), 0 failed. L3b (option-row editing: active, aliases, bounds) next.

DEC-050 — L3b LANDED 2026-09-12: THE OPTION ROW EDITOR, fixing INC-188 (the values textarea erased `label_en`/`label_am` and, since L1, `active`/`bounds`/`aliases` on every console save). Options are now rows in `components/attribute-option-rows.tsx` (B4) carrying value (locked once stored), both labels, the active switch (off = muted row + translated tag), up to five 32-character aliases and per-target bounds over the library's number definitions; dependents stay grouped by parent value; stored options deactivate rather than delete. The writer emits the door's strict record (`active` only when false, `bounds`/`aliases` only when non-empty), so a no-edit save is a no-op on the row. No client-side co-linkage rule: `boundsTargetNotNumber` / `boundsTargetNotColinked` render through `attribute-dialog-error`, naming the target. Proofs: AT-49 (no-edit save preserves every field, coverage unchanged), AT-50 (label + alias + deactivation + added option, read back), AT-51 (named co-linkage refusal, no trace). AT-2 and AT-31 re-anchored on the row editor. No migrations, no gate/registry, no workflows.

S36 addendum 5 (2026-09-12):

DEC-050 — LANDED IN FULL 2026-09-12: L1 (schema, doors, planner; migration d42d6246), L2a (effective card-rank rule; ee224de0, healed by 17074bc4 after a DEC-022 mark slip the guard caught), L2b-mig (export payload; 49399e15), L2b (file format, gate shapes and normalisation, refusal vocabulary EN+AM, IG-2 rows, AT-44, AT-46), L2b FIX (AT-15 v2 header; AT-46 as the rank proof after AT-45 drifted), L3a-mig (reader returns the cells; 2bfcc0e7), L3a (editor groups, help EN+AM, coverage column; AT-47/48), L3b (option row editor; AT-49/50/51; INC-188 fixed), L4 (this record; curation handoff §8; AGENTS.md laws). Supervisor slips logged: L1's scope omitted the export payload and the reader (unexecuted anchors — the whole definitions surface is doors + planner + export payload + reader); the L2b local-run grep excluded AT-15; the "fixed key order" clause contradicted jsonb's order. Executor corrections accepted: help_text_en predates P1; the door stores options as passed (normalisation is the gate's and the console's); the planner's closers stay open to `authenticated` per the preview semantics.

INC-188 — registered and fixed in L3b (see incidental-findings): the console's option save had been erasing option labels and, since L1, the v2 keys; the row editor round-trips every field (AT-49). Rule (AGENTS.md): a console save never drops a field it does not show.

DEC-054 — RATIFIED 2026-09-12: nightly 34678226906 green (490 passed, 0 gating) with the preflight self-test green on every push since 3f28e363; the intermediate run on a016153d failed at the migration linter for want of bun and was cancelled by the follow-up push, as predicted.

Flake watch: TR-12 continues to retry-pass (INC-185); the L2b-fix and L3a runs each added retry-passes — tallied at the next DEC-030 review.

NEXT: backfill imports for Vehicles, Real Estate and Electronics (files from the curation thread; counts line before Confirm) → the eleven remaining passes with the new cells → Locations era spec session (DEC-033) + posting-prerequisites census → U6 Posting.

C3-UX-6 (2026-09-12, operator walk of L3b): a long locked option value overflowed its cell into the label inputs at 1280 — fixed at the row grid (min-w-0/overflow-hidden cells, break-all values with a title, fractional columns). Ruling recorded from the same walk: the attributes editor carries English and Amharic only; further enabled languages reach option labels and definition names through the Translations console, never as more editor columns — C3-UX-3 therefore designs the option-label store in entity_translations keyed by attribute + option value for languages beyond Amharic. L3b walk accepted: labels intact after a no-edit save, inactive tag, alias saved, dependents grouped per parent, coverage 16/16.

DEC-057 — RATIFIED 2026-09-13 and L1 LANDED: option-conditioned allowed values — an option record may carry `allowed: { <select attribute_key>: [values] }` (≤5 targets, ≤50 values each); the target is a select definition co-linked wherever the owner is linked, never the owner, its parent or its dependents; values must exist in the target's list; folded by intersection at posting (DEC-051), an empty intersection refused by name. Migration 8909c981 (shape, door, normalizer) + b1a4891a (planner) re-declares `attr_option_shape`, `admin_upsert_attribute`, `attr_import_plan`, `attr_option_norm_v2` WHOLE with `attr_allowed_check` as the single rule; export payload and reader unchanged (read-back). L2 (gate, refusal keys, hostile rows, AT-52) and L3 (row-editor picker, AT-53, handoff §9, DEC-055/056/058 records) next.

DEC-057 — L2-mig LANDED 2026-09-13: migration 0b690297 gives `attr_allowed_check` a plan overlay (`p_defs`), so a file that creates or edits an `allowed` target in the same import resolves the target through the plan, as co-linkage already did; `attr_import_plan` re-declared WHOLE to pass it; the door unchanged by read-back. L2 (gate, refusal keys, hostile rows, AT-52) next.

DEC-057 — L2 LANDED 2026-09-13: the gate admits `allowed` (shape only; `{}` normalised away); every new refusal id and detail translated EN+AM; IG-2 rows per rule; AT-52 proves the same-file owner + target round-trip, the export echo, undo, and the named refusal. INC-189 registered (teardown failure after green tests, no evidence body) — DEC-059 candidate: teardown never reds a green shard, reporter captures post-test errors; judge next push + nightly. L3 (row-editor picker, AT-53, curation handoff §9, DEC-055/056/058 records) next.

DEC-057 — LANDED IN FULL 2026-09-13 (L1 8909c981+b1a4891a, L2-mig 0b690297, L2, L3 code at 0c3c5063 with these records): option-conditioned allowed values end to end — shape, door, planner with the plan overlay, gate, vocabulary EN+AM, IG-2 rows, AT-52/AT-53, the row-editor picker, curation handoff §9. DRIFT logged: L3's records landed one paste late (the executor lost the verbatim text; re-supplied), and its two spec files ran locally in separate runs. DEC-055 — RATIFIED 2026-09-13, built with the Locations era and posting: values stored in one canonical metric unit per family (unit catalog with factors; canonical spellings in handoff §9), posters enter in their country's unit, viewers see their country's unit with a toggle to the unit as posted; `unit_system` lives on the country profile; no curation-file impact. DEC-056 — RATIFIED, built at U7: display-only price conversion from a daily `fx_rates` table with a heartbeat, original price first, rate labelled, no payments (DEC-001). DEC-058 — RATIFIED, built after U7: a catalog inbox of signals (other-text captures, zero-result searches, unknown brands in titles) approved through the existing doors; the curation Project stays the research arm; never an autonomous writer. INC-190 registered (TR-34 whole-map snapshot; DEC-030 trigger met). NEXT: DEC-059 (INC-189 teardown resilience + reporter capture) with the INC-184/190 spec fixes → Vehicles follow-up file → Real Estate and Electronics backfills → Fashion.


DEC-059 — RATIFIED 2026-09-13 (harness ritual; landed at 1e7e04f7 with these records): the teardown never fails a green shard (three retries with backoff, then a warning for the reaper; out-of-namespace still a hard error); the reporter captures post-test lines from every source's log tail (red: "Post-test errors"; green: "Post-test warnings: n"), fixture captured from attempt 1 of the shard-6 log of run 34741970648; TR-29 and TR-34 scope their invariants outside the scratch namespace (INC-184, INC-189, INC-190 closed). Pre-committed judge: the landing's push green (met) and the next nightly green, else revert in full. Verdict logic unchanged (INC-186 remains the reporter's green blind spot). DRIFT logged: a scratch root `roadmap.md` and a self-test-written evidence file were auto-committed mid-turn — both corrected here; INC-191 registered and fixed. Standing rule: every prompt's VERBATIM record blocks are written to a scratch file before editing (step 0). ACT-C3-2 worked as designed (the browser guard refused and named the remedy).

2026-09-13 board — run 34753266967 red on TR-24 (INC-193, J6 global count) and the deletion guard (INC-192, unpassable by the executor); INC-194 (post-test window) found in the same evidence. All three fixed in one landing: the guard's manifest door (`docs/tracking/intentional-deletions.txt`, declaring the root roadmap.md), TR-24 on its own rows, the reporter's window at the final summary. DEC-059's judge continues (nightly pending) — these are riders under its rule.

DEC-057b — RATIFIED 2026-09-13 (operator walk: `storage-phones-tablets` refused under `series-phones` because Feature Phones carry no storage): co-linkage for `bounds` and `allowed` targets now means at least one shared category (the constraint applies wherever both attributes are present); a target linked in none of the owner's categories keeps the existing refusal. Migration ceabcf99 re-declares the judging helpers whole; callers unchanged by read-back. DEC-057b (copy, tests, docs) next; then the Vehicles follow-up file and the backfills.

DEC-057b — LANDED 2026-09-13: tests moved to the rule's edge (IG-2 and AT-51/AT-53 refuse a target linked in none of the owner's categories and accept one linked in some); refusal copy and the two constraint buttons reworded EN+AM ("Limit a number…", "Restrict a list…"); attributes.md and curation handoff §8/§9 corrected. The migration-first landing produced a predicted red window on IG-2 (j) — closed here; lesson logged: when a rule change flips an existing probe, the spec change lands in the same push as the migration is applied, or the probe is flipped first.

C3-UX-7/8/9 — LANDED 2026-09-13 at ef0a5c89, reconciled here (operator walks of the Vehicles and Real Estate backfills): option-row search and parent filter for long lists; the constraint pickers restricted to co-linked targets (they had offered every definition in the library; a definition linked nowhere yet sees every candidate — executor decision, accepted); the attributes import dialog previews without a links file. DRIFT at the first landing: the filter-then-save proof was missing (AT-55 added here), the local run was narrowed to four tests, the verbatim records were replaced by the executor's own words and docs/features/imports.md was edited outside scope — all reconciled; standing rule from this: code landings carry only their changelog line, records land in a separate docs paste after verification. Real Estate backfill IMPORTED 2026-09-13: changed 28 · unchanged 0 · refused 0, walk correct, two relabelled definitions' Amharic approved; D1 lot ceiling 1,000 ha, D4 units moved from labels to the unit cell, seven `allowed` records (bungalow → G+0; condominium schemes → certificate or pending). Vehicles follow-up IMPORTED 2026-09-13: changed 4, 209 `allowed` records on the four model lists, `isuzu_npr` carries ኦባማ / Obama / ክንጅት / Kinijit. Next: Electronics backfill (Ge'ez series labels, storage/RAM sets), then Fashion.

2026-09-14 — THE THREE BACKFILLS ARE COMPLETE. Electronics backfill IMPORTED: changed 42 · unchanged 0 · refused 0 (a first preview refused one row on an alias shared across two brands' series — the per-definition uniqueness rule; brand-qualified and re-delivered), 42 written; 136 product labels to Ge'ez with 233 Latin aliases; `battery_capacity_mah` at mAh 300–30000 (charge family added to §9); free:60 on the two text fields; both help texts on all 42; `operating_system` gains harmonyos/watchos/wear_os; `color` bilingual (a shared Fashion definition, filled ahead of its pass by ruling); 94 `allowed` records — iPhone storage on eleven Apple options, OS per series (Android 66, Huawei android+harmonyos, iOS 10, iPadOS 4, macOS 5, Windows 2, watchOS/Wear OS) — brands clean, PC series with ChromeOS variants and feature-phone-capable brand series left free; zero bounds (no year attribute in the vertical); walk correct; Amharic approved. With Vehicles (backfill + follow-up) and Real Estate, every curated vertical now carries the DEC-050/057 cells and specs. Checkpoint handoff docs/governance/handoffs/2026-09-14-engineering-thread-handoff.md imported (sha256 13652c23…), superseding 2026-09-10 (the 2026-09-12 checkpoint was never imported — the operator chose to continue the thread). NEXT: Fashion (export → review note → files → counts line → import → walk → Amharic), then the remaining ten in the ruled order; riders when idle: INC-185, INC-186 reporter DEC, DEC-049b, ACT-C3-1; after the eleven: library sweep → Locations era (DEC-033) + posting-prerequisites census → U6.

2026-09-14 — THREE MORE PASSES IMPORTED (curation Project, rulings in this thread). FASHION: categories 1 added (Accessories; Kids' Clothing already existed as a Babies & Kids child with Fashion as secondary parent, left byte-identical; a name_am fix for Jewelry & Watches landed in a second file); attributes 37 added · 14 changed · 9 unlinked · 7 deleted · 30 unchanged, imported with the filter at All categories; size and shoe-size cascades under size_system / shoe_size_system (International, EU, US, UK, Kids by Age; EU / US Men / US Women / UK) keeping every existing value; gender-fashion replaces three gender keys; material to a select with Chiffon and Shemma; traditional_type, uniform_type, bag_type, jewelry_type, jewelry_material (21K first), brand-fashion led by the Ethiopian houses; root condition-fashion at rank 2. HOME & GARDEN: categories 3 Amharic renames; attributes 28 added · 17 changed · 3 unlinked · 2 deleted · 14 unchanged; the Appliances shelf filled — appliance_type with the injera mitad, appliance_brand from the live market list, power_source with fifteen electric-by-definition allowed sets, voltage, capacity L, load kg, power W; furniture dimensions in cm; product_origin; cookware_material with ሸክላ; boundary rulings: every powered device is an Appliance, garden furniture is Furniture, garden tools stay while power tools go to Construction Material; one Appliances category by the attribute test. SERVICES: categories 1 added (IT & Tech Services), 5 Amharic repairs (one label had said "kidney and hormone medicines"); attributes 31 added · 13 changed · 5 unlinked · 3 deleted · 71 unchanged; the listing model for providers — pricing_type as the root rank-2 card (per hour / day / job / month / quote / negotiable), delivery_method, availability (multi), provider_type, experience_years, service_area_km, languages_spoken (thirteen), certified as a seller attestation; thirteen service-type lists with the Ethiopian additions; two cards per category by ruling; peers recorded in the change note (Jiji Ethiopia/Nigeria, Thumbtack, Bark, TaskRabbit) with improvements for later eras (request-for-quote, verification badges on top of the attestation, reviews, portfolio images, slot booking). Browse-era item recorded: the marketplace rail must reveal child categories on hover/tap (U7). Assets badge (icons/images) outstanding on several categories — the Images console's task. NEXT: Travel & Accommodation → Construction Material → Beauty & Personal Care → Sports & Leisure → Agriculture & Farming → Babies & Kids → Pets & Animals → Commercial Equipment → library sweep.

INC-196 — L1 LANDED 2026-09-14: admin_commit_attribute_import re-declared whole with two-pass link writes (clear changing ranks → unlinks → adds and final states), audit rows unchanged in shape so undo is untouched (read-back); proofs for a rank shift, a rank swap, an add-over-move and their undo. Migration 9fc2d238. L2 (the dialog surfaces the error key and detail; AT-58 imports a rank swap through the route) next. Travel's import was completed by hand-clearing seats' rank first; the files themselves were correct.

INC-197 — LANDED 2026-09-14: undo two-pass link restoration (migration 23e2010b); the commit unchanged by read-back. Lesson logged with INC-196: any loop that writes card_rank inside one transaction clears before it sets — commit and undo both, and any future writer.

2026-09-14 — THREE MORE PASSES IMPORTED; TWO COMMIT DEFECTS CLOSED. TRAVEL & ACCOMMODATION: categories 1 added (Event & Wedding Venues, secondary parent Services), 6 changed incl. the four Amharic repairs and two renames (Tour Packages & Guides; Travel Agents, Tickets & Visa); attributes 57 added · 2 changed · 12 unchanged — pricing_type-travel at the root rank 2 (per night / person / package / day / event / hour / quote), accommodation set (type, star rating as a seller statement, room type, guests, beds, meals, amenities), tour packages (circuit types with place aliases, duration days, group size, includes), agents (Hajj & Umrah), vehicle-hire re-ranked (model to 3, seats off the card; driver_included, hire_vehicle_type), venues (wedding hall, capacity), dining (cuisine with ፆም), attractions; bookable column recorded for DEC-053 (seven bookable, two not). The first attributes commit failed on a rank collision (INC-196) and was completed after clearing seats' rank by hand. CONSTRUCTION MATERIAL: categories 3 added (Paint, Ceiling & Finishes; Doors & Windows; Power & Hand Tools), 4 changed; attributes 68 added from an empty vertical — unit_of_sale at the root rank 2 (twelve units: bag, quintal, ton, m², m³, piece, sheet, litre, can, metre, truckload, set), per-category type at 1 and price-determining spec at 3 (brand for cement/paint/tools, rebar diameter, tile size, sheet gauge, thickness, pipe size, cable size, door width), four numbers in mm/m, five brand lists with the Ethiopian mills and makers; one preview refused a new key wearing an existing label (width_mm "Width" vs width_cm) — relabelled Board Width; boundary: power tools here, site machinery stays Commercial Equipment, heavy machinery Vehicles. BEAUTY & PERSONAL CARE: categories 2 added (Tools & Devices; Traditional & Natural), 2 changed; attributes 70 added · 13 changed · 5 unlinked · 4 deleted · 21 unchanged — the root emptied (no product field reaches Personal Care Services), condition Sealed/Opened and brand direct at each product category, two brand lists (beauty; fragrance in Jiji's facet order, Lattafa first, with three Ethiopian houses), scent family, volume mL, weight g, authentic_original as a seller attestation, expiry_year (year…year+5), skin tone, Personal Care Services on Services' pricing_type/provider_type/certified/delivery_method with ሽሩባ, Morocco bath and bridal; Sexual Wellness NOT in v1 by ruling (a policy decision for later); ኩሽ dropped pending a gloss. INC-196 CLOSED (commit two-pass link writes 9fc2d238; dialog shows the forwarded reason; AT-58) and INC-197 CLOSED (undo two-pass 23e2010b; AT-58 asserts both undos). Law: any loop that writes card_rank inside one transaction clears before it sets. Note: the Import gate guard job failed once on its environment (run for 0ed66b21) and passed on re-run — watched; a second occurrence gets a class rule. NEXT: Sports & Leisure → Agriculture & Farming → Babies & Kids → Pets & Animals → Commercial Equipment → library sweep.

DEC-049b — LANDED 2026-09-14 under the DEC-049 rule: actions/checkout@v4 → v7 ×3 (ci-status-report.yml ×2, guard-proof.yml ×1), actions/github-script@v7 → v9 (ci-status-report.yml) across ci/nightly/guard-proof/ci-status-reporter; judge: this push's run green and the next nightly green, else revert in full. Nothing else in the workflow graph changed (G22).

2026-09-14 — SPORTS, AGRICULTURE AND A NEW ROOT. SPORTS & LEISURE: categories 2 added (Musical Instruments; Games, Art & Hobbies), 3 changed; attributes 26 added · 9 changed · 5 unlinked · 2 deleted · 15 unchanged — the root keeps only condition-electronics at rank 2, fitness rewritten from the market's own type and brand facets (treadmill first; Reebok, Bodygym, York), one sport picker, item weight in kg and max speed in km/h (no horsepower field: kW is canonical and nobody quotes it), books with genre/language/format, the five Ethiopian instruments with Latin aliases, chess and ዳማ; rulings: sportswear and footwear are Fashion, no club list, bicycles stay Vehicles and surface here (secondary parent set in the console). AGRICULTURE & FARMING: categories 2 added (Coffee; Food, Spices & Beverages — since moved), 5 changed incl. two reactivations (Fertilizers & Agrochemicals; Animal Feed & Supplements); attributes 36 added · 17 changed · 8 unlinked · 3 deleted · 17 unchanged — unit_of_sale-agri at the root rank 2 (quintal, kg, bag, ton, litre, head, dozen, tray, bundle, piece, sack), the cow-is-refurbished condition unlinked and the leaky equipment list deleted, a 53-option breed cascade under animal type with parent-qualified aliases, coffee by ECX origin/grade/processing/form, teff by colour, harvest_year year-3…year, horsepower kept as a bounded select (market-convention exception to kW), seeds-inputs surfaced to Home & Garden. FOOD & BEVERAGES — a fifteenth ROOT (food-drink), commissioned because the diaspora and the marts that serve it would never look under Agriculture: units decide the boundary (quintal, head and bag are Agriculture; oz, lb, jar and pack are Food); ten leaves (Spices & Blends; Grains, Flours & Teff; Injera & Bakery; Coffee & Tea; Honey, Butter & Oils; Meat, Dairy & Eggs; Snacks & Sweets; Beverages; Frozen & Prepared; Packaged & Imported — the moved category — and a catch-all); categories 10 added · 2 changed; attributes 57 added · 1 changed · 2 unlinked, then a follow-up 8 added · 2 changed · 2 unlinked scoping the halal and fasting attestations to the categories where the question is real, relabelled "(seller's statement)" with "unticked makes no claim" help; the retail model — unit_of_sale-food at rank 2, net weight in g (imperial is DEC-055 display), fulfilment as a multi-select (pickup / local delivery / ships nationwide / ships internationally), origin, brand with Homemade first, coffee origin and roast level, expiry year. DEC-060 — RATIFIED 2026-09-14: no alcoholic-beverage or tobacco listings in v1 (no age gate, no seller licence attestation, no payments); brewing ingredients (gesho, bikil) allowed as groceries; the screening gateway (REQ-021) inherits the rule; an Alcoholic branch lands only with an age-gate DEC. The console auto-suffixed the first root slug (food-beverages-2); recreated as food-drink via the derived name "Food & Drink" and renamed by file; the empty root retired in the console because the file's delete previewed as unchanged (INC-198). Product items recorded for the Locations/U7 eras: per-country ordering of the roots (Food, Vehicles, Real Estate, Travel first for a diaspora reader) alongside excluded_country_codes; store-level attestations (halal, fulfilment, ships-on-Monday rules) belong to the storefront, not each listing. NEXT: Babies & Kids → Pets & Animals → Commercial Equipment → library sweep.

2026-09-15 — THE CURATION PROGRAMME IS COMPLETE: every root has been through the loop. BABIES & KIDS: categories 1 added (Feeding, Bath & Care), 3 changed, Baby Clothing retired (already inactive), Nursery Furniture reactivated by hand (see INC-199); attributes 19 added · 8 changed · 10 unlinked · 7 deleted · 24 unchanged — Kids' Clothing's four genders and three sizes reduced to the Fashion rows, the root supplies condition-babies-kids at rank 2 and gender-fashion, one eight-band age range, strollers and car seats with weight limit and a safety-certified seller's statement, brand-baby led by Mastela and Kidilo; baby food surfaces from Food's Packaged & Imported (secondary parent set in the console). PETS & ANIMALS: categories 4 changed, Fish & Aquariums and Pet Services reactivated (action = reactivate), Cats retired; attributes 35 added · 9 changed · 17 unlinked · 13 deleted · 22 unchanged — the root emptied, one Dogs & Cats category with a species → breed cascade in the market's own order, pet_age bands, animal_sex reused, three care attestations as seller's statements, listing_purpose with Free to Good Home and Rehoming, pet services owned here and surfaced to Services (secondary parent set in the console after the file's cell was dropped — INC-199), condition-audio-sound deleted. DEC-061 — RATIFIED 2026-09-15: only domestic companion animals may be listed (dogs, cats, the listed caged birds, aquarium fish, rabbits, guinea pigs, hamsters); no wildlife or protected species and no wildlife products; no animals under eight weeks; no fighting dogs or stud-fee auctions of banned breeds; no international shipping of animals; care facts are seller's statements; no health guarantees anywhere in the catalog; the species lists carry only the permitted animals and the screening gateway inherits the rule. COMMERCIAL EQUIPMENT: categories 2 added (Generators & Power Equipment; Salon & Barber Equipment), 5 renames (Site Machinery & Scaffolding; Office Furniture & Shop Fixtures; Restaurant, Café & Bakery; Medical, Dental & Lab; Industrial & Manufacturing Machinery); attributes 23 added · 11 changed · 7 unlinked · 7 deleted · 27 unchanged — the five remaining mis-keyed conditions deleted and condition-electronics at the root at rank 2 (the apex-era condition family is now consolidated on one key for products), power_source gains Diesel and voltage gains 380 V three-phase (Home & Garden's definitions, links unchanged), power_kw as the canonical power number, generators with kVA bands and the market's brand order, site machinery with sale/rent and unit of sale for scaffolding, printers and heavy machinery pointed to their owners; Electronics' Printers & Office Electronics surfaces here (secondary parent set in the console). DEC-049b — RATIFIED 2026-09-15: nightly 34937518300 green under the bumped actions; the reporter advanced the status file under github-script@v9. INC-199 registered. TOTAL: fifteen roots, the eleven passes plus Vehicles/Real Estate/Electronics backfills and the Food & Beverages root, all with DEC-050/057 cells; Amharic approved after every import. NEXT — THE LIBRARY SWEEP: INC-198/199 planner fixes first; then, by curator file on the operator's rulings, the root order (similar roots adjacent), a keep/merge review of the fifteen roots with market numbers, deletion of retired categories with no listings and no children, the queued key renames (product_type-seeds-inputs, furniture_type-baby-gear, service_type-pet-supplies-services, condition-electronics → a global product condition, Realme/Xiaomi/OnePlus descriptor labels), and per-country root order recorded for the Locations era; then the Locations era spec (DEC-033) with the posting-prerequisites census.

INC-198/199 — L1 LANDED 2026-09-15 (migration de5a1373): the category planner and commit re-declared whole — empty-root delete, statusNeedsAction refusal, action rows carrying their cell changes; proofs for each and their undo. The sweep's delete pass waits on L2's test coverage.

INC-200 — LANDED 2026-09-15 (migration 42ea3ded): admin_upsert_attribute and attr_import_plan re-declared whole — a type change clears inapplicable cells (text cells off non-text, number cells off non-number, options off text/number/boolean), previewed as clears, undone with the type; proofs for text → multi_select and number → single_select. The compatible_make conversion (Universal + the 44 makes + Other) lands as a one-row file next.

2026-09-15 — LIBRARY SWEEP IMPORTED; THE CURATION ERA IS CLOSED. Categories: 13 changed (the fifteen roots renumbered 1–15 into the popularity-weighted rail), 31 deleted (twelve apex-era orphan roots incl. the seven-root Jobs cluster, nineteen pass-retired categories), 137 unchanged — zero retired rows remain, 142 active listing categories. Attributes: 25 added · 15 changed · 19 unlinked · 12 deleted · 305 unchanged, then the compatible_make conversion (1 changed) after INC-200 — six keys renamed by create · relink · delete while listings are zero (input_type, furniture_type-nursery, service_type-pet-services, age_range, unit_of_sale-construction, and condition as the global product condition on the seven-state union absorbing condition-electronics, condition-home-garden and condition-babies-kids; conditions 6 → 4), compatible_make text → multi_select (Universal, the 44 makes, Other), the three descriptor labels to "Number Series" with the old labels as aliases, Other added to fifteen open lists, the four Jobs definitions deleted; 327 definitions, zero unlinked, every card rank landed on its new key at the same position. Five Electronics leaves (Smartphones, Tablets, Feature Phones, Laptops, Desktops) found still retired from the Electronics pass's un-actioned is_active and reactivated by hand — nobody could have posted a phone. The first attributes commit failed on compatible_make (INC-200 — the type change kept preset and max_length; the commit wrote type and cells in two statements) and landed without that row; INC-200 LANDED and CLOSED (migration 42ea3ded: door, planner and commit re-declared whole; the reporter dialog showed the reason once published). Locations rail order recorded for the era: abroad Food, Fashion, Beauty, Real Estate, Travel lead; at home Vehicles, Real Estate, Electronics. Executor corrections accepted this era: INC-198 (an empty root already deleted — the "unchanged" was an unknown slug), INC-200 (the two-statement commit). Curation stays continuous: the same loop for every new brand, category or option, fed by the DEC-058 inbox after U7. Checkpoint handoff docs/governance/handoffs/2026-09-15-engineering-thread-handoff.md imported (sha256 463ba55f…), superseding 2026-09-14. NEXT: Locations era spec session (DEC-033) → posting-prerequisites census → U6 Posting.

S37 · 2026-09-15 · LOCATIONS ERA SPEC SESSION (ratified)

Spec ratified and imported: /docs/governance/locations-era-spec.md (v2; sha256 32d66532…). Executes DEC-033 — the S34 U5-draft locations section carried unchanged (Ethiopia-deep tree, diaspora seed, ancestry trigger, locations console, country activation) — on the operator's rulings of 2026-09-15 (T1–T7) and a review of apex-marketplace d920852 (coverage never persisted there, browse never widens, detection via a third-party IP API — a UI and admin-tooling donor only). Era order: G0 → L1a → L1b → L1c → L2 → L3 (curator files: countries, Ethiopia-deep, diaspora) → L4a (A7 geo-guess spike) → L4b → era gate; then the posting-prerequisites census → U6 Posting. Forward-scan: every later REQ read, none contradicted; the July posting-flow one-country rule is superseded openly by DEC-064.

Laws recorded (spec §2): tree law (four levels country → region → city → sub_city; ancestor columns country_code / region_id / city_id filled by trigger); concentration law (metro-scale cities, suburbs as aliases, one union tree across communities, a place absent from the tree is a decision); country law (countries = reference root + country profile; is_active = the market is open; identity is not a market); visibility law (a node is visible iff it and every ancestor are active; path rule in the read, never cascading writes; FKs never orphaned); naming law (name_en source, other languages in entity_translations, the name_am cell a carrier via IE-4a, the column fallback made unnecessary, column drop later on operator go); aliases law; seed and curation law (files through the one gate, never migrations; the curator authors from GeoNames CC-BY offline; nothing gazetteer-like in-app; review every quarter and at every market opening); country profile law (DEC-055 first landing); coverage law (DEC-064); guess law (DEC-063); stream law (DEC-065); persistence law (ethio_area preference cookie, profile column at U7, the guess never written); weight law (one ETag'd JSON per country; zero location queries on open).

DEC-033 — EXECUTING 2026-09-15: era opened at S37 with the landing map above.

DEC-055 — first landing named 2026-09-15: unit_system, currency_code and display_order land on the country profile at L1a; values ride the L3 countries file; canonical storage and the viewer toggle stay at U6-C/U7.

DEC-062 (2026-09-15): E2E global-setup reaper gains scratch-location reap — slug LIKE 'e2e-%', older than the DEC-036 window, children before parents, delete-only, real rows and fence languages untouched, the batch's location_import_revisions with it. Named harness delta under G22; pre-committed rule: reap is delete-only and nothing outside the e2e- prefix is touched.

DEC-063 (2026-09-15): Geo-guess source law — the visitor's starting area is a first guess from the edge's request geo facts (Cloudflare: country, region code, city): no third-party IP API, no edge function (INC-096); mapped to the curated tree country by code → region by iso_3166_2 → city/sub-city by slug or alias → else the deepest level matched; nothing matched = no guess, never a default; recomputed per request, never persisted, never a security input. Pre-committed judge for the L4a spike: cf object reachable → country + region + city; else CF-IPCountry present → country only; else none with all-null facts and no other fallback.

DEC-064 (2026-09-15): Coverage plans — a listing's coverage is a set of tree nodes at any level (listing_locations) plus an optional worldwide record; limits are admin-set per advertisement type (promotion tier per REQ-024) in coverage_plans (max_cities, max_regions, max_countries, allow_everywhere), seeded free = 1/1/1/off and changed in the console without code; posting prefills preferred posting location → last listing → the guess, always editable; U6-A enforces the plan server-side and retires A2's one-country trigger by named DEC. Supersedes the July posting-flow §Step 1 one-country rule as a hard law. All free in v1; the tiers are the pay-ready seam.

DEC-065 (2026-09-15): Stream law — the feed is one continuous nearest-first stream: the viewer's area, then the rest of the region, then the rest of the country, then worldwide, each section labelled, every card showing its place, no question and no click; match = a coverage node contains the scope or the scope contains it; nearest-first by center distance always; the manual override outranks the ladder. U7 builds the query; the Locations era guarantees coordinates on every city and sub-city, geo_distance_km, and the ancestor columns.

REQ-005 NOTE (2026-09-15): four levels; seed by file; the stream replaces the tier switch (honest labels kept); the saved area wins over the guess; the guess is never persisted; markets = open countries.

REQ-018 NOTE (2026-09-15): currency_code on the country profile is the poster's default currency; conversion display and rates remain DEC-056 at U7.

REQ-023 NOTE (2026-09-15): country_root_order is the per-country cold-start rail order (at home Vehicles · Real Estate · Electronics lead; abroad Food · Fashion · Beauty · Real Estate · Travel lead); a later learner treats it as a pin.

REQ-024 NOTE (2026-09-15): coverage limits per promotion tier (DEC-064) are the coverage-pricing seam; promotion records unchanged.

REQ-034 NOTE (2026-09-15): the tos-privacy-source ledger gains A14 (third-party data attribution: GeoNames CC BY 4.0) and B12 (preference cookies: language star and chosen area; no personal data).

---

L1a — LANDED 2026-09-15 (migrations 4e1e819a mark 20260915170000 and 55ecfe24 mark 20260915180000; run 34995568027 green incl. staging parity; promoted): the tree is four levels (sub_city); ancestor columns region_id/city_id filled by the ancestry guard and cascade triggers (rootMustBeCountry, parentMissing, levelMismatch, crossCountry, badSlug, parentInactive, missingCoordinates); aliases, iso_3166_2, display_order, source; the coordinates law on cities and sub-cities; geo_distance_km (haversine, no PostGIS); the country profile (unit_system, currency_code, display_order — DEC-055 first landing); country_root_order; coverage_plans seeded free = 1/1/1/off (DEC-064); locations:manage and countries:manage retired into view/create/update/restructure/import and view/update/activate, the new coverage resource (view/update), countries RPC-only; ten admin doors with deny proofs; the scratch-location E2E helper aligned to the four-level tree. Supervisor rulings logged (G17): countries:update split from countries:activate so a currency edit never demands step-up; countries writes RPC-only because a table-level update right cannot tell a currency edit from opening a market. Executor deviations accepted: the level CHECK matched on its value list (two constraints name level), the scratch slug strips edge hyphens, geo_distance_km search_path pinned in L1a-2 (H2), P1's extra hide step. Standing tool note: the migration tool never surfaces RAISE NOTICE and its query tool runs as a client role — proofs are exception-gated, reports carry query read-backs.

L1b — LANDED 2026-09-15 in two parts. L1b-M (55cdd205 mark 20260915190000; run 34999540741 green): location_import_revisions (deny-all), the key/path/export helpers, loc_import_plan over two files (countries · locations), admin_export_locations and the three doors (preview · commit with one-statement writes, source = import, IE-4a Amharic through the translation door, batch-tagged revisions with am_state · undo). Design refinement logged (G17): location slugs are unique per parent, not globally, so the file's identity is the slash key of slugs (ethiopia/oromia/adama) and level is derived from depth — no parent_slug column; spec §4 L1b's column list reads accordingly. L1b-C (ae6b3804 mark 20260915200000; run 35003212572 green): registry family with the decimal column type and the country-code scope kind, gate cases, POST /api/admin/locations/import (preview · commit · undo), GET /api/admin/locations/export (countries | locations), two hostile-catalogue entries (Family gains identityReason, coverage-neutral). INC-201 registered and fixed in the same landing (admin_undo_location_import re-declared whole; deleted rows return parents-first; P6 proves the ordering expression, P7 the ten untouched functions). Executor slips logged: git add -A during proof collection (harmless), types.ts reported modified though unchanged. INC-203 registered OPEN (the translation-door coupling of import commits).

L1c — LANDED 2026-09-15 in two parts. L1c-M (efbee3c4 mark 20260915210000; run 35011187482 red on TR-28 — INC-202, unrelated to the migration; re-run green): get_location_tree (open market + active anchor; a row visible only with every ancestor active, read from the ancestor columns; no recursion), get_location_tree_version, get_open_countries — anon + authenticated EXECUTE, no permission check on the public path (DEC-013 §10); P3's timestamp term proven by construction (now() is frozen inside one transaction). L1c-C (run 35013925918 green; main promoted at aa75fd56): GET /api/locations/$country behind version → in-process cache → ETag → 304 (the bundle route's pattern, bundleEtag reused), closed or unknown market = 404 never an empty 200, eleven fields and no name_am (law 5), LR-1..4 in e2e/locations-tree.spec.ts; INC-202 fixed (a thrown gate fetch is retried once; the mirror names degradation; TR-28 judges hreflang against DB truth per J4). Locations era: L1 COMPLETE; next L2 (console).

S38 · 2026-09-16 · LOCATIONS ERA — L2 AND L4 RECORDS (batched after verification)

L2 — LANDED 2026-09-15/16 (console), nine landings, every one verified by fresh clone and the operator's published-URL walk. L2a (Tree + Transfer tabs, LT-1..7, DEC-062 reaper delta): walk-found DRIFT — the roster rendered six verbs per row and clipped; supervisor slip INC-204. L2a-R (the categories convention restored: one edit icon per row, the editor's verb bar, TipBadge lifted to a shared primitive, one read per country; LT-8..11): red on format check — executor slip INC-205. R-CI (c006518b): the format red fixed; DEC-066 — the CI Status Reporter lands every failed build/guard job's evidence in docs/tracking/guards-last-failure.md on every run (extract-only, self-report, PLATFORM-ORIGIN label); the §8 "operator log paste" fallback is retired. L2a-R2 (4b2c8273; run 35034575039): no tabs — one roster whose DataTable toolbar carries find and transfer groups scoped by the toolbar's country; the shared import dialog gains an optional title. L2b-M (d26e6036): admin_list_locations with NULL = every country; countries_anchor_on_insert (a country row is born with an inactive anchor; backfilled); admin_list_countries. L2b-C1 (run 35040094989; seed 405bc914 mark 20260916010000): every ISO 3166-1 country seeded closed with its anchor; "All countries" default; the Locations rail group with the Countries section (open/close with step-up and forced close, profile, rail order, the countries file in its own toolbar); CO-1..8, LT-13; DEC-062 extended to scratch markets (ISO user-assigned codes only). L2b-C2 (186a9cb4 mark 20260916020000; run 35045770286): the Coverage section (CV-1..6); INC-206 fixed. L2c (e43f454e; run 35101545574 red on TR-12 → INC-207; re-run green): "Add a country" removed; the Locations group owns a clickable overview at /admin/locations and Places moves to /admin/places (breadcrumb Admin › Locations › Places); anchors never in the Places roster (a market opens only from Countries); open/closed and active/retired explained on hover and in a legend; app-rail lets a group with a path render its header as a link. L2d (d7eb0f2e; run 35110949027 red on CV-4 → R-CV): toolbars in the categories shape on all three consoles; renamed strings on NEW keys (admin.section.places.*, admin.locations.import.openPlaces); INC-207 fixed. R-CV + L4a (46813da6): CV-3/CV-4 on scratch plans (J3). Rulings recorded (spec §5 amended): every admin table is the DataTable primitive with its own toolbar and no tabs; Countries and Coverage are rail sub-sections under a clickable Locations overview; countries writes are RPC-only and countries:update is split from countries:activate; the coverage resource (view/update) gates plans; anchors are never listed among places; the "Add a country" button does not exist (the ISO seed is the list). Spec law 7 amended: countries are ISO 3166-1 reference data seeded by migration (L2b-S); places land by file. C3-UX-2 closeout amended: a rail group MAY own a page, and only then is its header a link. New-key law (Knowledge v3.9 proposal, D5): a UI string whose meaning changes gets a NEW key — an approved database row for the old key shadows the compiled file forever. Runtime note: the Places roster lists only rows below the anchor; the country picker opens on "All countries".

L4 — LANDED 2026-09-16. L4a (46813da6): GET /api/geo under the DEC-063 judge; the operator's published-URL read returned source "cf-header" (country US) — VERDICT: the Cloudflare cf object is not reachable through the Start/nitro wrapper (executor census: no request-event export), the edge's cf-ipcountry header is. L4b (28a22579; run 35130344823): the shell picker reads the open markets (GET /api/locations, cached, content-hash ETag) and the chosen country's tree from the L1c route (no table read; name_en only; a sub-city is a fourth step); the root's SSR context carries the saved-area cookie and the geo answer with no database call; a pick saves ethio_area = "<CC>:<node id>" (one year, SameSite=Lax) and clearing removes it; the guess is never written; LS-1..5. L4b-2 (8f63e1a3; run 35135712639): DEC-063 AMENDED — a fourth source cf-visitor (cf-iplatitude/longitude, cf-ipcity, cf-region-code) ranks above the country header; malformed coordinates discredit the visitor set (falls to cf-header), absent coordinates leave a valid city or region code standing; the shell resolves the guess over the cached tree — nearest curated city or sub-city within 60 km (a sub-city only within 8 km), else the region by ISO 3166-2 code, else a city by slug (aliases are not in the public payload by design — weight law; a named seam), else the country anchor — and names it in the caption; GE-4/5, LS-6..10. Production answers country-level until the ethio.com cutover: the operator's Cloudflare zone (ethio.com) serves the OLD site today with the apex and www DNS-only; the new app is served from Lovable's URL behind Lovable's own edge; "Add visitor location headers" is already ON in the operator's zone (2026-09-16) and takes effect when ethio.com is cut over to this app proxied through that zone — a launch-gate item.

Amendment proposals for the operator to install: Claude instructions v1.11 — §8: a request for a log paste is a supervisor slip; the reporter (DEC-066) lands every failed job's evidence in docs/tracking/*-last-failure.md; the supervisor reads ci-status.md (two-step), e2e-last-failure.md and guards-last-failure.md before any ruling, and reads the run page itself through the actions index when a file lags; §12 G27: a test never mutates a real reference row, in any project (J3 restated as a supervisor gate). Lovable Knowledge v3.9 — D5 (the new-key law above); C8: every admin table is the DataTable primitive with its own toolbar; no tabs; transfer controls live in the toolbar.

S39 · 2026-09-16 · U6 POSTING SPEC SESSION (ratified) + POSTING-PREREQUISITES CENSUS + MEDIA STORAGE

Ratified and imported: /docs/governance/u6-posting-spec.md (v2; sha256 094db2ef…), /docs/governance/posting-prerequisites-census.md (sha256 9ffe0e7a…), /docs/governance/media-storage-memo.md (sha256 0bca444b…). The census closes the question the 2026-09-11 queue correction asked: nothing precedes U6-A — the home-country and contact-channel capture live inside the first posting flow by law (REQ-006; identity-schema deferral); tags are U7 (REQ-041); brands are attribute options; the curator's two location files proceed in parallel and close the Locations era gate on import. Era order: G1 → A1 → A2 → B1 → B2 → C1 → C2 → D1 → D2 → E1 → E2 → gate. Forward-scan: REQ-023–041 read, none contradicted; two July locks amended by operator ruling (below).

Rulings 2026-09-16 (D1–D10): AI assistance for title/description from day one under the grounded-only law with a deterministic fake for tests (July manual-v1 lock amended); the map pin is a category capability (`map_pin`), lazy-loaded, and a seller's shop pin arrives with storefronts (REQ-008); a price period per category (once·hour·day·week·month·year) with ranges as attributes; screening on Gemini via the provisioned key with dials tuned toward automation, restricted categories always held, strikes, auto-removal, retroactive sweeps and appeals; residency = three facts, one rule (observed country at the first contribution, immutable; declared home country as preference; each post's partition key = the observed country at that post; mismatch a review signal; the rule in `residency_country_for`); contact preference as a profile shape (messages always; phone/Telegram/WhatsApp optional, show/hide); photos stripped of hidden metadata on the server by keeping only image data, variants made on the phone, one optional YouTube link per listing never hosted; the expiry job is the background expiry/reminder job (pg_cron if exposed, else the nightly's scheduler); tags at U7; a duplicate-post soft warning.

DEC-067 (2026-09-16): Price period law — `listings.price_period`, `categories.default_price_period` + `price_period_locked`; ranges are attributes; free/contact carry no amount.

DEC-068 (2026-09-16): Residency three-facts law — `user_directory.observed_country_code` (server-set at the first contribution, immutable), `home_country_code` (declared, editable, defaults only), every listing's partition key set by `residency_country_for(user, request)` to the observed country at that post; never from the client; observed ≠ declared is a review signal.

DEC-069 (2026-09-16): Photo strip law — the server strip route keeps only image data (JPEG APPn/COM/XMP, WebP EXIF/XMP/ICC, PNG text chunks dropped), verifies the real format by magic bytes, is the only writer of `exif_stripped = true`, and is proven by captured fixtures with GPS/XMP/ICC before any upload ships (REQ-036 acceptance); the phone makes the three variants.

DEC-070 (2026-09-16): Three-tier screening — draft → screening → live | reduced | rejected (appeal) | held; one gateway route (structure → sanitise → files → content-policy model); thresholds are dials tuned toward automation; restricted categories held; strikes on `standing` shorten allowances and suspend at a dial; a re-screen sweep applies rule/model updates retroactively; every verdict audited with model, version and rationale; a CI guard proves no write path bypasses the gateway.

DEC-071 (2026-09-16): Rate-limit primitive — `rate_limits` + `consume_rate_limit(action, key, limit, window)`; applied to post/draft, edit, upload, report, message, resend; refusals name the window; never client-only.

DEC-072 (2026-09-16): AI assist from day one — `/api/listings/assist` drafts title/description ONLY from the seller's facts (category, attributes, declared photo facts); never quality claims; seller accepts, edits or writes; deterministic fake mode in E2E.

DEC-073 (2026-09-16): Video link law — one optional YouTube URL per listing, validated to the YouTube shape, stored as a URL, never hosted or embedded in the feed; thumbnail on the card, player on tap through the no-cookie domain; screened like text.

DEC-074 (2026-09-16): Automated enforcement — human involvement is the exception; confident violations auto-reject or auto-remove; strikes, suspension at a dial, retroactive sweeps, user reports feeding the same queue, appeals on every automatic action.

DEC-075 (2026-09-16): Media storage — one storage adapter routes every media write and URL by partition (keys `<partition>/<user_id>/<listing_id>/<photo_id>/<variant>`; the database stores partition + key, never a provider URL; the strip route is the only writer); rest-of-world on Cloudflare R2 through its S3 API (zero egress); Supabase Storage stays for admin assets; the Ethiopia partition (in-country object storage beside an in-country database — Ethio Telecom Cloud if its S3 API checks out, else MinIO on Wingu/Raxio colocation, served through Cloudflare's Addis edge) is DESIGNED now and stood up on the licensing trigger. OPERATOR RULING: one physical partition at launch, images included, for as long as the platform operates without an Ethiopian licence (ESTIMATE 1–3 years); the split must then be a configuration and a copy by predicate, never a redesign. Counsel questions added to Q-014: photos as personal data; edge caching as transfer; ECA registration; the proclamation's reach over a controller without an Ethiopian establishment.

REQ-033 NOTE (2026-09-16): the launch shape is amended — ONE physical partition at launch; the Ethiopia partition stands up on the licensing trigger (a licensing-trigger checklist replaces the launch-gate partition item); the only partition item left on the launch gate is a staging rehearsal of the copy-by-predicate split proving it is configuration plus a copy.

REQ-018 NOTE (2026-09-16): price period per category (DEC-067). REQ-019 NOTE: one optional YouTube link (DEC-073); variants on the phone. REQ-020 NOTE: AI assist from day one, grounded-only (DEC-072); the home country and contact preference captured inside the first posting. REQ-021 NOTE: the gateway is DEC-070's route; REQ-022 NOTE: the screening states join the state machine; REQ-036 NOTE: DEC-069 is the implementation and its fixtures the acceptance; REQ-038 NOTE: `search_tsv` from day one; DEC-071 is the rate-limit primitive.

---

S40 · 2026-09-17 · LOCATIONS ERA CLOSED · U6 SPEC v3 · MODERATION DESIGN · RECORDS S39→S40

LOCATIONS ERA CLOSED 2026-09-17 (four-lens review: /docs/governance/reviews/locations-era-closeout-2026-09-17.md; sha256 97e0bf8b…). L3 landed through the console in five curator cycles (Ethiopia deep 216 rows; diaspora; cycle-2 deltas; worldwide 107 + Orlando; the combined Ethiopian+Eritrean review 57) plus countries files opening markets: 18 open at close, Switzerland and Norway opened at cycle 4; retired metro-folded rows deleted; every ISO country carries currency and unit profile. Product rulings 2026-09-16: the community the tree serves is Ethiopians AND Eritreans and every curation dial counts both (REQ-005 note); every country with a significant presence is seeded with its concentration cities (community evidence acceptable, marked by class); the operator opened Saudi Arabia, Sudan and South Africa with the curator's cautions kept on record; Eritrea's tree is seeded closed pending the operator's decision. Landings after S39: L3-FIX (INC-208 plain negatives in decimal import columns; INC-209 the country anchor exempt from parentInactive — a market's tree is prepared while closed; 7fcdddce mark 20260916230000), U6-A1 (two migrations fd11c7ab/7145d6e9, marks 20260917000000/20260917010000: the posting read, the validation authority, capabilities, price period, search column, residency facts, revisions, verdicts, rate limits; INC-210 rider), A1-R (the two permission keys — the D2 law proved by the i18n guard; INC-211 picker stale tree; INC-214 TR-34 prune by identity; L4b-3 auto-select), R-LT13 (INC-216: leaked open scratch markets crowding page 1; loud destroy; afterEach cleanup; search-based assertions). DEC-063 amended (visitor-location source; production country-level until cutover). REQ-033 amended (one partition at launch). DEC-075 media storage ratified.

U6 SPEC v3 (D11–D18, operator-approved 2026-09-17, appended as §12 of /docs/governance/u6-posting-spec.md; resulting sha256 b61e06aa…): D11 leaf-only posting with search-to-leaf; D12 step-level validation on autosave; D13 currency law (post in any currency; viewers see as posted; indicative daily-rate conversion); D14 stale drafts (30 days, reminder at 25, 14 days when never past step 1); D15 two-pass photo screening (policy at upload, coherence at publish); D16 the moderation system as designed in /docs/governance/moderation-design.md (sha256 dde0427c…): one judge one call, three bands, a second AI opinion before any human, time-boxed holds (24 h, a dial; an admin notification on a disagreement), trust tiers T0–T3 moving thresholds, the automated action ladder, automated first-round appeals, weighted reports, sweeps, a weekly 50-item calibration sample as the only routine human task, one admin-editable dials table, the DSA Art. 16/17/20/23 shape by design with Art. 30 trader traceability as a seam pending counsel; D17 minimal first-posting identity (alias, seller type person|business with business name, one contact channel, home country); D18 option-carried facts prefilling sibling attributes (extends DEC-050). Q-014 gains: DSA applicability to a classifieds site with off-platform contracts and to a micro-enterprise; Art. 30 trader traceability.

Instructions/Knowledge amendment proposals (operator installs): Knowledge v3.10 — E1 gains "every new table's migration REVOKEs the birth grants from anon/authenticated before its own GRANTs (Supabase pg_default_acl; INC-212)"; J-audit gains "cleanup survives a timeout (afterEach/hook, never only a finally in the body; a destroy that cannot verify throws)" (INC-216). Instructions v1.12 — §12 G28: a page-position assertion on a real row is never valid on a shared roster (search or DB truth) — the CV-4/INC-214/INC-216 class at three occurrences.

---

DEC-076 (2026-09-23): The sign-in door and the expired prior session — a deliberate sign-in SUPERSEDES an expired session: the expired session is evicted (local sign-out + its INC-223 clocks) BEFORE the credentials are sent, so no stale refresh remains to race; a live session is never touched, because a failed sign-in must not sign anybody out; and the door reports success only once the new session is committed to storage and readable, a grant that never commits being an honest failure with one `[ssr-error]` line rather than a silent redirect to a signed-out marketplace (INC-266).

---

S41 · 2026-09-18 → 2026-09-25 · U6 BUILD (A2 → C2 + R-fixes) · CATALOG DEPENDENCY PASSES (cycles 1–18) · FINDER D37-1 · HARNESS DEC-077/078 · RECORDS S40 → S42 (batched after verification; every landing verified by fresh clone against docs/_changelog.md)

U6 LANDED (per the changelog, 2026-09-17 → 2026-09-25): A1 (09-16), A2-M and A2-C (the draft door re-declared whole; the routes), B1 (photo pipeline server side) and B2 (on-device pipeline, inside C1a), C1a/C1b (wizard steps 1–4), C2a/C2b (steps 5–8; publish to screening with the D1 gateway hook deferred), the R-series correctives U6-C1-R1 → R3b-4 (map pin), LAYOUT-1 primitives, the import cells M-MAINT-2 (allowed_options/default_value, plan photo cap D22), M-MAINT-3/3b (visible_when, D24), M-FACTS, M-ORDER (display_order), M-SHAPE (+ corrective), M-SWATCH (D28), the wizard rulings D24–D28, D30/D33 reader half, D34, D35, D36 (superseded by D41), D37-1 finder (+ E1 corrective + DEC-022 closer + INC-273), D38/D40, D39, D41, and DEC-076 (sign-in). NOT landed: D1 screening gateway (publish still defers to it), D2 admin review, E1 My Listings, E2 expiry job, the D30/D33 file-cell half, D31 (spec v1 for approval 2026-09-25), D37-2 (wizard picker) and D37-3 (marketplace bar, after U7), D42 (backlog). The era gate is open.

CATALOG: dependency/completeness passes ran through the curation Project — cycle 1 Vehicles (2026-09-20) … cycle 17 — over all 17 roots under the per-leaf fact template (≥ 95 % coverage per leaf, coverage tables in each review note); cycle 18 (the §10 seller-view re-check of the twelve pre-§10 roots) batch 1 (Services, Home & Garden, Clothing & Shoes) imported 2026-09-24 with seven follow-up files, batch 2 (Babies & Kids, Beauty & Personal Care, Agriculture & Farming) plus a four-file addenda set imported 2026-09-25 (16 files, every preview as expected; Babies file 1 corrected by dropping two stale echo rows — INC-279 class); batch 3 (Commercial Equipment, Sports & Leisure, Pets & Animals) chartered; batch 4 (Travel, Food) follows. Standing curator rules coined this era: R17 popularity order, Other last; R21 single-value locks; R22 family adjacency (fridge/freezer, washer/dryer) weighed with popularity; R23 brand right after identity, then the item's own facts, power, seller-side; R24 per-identity brand narrowing through `allowed` where makers differ by type (HG-17 landed); the unfiltered-export check on every new key and restated shared row (INC-279). Product rulings applied: D41 every row open (no "More details"); D42 service-area km removed from Services (a map radius is a later wizard idea); D43 the Fashion root reads "Clothing & Shoes" / ልብስና ጫማ (slug kept; "fashion" becomes a finder synonym at D37-2).

HARNESS: DEC-062 reaper family extended (INC-274 audit prune, INC-276 category reaper, DEC-077 photo reaper); DEC-078 flaky bodies; the flake ledger read whole on 2026-09-25 (217 lines since 09-19) → INC-282 → INC-287 registered, INC-218 step 3, INC-280/281 confirmed fixed by the ledger going quiet after their commits; the DEC-078 rule is on watch (three flaky runs with bodies, report < 200 KB).

DEC-077 (2026-09-24, ratified by the operator's send of the harness-hygiene prompt; landed 2026-09-25 in two parts, 11f94620 and f74cb7a4): PHOTO STORAGE REAPER (a G22 harness change with its rule pre-committed). The DEC-062 listing reaper removes every storage object under `default/<seller_id>/<listing_id>/` for each scratch listing it reaps, in the same setup pass, and re-lists the prefix afterwards (non-empty ⇒ throw). The backlog is enumerated STORAGE-SIDE (`list("default")`, offset-paged) and classified against the full auth census: an orphan-user folder is purged whole; a live e2e seller loses only folders whose listing is gone; a live non-e2e user is never touched; bound 5,000 objects per run with "more remain"; every shard-1 setup uploads and purges a 1-byte proof object and throws unless exactly 1 was removed and the re-list is empty, so a green CI run is itself the proof the delete path ran. Supabase provider only (`MEDIA_PROVIDER` unset ⇒ supabase, `src/server/media/storage.ts:50`). Evidence: two staging setup runs (3 orphan objects under 1 orphan-user folder removed; second run 0), run 36101409324 green. Closes INC-278. Supervisor slip logged: part 1 was designed to walk live e2e sellers although the orphans belong to reaped users whose listing rows cascade away (`20260804174739…sql:14`), and its delete path was never exercised — corrected by part 2 (G3-addendum class: a design stated as a fact before the decisive line was read).

DEC-078 (2026-09-25, landed in two parts, 61f08491 and ca322796): FLAKY-ATTEMPT EVIDENCE. For every FLAKY test (failed, then passed on retry) the merged report carries the first failed attempt's body in the failure shape — the 40-line message and the matched error-context tail, or the "context file not found" line — under "## Flaky bodies (DEC-078)" after the Flake ledger, at most 10 per run; the four "Upload error contexts" steps run on every outcome (`if-no-files-found: ignore`); part 2: sources and the context index are loaded above the green branch so a GREEN run carries the same sections and its verdict writes the real `flaky=<n>`; one shared `flakeLedgerSection`. Fixture-proven (the captured results with the first test flipped to flaky; a replayed 11-flip for the cap). Rule (pre-committed): kept if the next three runs that carry a flake show one body per FLAKY line and the report stays under 200 KB; else git revert. Reason: 217 retried-then-passed lines in seven days and nine tests over DEC-030's threshold with no body to root-cause from (G21).

RULINGS INDEX D19–D43 (operator rulings applied 2026-09-18 → 2026-09-25; the U6 spec gains §13 with this index): see docs/governance/u6-posting-spec.md §13.

INSTRUCTIONS/KNOWLEDGE AMENDMENT PROPOSALS (operator installs): Claude instructions v1.13 — §8 gains "every verification reads the flake ledger whole: a test with ≥ 3 matrix-lane lines in 7 days and no INC is a finding, dispositioned in the same turn" (S41 LY-6 miss, S42 the unread ledger 09-19 → 09-25; second occurrence). Knowledge v3.11 — J-laws gain "one local E2E suite at a time (every local run maps to guess slot 9 — INC-285)" and "a test locates a shared-roster row by identity, never by position (G28; INC-283/284)"; E-laws gain the DEC-079 price-basis convention once D31 is ratified ("a pricing-basis attribute is keyed `pricing_type*` / `unit_of_sale-*`, is a card, is the only such attribute at a leaf, and its values are drawn from the DEC-079 shape table").

SUPERVISOR SLIPS (S41/S42, logged with the executor's honesty): S41 — the LY-6 DEC-030 threshold missed on 09-21 (INC-280 registered 09-25); S42 — the flake ledger not read whole between 09-19 and 09-25; the DEC-077 part-1 design walked live sellers (above); the shallow-seed miscount and the LT-13 page-1 assumption (S39/S40, already in instructions v1.12); the INC-279 delivery_available overwrite not caught before import (capability gap: no unfiltered export was attached then — closed by the class rule). Caught before damage, not slips: the Babies echo rows (2026-09-25), the addenda order-of-lists note (allowed lists show in definition order — `visibleOptionsOf`).

NUMBERING: next free INC-288; DEC-079 reserved for D31 (price-basis law) pending approval; next free DEC after it 080.
