...
export function LocationsOverview() {
  ...
  const marketStats = useMemo(() => {
    ...
    return { open: rows.filter((row) => row.isActive).length, closed: rows.filter((row) => !row.isActive).length };
  }, [countries.data]);
  const placeStats = useMemo(() => {
    ...
    return {
      ...
      total: rows.length,
      levels: levels.map((level) => ({ level, count: rows.filter((row) => row.level === level).length })),
    };
  }, [places.data]);
  ...
  return (
    <div data-testid="admin-overview-locations" className="min-w-0 space-y-6">
      <div>
        <h1 className="text-lg font-semibold text-foreground">{t("admin.overview.locations.title")}</h1>
        <p className="mt-1 text-sm text-muted-foreground">{t("admin.overview.locations.body")}</p>
      </div>

      {loading ? <p className="text-sm text-muted-foreground">{t("admin.overview.loading")}</p> : null}
      {error ? (
        <p role="alert" className="text-sm text-destructive">{t("admin.overview.error")}</p>
      ) : null}

      <section className="grid grid-cols-1 gap-3 sm:grid-cols-3" aria-label={t("admin.overview.stats") }>
        <article className="rounded-md border border-border bg-card p-4" data-testid="overview-stat-markets">
          ...
          <p className="mt-1 text-xs text-muted-foreground">{t("admin.overview.markets.caption")}</p>
        </article>
        <article className="rounded-md border border-border bg-card p-4" data-testid="overview-stat-places">
          ...
          <p className="mt-2 text-xs tabular-nums text-muted-foreground">
            {placeStats.levels.map(({ level, count }) => `${t(`admin.locations.level.${level}`)} ${count}`).join(" · ")}
          </p>
        </article>
        <article className="rounded-md border border-border bg-card p-4" data-testid="overview-stat-plans">
