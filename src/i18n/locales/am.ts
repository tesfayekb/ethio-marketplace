import type { Messages } from "../types";

export const am: Messages = {
  "app.name": "ethio.com",
  "app.tagline": "ይግዙ እና ይሽጡ",
  "language.label": "ቋንቋ",
  "language.english": "እንግሊዝኛ",
  "language.amharic": "አማርኛ",
  "language.enShort": "EN",
  "language.amShort": "አማ",
  "language.star": "በዚህ መሣሪያ ላይ እንደ ነባሪ ቋንቋ አድርግ",
  "language.starred": "በዚህ መሣሪያ ላይ ነባሪ ቋንቋ",
  "nav.home": "መነሻ",
  "common.loading": "በመጫን ላይ…",
  "common.error": "የሆነ ስህተት ተከስቷል",
  "common.retry": "እንደገና ይሞክሩ",
  "common.empty": "እስካሁን ምንም የለም",
  "common.cancel": "ተወው",
  "common.close": "ዝጋ",
  "common.goHome": "ወደ መነሻ ይሂዱ",
  "common.tryAgain": "እንደገና ይሞክሩ",
  "error.pageNotFound": "ገጹ አልተገኘም",
  "error.pageNotFoundBody": "የሚፈልጉት ገጽ የለም ወይም ተዛውሯል።",
  "error.pageFailed": "ይህ ገጽ አልተጫነም",
  "error.pageFailedBody": "በእኛ በኩል የሆነ ስህተት ተከስቷል። እንደገና ማደስ ወይም ወደ መነሻ መመለስ ይችላሉ።",
  "auth.signIn": "ይግቡ",
  "auth.createAccount": "አካውንት ይክፈቱ",
  "auth.email": "ኢሜይል",
  "auth.password": "የይለፍ ቃል",
  "auth.emailPlaceholder": "you@example.com",
  "auth.passwordPlaceholder": "ቢያንስ 8 ቁምፊዎች",
  "auth.showPassword": "የይለፍ ቃሉን አሳይ",
  "auth.hidePassword": "የይለፍ ቃሉን ደብቅ",
  "auth.signInButton": "ይግቡ",
  "auth.signUpButton": "አካውንት ይክፈቱ",
  "auth.toggleToSignUp": "አዲስ ነዎት? አካውንት ይክፈቱ",
  "auth.toggleToSignIn": "አካውንት አለዎት? ይግቡ",
  "auth.checkEmail": "ኢሜይልዎን ይመልከቱ",
  "auth.checkEmailBody": "የማረጋገጫ ሊንክ ልከንልዎታል። አካውንትዎን ለመጨረስ ይክፈቱት።",
  "auth.checkEmailSentTo": "ወደ {email} ሊንክ ልከንልዎታል። አካውንትዎን ለመጨረስ ይክፈቱት።",
  "auth.resend": "የማረጋገጫ ኢሜይል እንደገና ላክ",
  "auth.resendSent": "የማረጋገጫ ኢሜይል ተልኳል።",
  "auth.resendNeutral": "ኢሜይልዎ ገና ካልተረጋገጠ፣ አዲስ ሊንክ በመንገድ ላይ ነው።",
  "auth.resendCooldown": "እንደገና ለመላክ በ{s} ሰከንድ ውስጥ ይቻላል",
  "auth.resendLimitReached": "የመላክ ገደብ ደርሷል — ጥቂት ደቂቃዎች ይጠብቁ። ኢሜይልዎ ተረጋግጧል? ይግቡ።",
  "auth.confirmedInline": "✓ ኢሜይልዎ ተረጋግጧል",
  "auth.alreadyConfirmedSignIn": "አስቀድሞ ተረጋግጧል? ይግቡ",
  "auth.confirmedTitle": "ኢሜይል ተረጋግጧል",
  "auth.confirmedBody": "ገብተዋል፤ መጀመር ይችላሉ።",
  "auth.continue": "ይቀጥሉ",
  "auth.linkInvalid": "ይህ ሊንክ ልክ ያልሆነ ወይም ጊዜው ያለፈበት ነው",
  "auth.linkInvalidBody": "አዲስ የማረጋገጫ ኢሜይል ይጠይቁና እንደገና ይሞክሩ።",
  "auth.backToSignIn": "ወደ መግቢያ ተመለስ",
  "auth.noSessionTitle": "ጥቂት ቀርቷል",
  "auth.noSessionBody": "ሊንኩን በሌላ መሣሪያ ወይም አሳሽ ከከፈቱት፣ አካውንትዎ እዚያ ተረጋግጧል። ለመቀጠል እዚህ ይግቡ።",
  "auth.signOut": "ይውጡ",
  "auth.signedInAs": "ገብተዋል",
  "auth.checking": "ክፍለ ጊዜዎን በመፈተሽ ላይ…",
  "auth.working": "እባክዎ ይጠብቁ…",
  "auth.orContinueWith": "ወይም በሚከተለው ይቀጥሉ",
  "auth.orSeparator": "ወይም በሚከተለው ይቀጥሉ",
  "auth.continueWithGoogle": "በGoogle ይቀጥሉ",
  "auth.linkRefusedTitle": "መጀመሪያ ይህን አድራሻ ያረጋግጡ",
  "auth.linkRefusedBody":
    "ይህ አድራሻ ከGoogle ጋር ከመያያዙ በፊት በኢሜይል መረጋገጥ አለበት። በኢሜይል ቅጹ ይቀጥሉና የምንልክልዎን የማረጋገጫ ሊንክ ይክፈቱ።",
  "auth.telegramSlot": "በTelegram ይቀጥሉ (በቅርቡ)",
  "auth.errorInvalidCredentials": "ኢሜይሉና የይለፍ ቃሉ አይዛመዱም።",
  "auth.errorEmailNotConfirmed": "ከመግባትዎ በፊት ኢሜይልዎን ያረጋግጡ።",
  "auth.errorEmailInUse": "በዚህ ኢሜይል የተከፈተ አካውንት አለ።",
  "auth.errorWeakPassword": "ረዘም ያለና ጠንካራ የይለፍ ቃል ይምረጡ።",
  "auth.errorInvalidEmail": "ትክክለኛ ኢሜይል አድራሻ ያስገቡ።",
  "auth.errorRateLimited": "ብዙ ሙከራዎች ተደርገዋል። ትንሽ ቆይተው እንደገና ይሞክሩ።",
  "auth.errorMissingFields": "ኢሜይልዎንና የይለፍ ቃልዎን ያስገቡ።",
  "auth.errorGeneric": "የሆነ ስህተት ተከስቷል። እባክዎ እንደገና ይሞክሩ።",
  "settings.navLabel": "ቅንብሮች",
  "settings.title": "ቅንብሮች",
  "settings.identity": "የእርስዎ አካውንት",
  "settings.displayName": "ስም",
  "settings.email": "ኢሜይል",
  "settings.deviceLanguage": "ነባሪ ቋንቋ (ይህ መሣሪያ)",
  "settings.deviceLanguageNone": "አልተዘጋጀም",
  "settings.memberSince": "አባል የሆኑበት ቀን",
  "settings.methods": "የመግቢያ መንገዶች",
  "settings.methodsError": "የመግቢያ መንገዶችዎን መጫን አልተቻለም።",
  "settings.providerEmail": "ኢሜይልና የይለፍ ቃል",
  "settings.providerGoogle": "ጉግል",
  "settings.lastUsed": "መጨረሻ የተጠቀሙበት {when}",
  "settings.lastUsedNever": "እስካሁን አልተጠቀሙበትም",
  "settings.linkGoogle": "ጉግልን ያገናኙ",
  "settings.unlink": "ያላቅቁ",
  "settings.unlinked": "የመግቢያ መንገዱ ተወግዷል።",
  "settings.lastMethodGuard": "ይህ ብቸኛው የመግቢያ መንገድዎ ነው። ከማስወገድዎ በፊት ሌላ ይጨምሩ።",
  "settings.security": "ደህንነት",
  "settings.changePassword": "የይለፍ ቃል ይቀይሩ",
  "settings.currentPassword": "አሁን ያለው የይለፍ ቃል",
  "settings.newPassword": "አዲስ የይለፍ ቃል",
  "settings.passwordChanged": "የይለፍ ቃልዎ ተቀይሯል።",
  "settings.changeEmail": "ኢሜይል ይቀይሩ",
  "settings.newEmail": "አዲስ የኢሜይል አድራሻ",
  "settings.emailChangeSent":
    "የማረጋገጫ አገናኞች ወደ አሁኑም ወደ አዲሱም አድራሻዎ ተልከዋል። ሁለቱንም ሲከፍቱ ብቻ ኢሜይልዎ ይቀየራል።",
  "settings.signOutOthers": "ከሌሎች መሣሪያዎች ይውጡ",
  "settings.signOutOthersConfirm": "ከዚህ መሣሪያ በስተቀር ከሁሉም ቦታ ይውጡ?",
  "settings.signOutOthersConfirmYes": "አዎ፣ ያስወጣቸው",
  "settings.cancel": "ይቅር",
  "settings.signOutOthersDone": "ሌሎች መሣሪያዎች ወጥተዋል።",
  "settings.save": "አስቀምጥ",
  "account.overview.title": "አካውንት",
  "account.overview.loadError": "የአካውንትዎን አጠቃላይ እይታ መጫን አልተቻለም።",
  "account.overview.profile": "መገለጫ",
  "account.overview.alias": "የሻጭ መጠሪያ",
  "account.overview.sellerType": "የሻጭ ዓይነት",
  "account.overview.business": "የንግድ ስም",
  "account.overview.memberSince": "አባል የሆኑበት ቀን",
  "account.overview.homeCountry": "የመኖሪያ አገር",
  "account.overview.notSet": "አልተዘጋጀም",
  "account.overview.editProfile": "መገለጫ አርትዕ",
  "account.overview.listings": "የእኔ ዝርዝሮች በአጭሩ",
  "account.overview.noListings": "እስካሁን ዝርዝር የለም",
  "account.overview.saved": "የተቀመጡ",
  "account.overview.nothingSaved": "እስካሁን የተቀመጠ የለም",
  "account.overview.notifications": "ማሳወቂያዎች",
  "account.overview.noNotifications": "እስካሁን ማሳወቂያ የለም",
  "account.overview.quickActions": "ፈጣን እርምጃዎች",
  "account.overview.status.draft": "ረቂቅ",
  "account.overview.status.screening": "በግምገማ ላይ",
  "account.overview.status.active": "ንቁ",
  "account.overview.status.paused": "ለጊዜው የቆመ",
  "account.overview.status.sold": "የተሸጠ",
  "account.overview.status.expired": "ጊዜው ያለፈ",
  "account.overview.status.rejected": "ማስተካከያ የሚፈልግ",
  "auth.errorWrongCurrentPassword": "አሁን ያለው የይለፍ ቃል ትክክል አይደለም።",
  "auth.errorLastMethod": "ብቸኛውን የመግቢያ መንገድዎን ማስወገድ አይችሉም። መጀመሪያ ሌላ ይጨምሩ።",
  "auth.errorPasswordTooShort": "ቢያንስ 8 ቁምፊዎችን ይጠቀሙ።",
  "auth.forgotPassword": "የይለፍ ቃልዎን ረሱት?",
  "auth.resetTitle": "የይለፍ ቃልዎን ዳግም ያስጀምሩ",
  "auth.resetBody": "በመለያዎ ላይ ያለውን ኢሜይል ያስገቡ፤ የዳግም ማስጀመሪያ አገናኝ እንልክልዎታለን።",
  "auth.resetSend": "የዳግም ማስጀመሪያ አገናኝ ላክ",
  "auth.resetNeutral": "ለዚያ አድራሻ መለያ ካለ፣ የይለፍ ቃል ዳግም ማስጀመሪያ አገናኝ በመንገድ ላይ ነው።",
  "auth.resetLimit": "የጥያቄ ገደብ ደርሷል — እንደገና ከመሞከርዎ በፊት ጥቂት ደቂቃዎች ይጠብቁ።",
  "auth.resetNewTitle": "አዲስ የይለፍ ቃል ይምረጡ",
  "auth.resetNewBody": "ለመለያዎ አዲስ የይለፍ ቃል ያስገቡ።",
  "auth.resetSubmit": "አዲሱን የይለፍ ቃል አስቀምጥ",
  "auth.resetDone": "የይለፍ ቃልዎ ተዘምኗል። አሁን በእሱ መግባት ይችላሉ።",
  "auth.resetLinkInvalid": "ይህ የዳግም ማስጀመሪያ አገናኝ ልክ ያልሆነ ወይም ጊዜው ያለፈበት ነው",
  "auth.resetLinkInvalidBody": "አዲስ አገናኝ ይጠይቁና እንደገና ይሞክሩ።",
  "auth.errorResetLinkInvalid": "ይህ የዳግም ማስጀመሪያ አገናኝ ልክ ያልሆነ ወይም ጊዜው ያለፈበት ነው። አዲስ ይጠይቁ።",
  "settings.passwordMethod": "የይለፍ ቃል",
  "settings.passwordMethodPresent": "በይለፍ ቃል መግባት ይችላሉ",
  "settings.passwordMethodAbsent": "የይለፍ ቃል አልተዘጋጀም",
  "settings.setPasswordHint": "ለማዘጋጀት፣ በመግቢያ ገጹ ላይ \u201cየይለፍ ቃልዎን ረሱት?\u201d ይጠቀሙ።",
  "settings.removePassword": "የይለፍ ቃል አስወግድ",
  "settings.removePasswordConfirm": "የይለፍ ቃልዎን ያስወግዱ? ከዚያ በኋላ በሌሎች ዘዴዎችዎ ብቻ ይገባሉ።",
  "settings.removePasswordConfirmYes": "አዎ፣ የይለፍ ቃሌን አስወግድ",
  "settings.passwordRemoved": "የይለፍ ቃልዎ ተወግዷል።",
  "settings.passwordStateError": "የይለፍ ቃል እንዳለዎት ማረጋገጥ አልቻልንም።",
  "panel.marketplace": "ገበያ",
  "panel.myListings": "የእኔ ማስታወቂያዎች",
  "panel.account": "አካውንት",
  "panel.admin": "አስተዳደር",
  "navSection.menu": "ዝርዝር",
  "navSection.identity": "ማንነት",
  "navSection.accessControl": "የመዳረሻ ቁጥጥር",
  "navSection.moderation": "ቁጥጥር እና ግምገማ",
  "navSection.marketplace": "ገበያ",
  "navSection.content": "ይዘት",
  "navSection.support": "ድጋፍ",
  "navSection.system": "ሲስተም",
  "navSection.manage": "አስተዳደር",
  "nav.dashboard": "ዳሽቦርድ",
  "nav.postListing": "ማስታወቂያ ይለጥፉ",
  "nav.myListings": "የእኔ ማስታወቂያዎች",
  "nav.messages": "መልእክቶች",
  "nav.featured": "የተለዩ",
  "nav.overview": "አጠቃላይ እይታ",
  "nav.saved": "የተቀመጡ",
  "nav.activity": "የእኔ እንቅስቃሴ",
  "nav.notifications": "ማሳወቂያዎች",
  "nav.addresses": "አድራሻዎች",
  "nav.profile": "መገለጫ",
  "nav.signInSecurity": "መግቢያ እና ደህንነት",
  "nav.help": "እገዛ",
  "nav.users": "ተጠቃሚዎች",
  "nav.verification": "ማረጋገጫ",
  "nav.roles": "ሚናዎች",
  "nav.permissions": "ፈቃዶች",
  "nav.reports": "ሪፖርቶች",
  "nav.screeningQueue": "የምርመራ ወረፋ",
  "nav.allListings": "ሁሉም ማስታወቂያዎች",
  "nav.categories": "ምድቦች",
  "nav.locations": "አካባቢዎች",
  "nav.pages": "ገጾች",
  "nav.translations": "ትርጉሞች",
  "nav.supportTickets": "የድጋፍ ጥያቄዎች",
  "nav.organisation": "ድርጅት",
  "nav.systemSettings": "የሲስተም ቅንብሮች",
  "shell.skipToContent": "ወደ ይዘቱ ዝለል",
  "shell.openMenu": "ዝርዝሩን ክፈት",
  "shell.closeMenu": "ዝርዝሩን ዝጋ",
  "shell.menuTitle": "ዝርዝር",
  "shell.searchLabel": "ማስታወቂያዎችን ይፈልጉ",
  "shell.searchPlaceholder": "ማስታወቂያዎችን ይፈልጉ",
  "shell.accountMenu": "የአካውንት ዝርዝር",
  "shell.searchClose": "ፍለጋን ዝጋ",
  "shell.themeToggle": "ብሩህ ወይም ጨለማ ገጽታ ቀይር",
  "shell.breadcrumbLabel": "የመንገድ ዱካ",
  "shell.switchPanel": "ክፍል ቀይር",
  "shell.panelLabel": "ክፍል",
  "shell.mainNav": "ዋና አሰሳ",
  "shell.allCategories": "ሁሉም ምድቦች",
  "shell.categoriesLabel": "ምድቦች",
  "shell.categoriesEmpty": "እስካሁን ምድብ የለም",
  "shell.placeholderTitle": "በራሱ ባህሪ ይመጣል",
  "shell.placeholderBody": "ይህ ክፍል ገና አልተገነባም። በራሱ ባህሪ ይመጣል።",
  "location.label": "አካባቢ",
  "location.everywhere": "ሁሉም ቦታ",
  "location.anyArea": "ማንኛውም አካባቢ",
  "location.country": "አገር",
  "location.region": "ክልል",
  "location.city": "ከተማ",
  "location.subCity": "ክፍለ ከተማ",
  "location.empty": "አካባቢዎች የሉም",
  "location.readFailed": "አካባቢዎችን መጫን አልተቻለም — በቅርቡ እንደገና ይሞክሩ",
  "location.guessAreaCaption": "ከ{area} አቅራቢያ ያሉ ማስታወቂያዎች ይታያሉ — ይቀይሩ?",
  "location.stubNote": "አካባቢ መምረጥ እስካሁን ማስታወቂያዎችን አያጣራም።",
  "shell.searchOpen": "ፍለጋ ክፈት",
  "shell.collapseRail": "የጎን ዝርዝርን አጥብብ",
  "shell.expandRail": "የጎን ዝርዝርን አስፋ",

  "footer.sectionCompany": "ድርጅት",
  "footer.sectionHelp": "እገዛ",
  "footer.sectionLegal": "ሕጋዊ",
  "footer.about": "ስለ እኛ",
  "footer.howItWorks": "እንዴት እንደሚሰራ",
  "footer.safety": "ደህንነት",
  "footer.contact": "ያግኙን",
  "footer.terms": "ውሎች",
  "footer.privacy": "ግላዊነት",
  "footer.rights": "መብቱ በሙሉ የተጠበቀ ነው።",
  "feed.heading": "በ{location} አካባቢ ያሉ ማስታወቂያዎች",
  "feed.scopeAll": "እርስዎ",
  "feed.loading": "ማስታወቂያዎች በመጫን ላይ…",
  "feed.emptyTitle": "በአካባቢዎ እስካሁን ምንም የለም",
  "feed.emptyBody": "ማስታወቂያ በመለጠፍ የመጀመሪያው ይሁኑ።",
  "feed.errorTitle": "ማስታወቂያዎችን መጫን አልቻልንም",
  "feed.errorBody": "ግንኙነትዎን ይፈትሹና እንደገና ይሞክሩ።",
  "feed.views": "{count} እይታዎች",
  "feed.noPhoto": "እስካሁን ፎቶ የለም",
  "price.free": "ነጻ",
  "price.negotiable": "የሚደራደር",
  "price.contact": "ለዋጋ ያግኙ",
  "tier.premium": "ፕሪሚየም",
  "tier.featured": "የተለየ",

  "admin.title": "አስተዳደር",
  "admin.body": "የሠራተኞች መሣሪያዎች እያንዳንዱ የአስተዳደር ገጽታ ሲጠናቀቅ እዚህ ይታያሉ።",
  "admin.loading": "መዳረሻዎን በመፈተሽ ላይ…",

  // TODO-native-review: provisional Amharic, pending native speaker review.
  "admin.nav.label": "የአስተዳደር ክፍሎች",
  "admin.landing.title": "አስተዳደር",
  "admin.landing.body": "የሚያስተዳድሩትን ክፍል ይምረጡ።",
  "admin.accessDenied": "ወደዚያ ክፍል መዳረሻ የለዎትም።",
  "admin.noSections.title": "የሚገኝ የአስተዳደር ክፍል የለም",
  "admin.noSections.body": "የእርስዎ ሚና እስካሁን የአስተዳደር ክፍል የለውም። መዳረሻ ከፈለጉ አስተዳዳሪን ይጠይቁ።",
  "admin.comingSoon": "እስካሁን አልተሠራም",
  "admin.section.users.title": "ተጠቃሚዎች",
  "admin.section.users.body": "የተጠቃሚ አስተዳደር በU1 ይመጣል።",
  "admin.section.roles.title": "ሚናዎች እና ፈቃዶች",
  "admin.section.roles.body": "የሚና እና ፈቃድ አስተዳደር በU2 ይመጣል።",
  "admin.section.audit.title": "ኦዲት እና ደህንነት",
  "admin.section.audit.body": "የኦዲት እና ደህንነት መሣሪያዎች በU3 ይመጣሉ።",
  "admin.section.places.title": "ቦታዎች",
  "admin.section.places.body": "በየገበያው ውስጥ ያሉ ክልሎች፣ ከተሞች እና ክፍለ ከተሞች።",
  "admin.section.countries.title": "አገሮች",
  "admin.section.countries.body": "እያንዳንዱ ገበያ፦ ክፍት ወይም ዝግ፣ መለኪያ፣ ገንዘብ እና የሐዲዱ ቅደም ተከተል።",
  "admin.section.coverage.title": "ሽፋን",
  "admin.section.coverage.body": "በየእቅዱ ሻጭ እስከምን ድረስ መድረስ ይችላል።",
  "admin.nav.group.catalog": "ካታሎግ",
  "admin.nav.group.locations": "አካባቢዎች",
  "admin.overview.locations.title": "አካባቢዎች",
  "admin.overview.locations.body": "ገበያዎችን፣ ቦታዎችን እና የሻጭ ሽፋንን በአንድ እይታ።",
  "admin.overview.loading": "የአካባቢ ድምሮች እየተጫኑ ነው…",
  "admin.overview.error": "የአካባቢ አጠቃላይ እይታው መጫን አልተቻለም።",
  "admin.overview.stats": "የአካባቢ ድምሮች",
  "admin.overview.links": "የአካባቢ ክፍሎች",
  "admin.overview.markets": "ገበያዎች",
  "admin.overview.markets.caption": "ክፍት · ዝግ",
  "admin.overview.places": "ቦታዎች",
  "admin.overview.places.caption": "ንቁ · ጠቅላላ",
  "admin.overview.plans": "እቅዶች",
  "admin.overview.plans.caption": "የሽፋን እቅዶች",
  "admin.section.categories.title": "ምድቦች",
  "admin.section.categories.body": "የምድብ አስተዳደር በU5 ይመጣል።",
  "admin.section.attributes.title": "ባህርያት",
  "admin.section.attributes.body": "የጋራ የባህሪ ቤተ-መዝገብ፦ ትርጓሜዎች፣ ምድቦቻቸው እና በዝርዝር ካርድ ላይ የሚታየው።",
  "admin.section.images.title": "ምስሎች",
  "admin.section.images.body": "የምስል መሣሪያዎች በU8 ይመጣሉ።",
  "common.dismiss": "አሰናብት",
  "session.idleWarning": "እንቅስቃሴ ባለመኖሩ በ{s} ሰከንድ ውስጥ ይወጣሉ",
  "session.staySignedIn": "ገብቼ ልቆይ",
  "session.signedOutIdle": "እንቅስቃሴ ባለመኖሩ ወጥተዋል",
  "session.expired": "የክፍለ ጊዜው ጊዜ አልቋል — እባክዎ እንደገና ይግቡ",
  // U1 — Users section (admin).
  // TODO-native-review: provisional Amharic, pending native speaker review.
  "admin.users.title": "ተጠቃሚዎች",
  "admin.users.searchLabel": "ፍለጋ",
  "admin.users.searchPlaceholder": "ስም፣ ኢሜይል ወይም የሻጭ ስም",
  "admin.users.statusFilter": "ሁኔታ",
  "admin.users.roleFilter": "ሚና",
  "admin.users.filter.all": "ሁሉም",
  "admin.users.status.active": "ንቁ",
  "admin.users.status.deactivated": "የተቋረጠ",
  "admin.users.loading": "ተጠቃሚዎችን በመጫን ላይ…",
  "admin.users.error": "ተጠቃሚዎችን መጫን አልተቻለም።",
  "admin.users.empty": "በእነዚህ ማጣሪያዎች ተጠቃሚ አልተገኘም።",
  "admin.users.total": "{n} ተጠቃሚዎች",
  "admin.users.prev": "ቀዳሚ",
  "admin.users.next": "ቀጣይ",
  "admin.users.openDetail": "ተጠቃሚን ክፈት",
  "admin.users.col.name": "ስም",
  "admin.users.col.email": "ኢሜይል",
  "admin.users.col.country": "ሀገር",
  "admin.users.col.status": "ሁኔታ",
  "admin.users.col.roles": "ሚናዎች",
  "admin.users.col.joined": "የተቀላቀለበት",
  "admin.users.detail.notFound": "ይህ ተጠቃሚ የለም።",
  "admin.users.detail.identity": "ማንነት",
  "admin.users.detail.alias": "የሻጭ ስም",
  "admin.users.detail.joined": "የተቀላቀለበት",
  "admin.users.detail.lastSignIn": "የመጨረሻ መግቢያ",
  "admin.users.detail.never": "በጭራሽ",
  "admin.users.detail.back": "ወደ ተጠቃሚዎች ተመለስ",
  "admin.users.status.title": "የመለያ ሁኔታ",
  "admin.users.status.reasonLabel": "ምክንያት",
  "admin.users.status.reasonPlaceholder": "ይህ መለያ ለምን ይቋረጣል?",
  "admin.users.status.reasonRequired": "መለያን ለማቋረጥ ምክንያት ያስፈልጋል።",
  "admin.users.status.deactivate": "አቋርጥ",
  "admin.users.status.activate": "አንቃ",
  "admin.users.status.changedAt": "የተቀየረበት",
  "admin.users.status.failed": "ሁኔታውን መቀየር አልተቻለም።",
  "admin.users.status.ownAccount": "የራስዎን የመለያ ሁኔታ እዚህ መቀየር አይችሉም — መለያዎን በቅንብሮች ውስጥ ያስተዳድሩ።",
  "admin.users.roles.title": "ሚናዎች",
  "admin.users.roles.none": "እስካሁን ሚና የለም።",
  "admin.users.roles.assign": "ሚና መድብ",
  "admin.users.roles.assignAction": "መድብ",
  "admin.users.roles.remove": "አስወግድ",
  "admin.users.roles.locked": "የስርዓት ሚና",
  "admin.users.roles.failed": "የሚና ለውጡ አልተሳካም።",
  "admin.users.activity.title": "እንቅስቃሴ",
  "admin.users.activity.empty": "የተመዘገበ እንቅስቃሴ የለም።",
  "admin.users.activity.loading": "እንቅስቃሴን በመጫን ላይ…",
  // U1g — edit user (admin).
  // TODO-native-review: provisional Amharic, pending native speaker review.
  "admin.users.edit.title": "መገለጫ አርትዕ",
  "admin.users.edit.description": "በሠራተኞች የተደረጉ እርማቶች በዚህ ተጠቃሚ እንቅስቃሴ ውስጥ ይመዘገባሉ።",
  "admin.users.edit.displayName": "የሚታይ ስም",
  "admin.users.edit.alias": "የሻጭ ስም",
  "admin.users.edit.aliasHelp": "ከ3–30 ቁምፊዎች፦ ፊደላት፣ ቁጥሮች፣ ክፍተት፣ ነጥብ፣ ሰረዝ ወይም ከስር ሰረዝ።",
  "admin.users.edit.country": "የትውልድ ሀገር",
  "admin.users.edit.countryNone": "አልተቀመጠም",
  "admin.users.edit.save": "ለውጦችን አስቀምጥ",
  "admin.users.edit.saved": "ተቀምጧል።",
  "admin.users.edit.failed": "መገለጫውን ማስቀመጥ አልተቻለም።",
  "admin.users.edit.errorNameRequired": "የሚታይ ስም ያስፈልጋል።",
  "admin.users.edit.errorAliasTaken": "ይህ የሻጭ ስም አስቀድሞ ተይዟል።",
  "admin.users.edit.errorAliasFormat": "ይህ የሻጭ ስም አይፈቀድም።",
  "admin.users.edit.errorCountry": "ይህ ሀገር አይገኝም።",
  "admin.users.edit.errorStepUp": "ይህን ለውጥ ለማስቀመጥ የአንድ ጊዜ ኮድ ያረጋግጡ።",
  "admin.users.edit.errorPermission": "ይህን መገለጫ ማርትዕ አይፈቀድልዎትም።",
  // U2 — Roles & Permissions console.
  // TODO-native-review: provisional Amharic, pending native speaker review.
  "admin.roles.title": "ሚናዎችና ፈቃዶች",
  "admin.roles.subtitle": "ማን ምን ማድረግ እንደሚችል። እያንዳንዱ ለውጥ የአንድ ጊዜ ኮድ ይጠይቃል።",
  "admin.roles.loading": "ሚናዎችን በመጫን ላይ…",
  "admin.roles.error": "ሚናዎችን መጫን አልተቻለም።",
  "admin.roles.empty": "ገና ሚና የለም።",
  "admin.roles.col.name": "ሚና",
  "admin.roles.col.display": "የሚታይ ስም",
  "admin.roles.col.kind": "ዓይነት",
  "admin.roles.col.members": "አባላት",
  "admin.roles.col.permissions": "ፈቃዶች",
  "admin.roles.kind.system": "የሥርዓት",
  "admin.roles.kind.custom": "ብጁ",
  "admin.roles.create.open": "ሚና ፍጠር",
  "admin.roles.create.title": "ሚና መፍጠር",
  "admin.roles.create.description": "ብጁ ሚና ያለ ምንም ፈቃድ ይጀምራል።",
  "admin.roles.create.name": "የሚና ቁልፍ",
  "admin.roles.create.nameHelp": "ትንንሽ ፊደላት፣ ቁጥሮች፣ ሰረዝ ወይም ከስር ሰረዝ።",
  "admin.roles.create.displayName": "የሚታይ ስም",
  "admin.roles.create.descriptionField": "መግለጫ",
  "admin.roles.create.submit": "ሚና ፍጠር",
  "admin.roles.create.cancel": "ተወው",
  "admin.roles.create.failed": "ሚናውን መፍጠር አልተቻለም።",
  "admin.roles.detail.notFound": "ይህ ሚና የለም።",
  "admin.roles.detail.back": "ወደ ሚናዎች ተመለስ",
  "admin.roles.meta.title": "የሚና ዝርዝሮች",
  "admin.roles.meta.description": "የሚና ቁልፉ አይለወጥም፤ የሚታየው ስምና መግለጫው ግን ይለወጣሉ።",
  "admin.roles.meta.save": "ለውጦችን አስቀምጥ",
  "admin.roles.meta.saved": "ተቀምጧል።",
  "admin.roles.meta.failed": "ሚናውን ማስቀመጥ አልተቻለም።",
  "admin.roles.systemLockedNote": "ይህ የሥርዓት ሚና ነው። ማርትዕ፣ መሰረዝ ወይም ፈቃዶቹን መለወጥ አይቻልም።",
  "admin.roles.perm.title": "ፈቃዶች",
  "admin.roles.perm.description": "በአንድ ጊዜ አንድ ፈቃድ ይስጡ ወይም ያንሱ።",
  "admin.roles.perm.action": "እርምጃ",
  "admin.roles.perm.granted": "ተሰጥቷል",
  "admin.roles.perm.grant": "ስጥ",
  "admin.roles.perm.revoke": "አንሳ",
  "admin.roles.perm.locked": "ተቆልፏል",
  "admin.roles.perm.coreNote": "መሠረታዊ ፈቃድ — ሊነሳ አይችልም።",
  "admin.roles.perm.notAssignable": "በሥርዓቱ የሚተዳደር — ለብጁ ሚናዎች ሊሰጥ አይችልም",
  "admin.roles.perm.baselineBadge": "ሁሉም ሰው በ«ተጠቃሚ» ሚና በኩል ይህ አለው",
  "admin.roles.perm.stepUp": "ኮድ ይፈልጋል",
  "admin.roles.perm.failed": "የፈቃዱ ለውጥ አልተሳካም።",
  // U2a — የማትሪክሱ የቃላት ስብስብ (የገጽ ቃላት እንጂ መረጃ አይደለም)።
  "admin.roles.perm.action.access": "መድረስ",
  "admin.roles.perm.action.activate": "ማንቃት",
  "admin.roles.perm.action.approve": "አጽድቅ",
  "admin.roles.perm.action.assets": "ምስሎችና ንብረቶች",
  "admin.roles.perm.action.assign": "መመደብ",
  "admin.roles.perm.action.create": "መፍጠር",
  "admin.roles.perm.action.delete": "መሰረዝ",
  "admin.roles.perm.action.enforce": "ማስፈጸም",
  "admin.roles.perm.action.export": "ወደ ውጭ ማውጣት",
  "admin.roles.perm.action.machine": "በማሽን ተርጉም",
  "admin.roles.perm.action.manage": "ማስተዳደር",
  "admin.roles.perm.action.restructure": "መልሶ ማዋቀር",
  "admin.roles.perm.action.review": "መገምገም",
  "admin.roles.perm.action.update": "ማዘመን",
  "admin.roles.perm.action.use": "መጠቀም",
  "admin.roles.perm.action.view": "ማየት",
  "admin.roles.perm.resource.account_panel": "የመለያ ክፍል",
  "admin.roles.perm.resource.admin_panel": "የአስተዳደር ክፍል",
  "admin.roles.perm.resource.audit_logs": "የኦዲት መዝገቦች",
  "admin.roles.perm.resource.categories": "ምድቦች",
  "admin.roles.perm.resource.countries": "አገሮች",
  "admin.roles.perm.resource.coverage": "የማስታወቂያ ሽፋን",
  "admin.roles.perm.resource.impersonation": "በሌላ ተጠቃሚ ስም መግባት",
  "admin.roles.perm.resource.listings": "ማስታወቂያዎች",
  "admin.roles.perm.resource.locations": "አካባቢዎች",
  "admin.roles.perm.resource.permissions": "ፈቃዶች",
  "admin.roles.perm.resource.profiles": "መገለጫዎች",
  "admin.roles.perm.resource.roles": "ሚናዎች",
  "admin.roles.perm.resource.translations": "ትርጉሞች",
  "admin.roles.perm.resource.user_roles": "የተጠቃሚ ሚናዎች",
  "admin.roles.members.title": "አባላት",
  "admin.roles.members.count": "{n} አባላት ይህን ሚና ይዘዋል",
  "admin.roles.members.view": "አባላትን ተመልከት",
  "admin.roles.danger.title": "ይህን ሚና ሰርዝ",
  "admin.roles.danger.description": "ሚናውን መሰረዝ እሱንና ፈቃዶቹን ያስወግዳል። ይህ አይመለስም።",
  "admin.roles.danger.hasMembers": "ሚናውን ከመሰረዝዎ በፊት ሁሉንም አባላት ያስወግዱ።",
  "admin.roles.danger.confirmLabel": "ለማረጋገጥ የሚናውን ቁልፍ ይተይቡ",
  "admin.roles.danger.confirmHint": "ለማረጋገጥ {key} ይተይቡ",
  "admin.roles.danger.delete": "ሚና ሰርዝ",
  "admin.roles.danger.failed": "ሚናውን መሰረዝ አልተቻለም።",
  "admin.roles.error.stepUp": "ይህን ለውጥ ለማድረግ የአንድ ጊዜ ኮድ ያረጋግጡ።",
  "admin.roles.error.permission": "ይህን ለውጥ ማድረግ አይፈቀድልዎትም።",
  "admin.roles.error.systemLocked": "የሥርዓት ሚናዎች አይለወጡም።",
  "admin.roles.error.coreLocked": "ይህ መሠረታዊ ፈቃድ ሊነሳ አይችልም።",
  "admin.roles.error.hasMembers": "ይህ ሚና አሁንም አባላት አሉት።",
  "admin.roles.error.nameTaken": "ይህ የሚና ቁልፍ አስቀድሞ ተይዟል።",
  "admin.roles.error.nameRequired": "የሚና ቁልፍ ያስፈልጋል።",
  "admin.roles.error.nameFormat": "ትንንሽ ፊደላት፣ ቁጥሮች፣ ሰረዝ ወይም ከስር ሰረዝ ይጠቀሙ።",
  "account.deactivatedBanner": "መለያዎ ተቋርጧል — ድጋፍን ያግኙ።",
  "prim.state.loading": "በመጫን ላይ…",
  "prim.state.empty": "ገና የሚታይ ነገር የለም።",
  "prim.state.error": "የሆነ ችግር ተፈጥሯል።",
  "prim.table.previous": "ቀዳሚ",
  "prim.table.next": "ቀጣይ",
  "prim.table.of": "ከ",
  "prim.table.selected": "ተመርጠዋል",
  "prim.table.selectRow": "ረድፍ ምረጥ",
  "prim.table.selectAll": "ሁሉንም ረድፎች ምረጥ",
  "prim.table.actions": "እርምጃዎች",
  "mfa.title": "ባለ ሁለት ደረጃ ማረጋገጫ",
  "mfa.description": "ሚስጥራዊ እርምጃዎችን ለመጠበቅ ከማረጋገጫ መተግበሪያ የሚመጣ የአንድ ጊዜ ኮድ ይጨምሩ።",
  "mfa.statusOn": "ባለ ሁለት ደረጃ ማረጋገጫ በርቷል",
  "mfa.statusOff": "ባለ ሁለት ደረጃ ማረጋገጫ ጠፍቷል",
  "mfa.enroll": "ባለ ሁለት ደረጃ ማረጋገጫን ያዋቅሩ",
  "mfa.factorName": "የማረጋገጫ መተግበሪያ",
  "mfa.addedOn": "የተጨመረው {when}",
  "mfa.step1": "1. ይህን ኮድ በማረጋገጫ መተግበሪያዎ ይቃኙ",
  "mfa.step2": "2. ከመተግበሪያው የሚታየውን ስድስት አሃዝ ኮድ ያስገቡ",
  "mfa.qrAlt": "ለማረጋገጫ መተግበሪያዎ የQR ኮድ",
  "mfa.secretLabel": "መቃኘት ካልቻሉ ይህን ቁልፍ በእጅ ያስገቡ",
  "mfa.codeLabel": "ስድስት አሃዝ ኮድ",
  "mfa.codePlaceholder": "123456",
  "mfa.verifyActivate": "አረጋግጠው ያብሩ",
  "mfa.enrolledDone": "ባለ ሁለት ደረጃ ማረጋገጫ አሁን በርቷል።",
  "mfa.remove": "የማረጋገጫ መተግበሪያን አስወግድ",
  "mfa.removeConfirm": "ይህን የማረጋገጫ መተግበሪያ ለማስወገድ አሁን ያለውን ኮድ ያስገቡ።",
  "mfa.removeConfirmYes": "አዎ፣ አስወግደው",
  "mfa.removed": "የማረጋገጫ መተግበሪያዎ ተወግዷል።",
  "mfa.errorGeneric": "ይህን ማጠናቀቅ አልቻልንም። እባክዎ እንደገና ይሞክሩ።",
  "mfa.errorInvalidCode": "ኮዱ ትክክል አይደለም። ከመተግበሪያዎ አሁን ያለውን ኮድ ያስገቡ።",
  "mfa.errorNoFactor": "በዚህ መለያ ላይ የተዋቀረ የማረጋገጫ መተግበሪያ የለም።",
  "mfa.stepUpTitle": "ባለ ሁለት ደረጃ ማረጋገጫ ያስፈልጋል",
  "mfa.stepUpBody": "ይህ እርምጃ ባለ ሁለት ደረጃ ማረጋገጫ ይፈልጋል። ከማረጋገጫ መተግበሪያዎ አሁን ያለውን ኮድ ያስገቡ።",
  "mfa.stepUpConfirm": "አረጋግጠው ይቀጥሉ",
  "mfa.stepUpNoFactorTitle": "መጀመሪያ ባለ ሁለት ደረጃ ማረጋገጫን ያዋቅሩ",
  "mfa.stepUpNoFactorBody": "ይህ እርምጃ ባለ ሁለት ደረጃ ማረጋገጫ ይፈልጋል፤ ይህ መለያ ግን እስካሁን የማረጋገጫ መተግበሪያ የለውም።",
  "mfa.stepUpGoToSettings": "ወደ ቅንብሮች ይሂዱ",
  "mfa.stepUpUnavailableHint": "ይህን እርምጃ ለመፈጸም በቅንብሮች → ባለሁለት ደረጃ ማረጋገጫ ውስጥ የማረጋገጫ መተግበሪያ ያዘጋጁ።",
  "mfa.stepUpFreshSignIn": "አዲስ መግቢያ ሁልጊዜ ያልተረጋገጠ ሆኖ ይጀምራል፤ ስለዚህ ሚስጥራዊ እርምጃዎች ኮድ እንደገና ይጠይቃሉ።",
  "common.save": "አስቀምጥ",
  // U3 — ኦዲት እና ደህንነት።
  "admin.audit.title": "ኦዲት እና ደህንነት",
  "admin.audit.loading": "የኦዲት ክንውኖች በመጫን ላይ…",
  "admin.audit.error": "የኦዲት ክንውኖችን መጫን አልተቻለም።",
  "admin.audit.empty": "በእነዚህ ማጣሪያዎች ምንም ክንውን የለም።",
  "admin.audit.caption": "የኦዲት ክንውኖች",
  "admin.audit.stat.events24h": "ክንውኖች (24 ሰዓት)",
  "admin.audit.stat.events7d": "ክንውኖች (7 ቀን)",
  "admin.audit.stat.activeImpersonations": "ንቁ የመወከል ክፍለ ጊዜዎች",
  "admin.audit.stat.topAction": "ቀዳሚ እርምጃ",
  "admin.audit.chart.title": "በቀን የክንውኖች ብዛት",
  "admin.audit.chart.description": "ባለፉት 14 ቀናት የተመዘገቡ የኦዲት ክንውኖች።",
  "admin.audit.chart.bar": "{date}: {count} ክንውኖች",
  "admin.audit.col.time": "ሰዓት",
  "admin.audit.col.actor": "ተዋናይ",
  "admin.audit.col.action": "እርምጃ",
  "admin.audit.col.entity": "አካል",
  "admin.audit.col.meta": "ዝርዝሮች",
  "admin.audit.filter.action": "እርምጃ",
  "admin.audit.filter.entity": "የአካል ዓይነት",
  "admin.audit.filter.actor": "ተዋናይ ወይም አካል ይፈልጉ",
  "admin.audit.filter.from": "ከ",
  "admin.audit.filter.to": "እስከ",
  "admin.audit.filter.all": "ሁሉም",
  "admin.audit.expand": "ዝርዝሮች",
  "admin.audit.collapse": "ዝርዝሮችን ደብቅ",
  "admin.audit.detailTitle": "የክንውን ዝርዝር",
  "admin.audit.system": "ሲስተም",
  "impersonation.banner": "እንደ {name} በመመልከት ላይ — በ{time} ያበቃል",
  "impersonation.unknownTarget": "ይህ መለያ",
  "impersonation.open": "እይታውን ክፈት",
  "impersonation.endNow": "አሁን አቁም",
  "impersonation.viewAs": "እንደ ተጠቃሚው ይመልከቱ",
  "impersonation.reasonLabel": "እንደዚህ መለያ ለመመልከት ምክንያት",
  "impersonation.reasonPlaceholder": "የድጋፍ ጥያቄ #…",
  "impersonation.reasonTooShort": "ቢያንስ 5 ቁምፊ ያለው ምክንያት ይስጡ።",
  "impersonation.start": "ክፍለ ጊዜ ጀምር",
  "impersonation.cancel": "ሰርዝ",
  "impersonation.failed": "የመወከል ክፍለ ጊዜውን መጀመር አልተቻለም።",
  "impersonation.title": "እንደ {name} በመመልከት ላይ",
  "impersonation.readOnlyNote":
    "ለንባብ ብቻ የሆነ የድጋፍ እይታ — የዚህን መለያ መረጃ እያዩ ነው እንጂ እንደዚህ መለያ ሆነው እየሠሩ አይደለም። እዚህ የሚደረግ ምንም እርምጃ የእነሱን መረጃ አይለውጥም።",
  "impersonation.roadmapNote":
    "ሙሉ እንደ-ተጠቃሚ እይታ ታቅዷል (DEC-021)፤ በመወከል ጊዜ ሁሉንም ለውጦች በሰርቨር በኩል ይከለክላል።",
  "impersonation.endsIn": "የሚያበቃው በ",
  "impersonation.expired": "ይህ የመወከል ክፍለ ጊዜ አብቅቷል።",
  "impersonation.backToUser": "ወደ ተጠቃሚው ተመለስ",
  "impersonation.profileTitle": "መለያ",
  "impersonation.listingsTitle": "ማስታወቂያዎች",
  "impersonation.listings.caption": "የተወከለው መለያ ማስታወቂያዎች",
  "impersonation.listings.empty": "ይህ መለያ ማስታወቂያ የለውም።",
  "impersonation.listings.title": "ርዕስ",
  "impersonation.listings.status": "ሁኔታ",
  "impersonation.listings.price": "ዋጋ",
  "impersonation.listings.created": "የተፈጠረበት",
  "impersonation.profile.name": "የሚታይ ስም",
  "impersonation.profile.alias": "የሻጭ ስም",
  "impersonation.profile.country": "አገር",
  "impersonation.profile.status": "ሁኔታ",
  "impersonation.profile.joined": "የተቀላቀለበት",
  // U4b — Translations console (languages, strings editor, translator scope).
  "admin.section.translations.title": "ትርጉሞች",
  "admin.section.translations.body": "ቋንቋዎች፣ የቁልፍ ሽፋን እና የሐረግ አርታዒ።",
  "admin.translations.title": "ትርጉሞች",
  "admin.translations.subtitle": "ቋንቋዎች፣ ሽፋን እና የሐረግ አርታዒ።",
  "admin.translations.loading": "ቋንቋዎችን በመጫን ላይ…",
  "admin.translations.error": "ቋንቋዎችን መጫን አልተቻለም።",
  "admin.translations.empty": "እስካሁን ቋንቋ የለም።",
  "admin.translations.caption": "ቋንቋዎች",
  "admin.translations.stat.totalKeys": "በመሠረታዊ ካታሎግ ውስጥ ያሉ ቁልፎች",
  "admin.translations.stat.languages": "የነቁ ቋንቋዎች",
  "admin.translations.stat.approved": "በአጠቃላይ የጸደቁ",
  "admin.translations.col.language": "ቋንቋ",
  "admin.translations.col.code": "ኮድ",
  "admin.translations.col.nameEn": "የእንግሊዝኛ ስም",
  "admin.translations.meterNote": "ማተም በገጽታ ሽፋን ብቻ ይወሰናል፤ የይዘት ሽፋን መለኪያ ብቻ ነው።",
  "admin.translations.col.coverage": "ገጽታ",
  "admin.translations.col.interfaceTooltip": "የመተግበሪያ ጽሑፍ፦ ዝርዝሮች፣ አዝራሮች፣ መልዕክቶች (ማተምን ይወስናል)",
  "admin.translations.col.coverageData": "ይዘት",
  "admin.translations.col.contentTooltip": "የውሂብ ስሞች፦ ምድቦች እና አካባቢዎች (መለኪያ ብቻ)",
  "admin.translations.col.admin": "ሠራተኞች",
  "admin.translations.col.staffTooltip":
    "በሠራተኞች መሣሪያዎችና ቅድመ ዕይታዎች ውስጥ ይታያል፤ ሲጠፋ ከኮንሶል ዝርዝሮች ይደበቃል (መሠረታዊው ሁልጊዜ ክፍት ነው)",
  "admin.translations.col.public": "ለሕዝብ",
  "admin.translations.col.publicTooltip": "በጣቢያው ላይ ለጎብኚዎች በቀጥታ ይታያል",
  "admin.translations.badge.rtl": "ከቀኝ ወደ ግራ",
  "admin.translations.badge.base": "መሠረት",
  "admin.translations.badge.source": "የምንጭ ቋንቋ — በማመሳሰል ይያዛል፣ እዚህ አይስተካከልም",
  "admin.translations.coverage": "ከ{total} ውስጥ {approved} ጸድቀዋል",
  "admin.translations.publicGate": "ከ{total} ቁልፎች {remaining} ገና ማጽደቅ ይፈልጋሉ",
  "admin.translations.syncFirstTooltip": "ከመታተሙ በፊት ቁልፎችን አመሳስል",
  "admin.translations.switch.admin": "ለሠራተኞች ይገኛል",
  "admin.translations.switch.public": "ለጎብኚዎች ታትሟል",
  "admin.translations.open": "ሐረጎችን ክፈት",
  "admin.translations.add.title": "ቋንቋ ጨምር",
  "admin.translations.add.description": "አዲስ ቋንቋዎች መጀመሪያ ለሠራተኞች ብቻ ናቸው፤ መታተም በሽፋን ይወሰናል።",
  "admin.translations.add.code": "የቋንቋ ኮድ",
  "admin.translations.add.codePlaceholder": "so",
  "admin.translations.add.nameEn": "የእንግሊዝኛ ስም",
  "admin.translations.add.nameNative": "የራሱ ስም",
  "admin.translations.add.rtl": "ከቀኝ ወደ ግራ ጽሑፍ",
  "admin.translations.add.submit": "ቋንቋ ጨምር",
  "admin.translations.add.added": "ቋንቋ ተቀምጧል።",
  "admin.translations.add.pick": "ቋንቋ",
  "admin.translations.add.pickPlaceholder": "ቋንቋዎችን ይፈልጉ",
  "admin.translations.add.pickLoading": "የአቅራቢው የቋንቋ ዝርዝር እየተጫነ ነው…",
  "admin.translations.add.pickError": "የአቅራቢው የቋንቋ ዝርዝር አይገኝም — ቋንቋውን በእጅ ያስገቡ።",
  "admin.translations.add.pickEmpty": "ከዚያ ፍለጋ ጋር የሚዛመድ ቋንቋ የለም።",
  "admin.translations.add.pickSelected": "የተመረጠ፡ {language}",
  "admin.translations.add.manual": "አልተዘረዘረም — በእጅ ያስገቡ",
  "admin.translations.add.fromList": "ከዝርዝሩ ይምረጡ",
  "admin.translations.add.countries": "የሚነገርባቸው አገሮች",
  "admin.translations.add.countriesHint": "አማራጭ። ቋንቋውን በአገር ለማነጣጠር ይጠቅማል።",
  "admin.translations.add.countriesLoading": "አገሮች እየተጫኑ ነው…",
  "admin.translations.sync.title": "ቁልፎችን አመሳስል",
  "admin.translations.sync.description":
    "የተጠናቀረውን ካታሎግ ያስገባል፦ የእንግሊዝኛ መስመሮች ይጸድቃሉ፣ እያንዳንዱ የሠራተኛ ቋንቋ የጎደሉትን ቁልፎች እንደ ያልተተረጎሙ ያገኛል።",
  "admin.translations.sync.action": "ቁልፎችን አመሳስል",
  "admin.translations.sync.note": "የመጀመሪያው ማመሳሰል የተላኩትን የአማርኛ እሴቶች እንደ ጸድቀዋል ያስቀምጣል።",
  "admin.translations.sync.done":
    "{inserted} ቁልፎች በ{languages} ቋንቋዎች ተጨመሩ፣ {seeded} የአማርኛ እሴቶች ተዘርተዋል።",
  "admin.translations.error.stepUp": "ይህ እርምጃ ባለሁለት-ደረጃ ማረጋገጫ ይፈልጋል።",
  "admin.translations.error.scope": "ለዚህ ቋንቋ አልተመደቡም።",
  "admin.translations.error.coverage": "እያንዳንዱ ቁልፍ እስኪጸድቅ ድረስ ይህ ቋንቋ ሊታተም አይችልም።",
  "admin.translations.error.baseLocked": "እንግሊዝኛ የምንጭ ቋንቋ ነው፤ የሚቀየረው በቁልፍ ማመሳሰል ብቻ ነው።",
  "admin.translations.error.permission": "ይህን ለማድረግ ፈቃድ የሎትም።",
  "admin.translations.error.codeInvalid": "ትክክለኛ የቋንቋ ኮድ ያስገቡ (2–8 ፊደላት)።",
  "admin.translations.error.flaggedApprove": "የተለጠፉ ሐረጎች ሊጸድቁ አይችሉም — መጀመሪያ ቦታ ያዢዎቹን ያስተካክሉ።",
  "admin.translations.error.fileRejected": "ፋይሉ ሊነበብ አልቻለም። በUTF-8 ጽሑፍ ከ1 ሜባ በታች አድርገው እንደገና ይሞክሩ።",
  "admin.translations.error.fileShape": "ይህ የቃላት ፋይል አይደለም። መጀመሪያ ቋንቋውን ወደ ውጭ ልከው ተመሳሳይ አምዶች ይመልሱ።",
  "admin.translations.error.fileEmpty": "በዚህ ፋይል ውስጥ የሚገባ ረድፍ የለም።",
  "admin.translations.error.fileTooMany": "በአንድ ጊዜ በጣም ብዙ ነው። ፋይሉን ይክፈሉ ወይም ከአንድ ደቂቃ በኋላ ይሞክሩ።",
  "admin.translations.error.generic": "ያ ለውጥ ሊቀመጥ አልቻለም።",
  "admin.translations.strings.title": "ሐረጎች — {language}",
  "admin.translations.strings.back": "ሁሉም ቋንቋዎች",
  "admin.translations.strings.caption": "የትርጉም ቁልፎች",
  "admin.translations.strings.loading": "ሐረጎችን በመጫን ላይ…",
  "admin.translations.strings.error": "ሐረጎችን መጫን አልተቻለም።",
  "admin.translations.strings.empty": "ከእነዚህ ማጣሪያዎች ጋር የሚዛመድ ሐረግ የለም።",
  "admin.translations.strings.unknown": "ያ ቋንቋ በመቆጣጠሪያው ውስጥ አይገኝም።",
  "admin.translations.col.key": "ቁልፍ",
  "admin.translations.col.source": "የእንግሊዝኛ ምንጭ",
  "admin.translations.col.value": "ትርጉም",
  "admin.translations.col.status": "ሁኔታ",
  "admin.translations.col.provenance": "የመጨረሻ ለውጥ",
  "admin.translations.filter.search": "ቁልፎችንና እሴቶችን ፈልግ",
  "admin.translations.filter.all": "ሁሉም",
  "admin.translations.status.untranslated": "ያልተተረጎመ",
  "admin.translations.status.machine": "በማሽን",
  "admin.translations.status.edited": "የተስተካከለ",
  "admin.translations.status.approved": "የጸደቀ",
  "admin.translations.status.flagged": "የተመለከተ",
  "admin.translations.provenance.machine": "በማሽን",
  "admin.translations.provenance.human": "በሰው",
  "admin.translations.provenance.approvedBy": "በ{who} ጸድቋል",
  "admin.translations.provenance.none": "ተስተካክሎ አያውቅም",
  "admin.translations.expand": "አርትዕ",
  "admin.translations.collapse": "ዝጋ",
  "admin.translations.editor.label": "ትርጉም",
  "admin.translations.editor.source": "የእንግሊዝኛ ምንጭ",
  "admin.translations.editor.copy": "ምንጩን ቅዳ",
  "admin.translations.editor.copied": "ተቀድቷል።",
  "admin.translations.editor.save": "አስቀምጥ",
  "admin.translations.editor.saved": "ተቀምጧል።",
  "admin.translations.editor.approve": "አጽድቅ",
  "admin.translations.editor.clear": "አጽዳ",
  "admin.translations.editor.flagNote": "ተመልክቷል፦ {note}",
  // U4g-24 (INC-115) — የቦታ ያዥ ጥበቃና በአንድ ጠቅታ ማስተካከያ።
  "admin.translations.editor.placeholderRule": "የቦታ ያዦች በስማቸው ይሞላሉ — በእንግሊዝኛው ምንጭ እንዳሉት ሆነው ይቆዩ።",
  "admin.translations.editor.restorePlaceholders": "የቦታ ያዦችን መልስ",
  "admin.translations.editor.restoreHint":
    "መጀመሪያ ብዛታቸው መመሳሰል አለበት፦ ትርጉሙ {got} የቦታ ያዦች አሉት፣ የእንግሊዝኛው ምንጭ {want} አሉት።",
  "admin.translations.editor.notAssigned": "ለዚህ ቋንቋ አልተመደቡም — ሰርቨሩ ለውጦችን ይከለክላል።",
  // U4i ① — የቁልፍ አውድ ማስታወሻ (በመሠረት ረድፍ ላይ ይቀመጣል፤ ለሁሉም ቋንቋዎች ይታያል)።
  "admin.translations.editor.context": "የአውድ ማስታወሻ",
  "admin.translations.editor.contextPlaceholder": "የት እንደሚታይ፣ ቃናው፣ ገደቦች…",
  "admin.translations.editor.contextSave": "ማስታወሻ አስቀምጥ",
  "admin.translations.editor.contextNone": "እስካሁን ማስታወሻ የለም።",
  // U4i ② — በግንባታ ጊዜ የሚዘጋጅ "የት እንደሚውል" ካርታ።
  "admin.translations.editor.usedOn": "የሚውለው በ፦ {surfaces}",
  "admin.translations.editor.usedOnUnknown": "የሚውለው በ፦ በመፈተሽ ላይ…",
  "admin.translations.editor.usedOnDynamic": "የሚውለው በ፦ ተለዋዋጭ — ቀጥተኛ ጥሪ አልተገኘም።",
  // U4i ③ — የርዝመት ማስጠንቀቂያ ብቻ ነው፤ አያግድም።
  "admin.translations.editor.lengthWarning": "ረጅም፦ ከእንግሊዝኛው {ratio}× — አቀማመጡን ይመልከቱ።",
  "admin.translations.editor.lengthChip": "ረጅም ×{ratio}",
  // U4i ⑤ — CSV / XLIFF ወደ ውጭ መላክና ማስገባት።
  "admin.translations.transfer.title": "ወደ ውጭ ላክ / አስገባ",
  "admin.translations.transfer.note":
    "አሁን የተዘረዘሩትን ረድፎች ይልካል። የገቡት ረድፎች እንደ የተስተካከሉ ሆነው ይቀመጣሉ፤ ማጽደቅ ይፈልጋሉ። የተበላሹ የቦታ ያዦች ይሰየማሉ፤ የማይታወቁ ቁልፎች ይዘለላሉ።",
  "admin.translations.transfer.exportCsv": "CSV ላክ",
  "admin.translations.transfer.exportXliff": "XLIFF ላክ",
  "admin.translations.transfer.import": "ፋይል አስገባ",
  "admin.translations.transfer.importing": "በማስገባት ላይ…",
  "admin.translations.transfer.summary":
    "የገቡ {imported} · የተሰየሙ {flagged} · ያልተቀየሩ {unchanged} · የተዘለሉ {skipped}።",
  // U4i ⑦ — ለአቀማመጥ ምርመራ የሚያገለግል የውሸት ትርጉም፤ በጭራሽ አይታተምም።
  "admin.translations.transfer.exporting": "በመላክ ላይ…",
  "admin.translations.transfer.exportAllHint": "ሙሉውን ቋንቋና ወሰን ይልካል — የፍለጋና የሁኔታ ማጣሪያዎች ችላ ይባላሉ።",
  // U4i-7 (INC-125)
  "admin.translations.transfer.undo": "ይህን ማስገባት መልስ",
  "admin.translations.transfer.undoing": "በመመለስ ላይ…",
  "admin.translations.transfer.undoResult":
    "{restored} ተመልሰዋል · {conflicted} አልተነኩም (ከማስገባቱ በኋላ ተቀይረዋል)።",
  "admin.translations.transfer.undoHint":
    "ይህ ማስገባት የጻፈውን አሁንም የያዙ ረድፎችን ከቀድሞው የግምገማ ሁኔታቸው ጋር ይመልሳል። ከዚያ በኋላ የተስተካከሉ ረድፎች አይነኩም።",
  "admin.translations.error.deletePublished": "ከመሰረዝዎ በፊት ይህን ቋንቋ ከሕዝብ ያንሱት።",
  "admin.translations.error.deleteBase": "የመሰረት ቋንቋ ሊሰረዝ አይችልም።",
  "admin.translations.delete.action": "ሰርዝ",
  "admin.translations.delete.title": "{language} ({code}) ይሰረዝ?",
  "admin.translations.delete.body": "ይህ ቋንቋውንና በእሱ ስር የተከማቸውን ሁሉ በቋሚነት ያስወግዳል። መመለስ አይቻልም።",
  "admin.translations.delete.counts":
    "{ui} የገጽታ ሐረጎች · {entity} የይዘት ስሞች · {revisions} የታሪክ ግቤቶች · {assignments} የተርጓሚ ምደባዎች",
  "admin.translations.delete.countsLoading": "የሚሰረዘውን በመቁጠር ላይ…",
  "admin.translations.delete.countsError": "እነዚያ ቁጥሮች ሊነበቡ አልቻሉም።",
  "admin.translations.delete.confirmLabel": "ለማረጋገጥ {code} ይተይቡ",
  "admin.translations.delete.cancel": "ተወው",
  "admin.translations.delete.confirm": "ቋንቋ ሰርዝ",
  "admin.translations.delete.pending": "በመሰረዝ ላይ…",
  "admin.translations.delete.done":
    "{code} ተሰርዟል — {ui} የገጽታ ሐረጎች፣ {entity} የይዘት ስሞች፣ {revisions} የታሪክ ግቤቶች፣ {assignments} ምደባዎች።",
  "admin.translations.delete.publishedBlocked": "የታተሙ ቋንቋዎች መሰረዝ አይችሉም። መጀመሪያ ሕዝባዊውን ያጥፉ።",
  "admin.translations.pseudo.title": "የውሸት ትርጉም (Pseudo)",
  "admin.translations.pseudo.note":
    "የ{code} የአቀማመጥ ምርመራ ቋንቋን በተዘረጋ ጽሑፍ ይሞላል። ለአስተዳዳሪ ብቻ ሲሆን በጭራሽ አይታተምም።",
  "admin.translations.pseudo.run": "የውሸት ትርጉም አመንጭ",
  "admin.translations.pseudo.pending": "በማመንጨት ላይ…",
  "admin.translations.pseudo.progress": "{done} ከ{total}",
  "admin.translations.pseudo.summary": "የተጻፉ {written} · ያልተሳኩ {failed}።",
  "admin.translations.pseudo.confirmTitle": "የ{code} የውሸት ትርጉም ይመንጭ?",
  "admin.translations.pseudo.confirmBody":
    "እያንዳንዱ የእንግሊዝኛ ጽሑፍ ወደ {code} የአቀማመጥ ምርመራ ቋንቋ በማሽን ይጻፋል። {code} ለአስተዳዳሪ ብቻ ነው፤ በጭራሽ አይታተምም።",
  "admin.translations.pseudo.confirmCancel": "ተወው",
  "admin.translations.pseudo.confirmAction": "አመንጭ",
  // U4c — በማሽን መተርጎም (ውጤቶቹ ጊዜያዊ ናቸው)።
  "admin.translations.ai.row": "በAI ተርጉም",
  "admin.translations.ai.pending": "በመተርጎም ላይ…",
  "admin.translations.ai.bulkAction": "ያልተተረጎሙትን ሁሉ በAI ተርጉም ({count})",
  "admin.translations.ai.bulkActionFiltered": "ከተጣራው ውስጥ ያልተተረጎሙትን በAI ሙላ ({count})",
  "admin.translations.ai.confirmTitle": "ያልተተረጎሙትን ሐረጎች በማሽን ይተርጎሙ?",
  "admin.translations.ai.confirmBody":
    "{count} ሐረጎች ለማሽን ትርጉም ይላካሉ። ውጤቶቹ እንደ ጊዜያዊ የማሽን ትርጉም ይቀመጣሉ፤ ከመታተማቸው በፊት መጽደቅ አለባቸው።",
  "admin.translations.ai.confirmCta": "{count} ተርጉም",
  "admin.translations.ai.cancel": "ተወው",
  "admin.translations.ai.progress": "ከ{total} ውስጥ {done} ተልኳል…",
  "admin.translations.ai.summary": "{done} ተተርጉመዋል፣ {flagged} ተለይተዋል፣ {failed} አልተሳኩም።",
  "admin.translations.ai.failedList": "ያልተተረጎሙ: {keys}",
  "admin.translations.ai.error": "የማሽን ትርጉሙን ማጠናቀቅ አልተቻለም።",
  "admin.translations.translator.title": "የተርጓሚ ቋንቋዎች",
  "admin.translations.translator.scopeNote":
    "ለትርጉም አስተዳዳሪዎች ብቻ የታይ። የተመደቡ ቋንቋዎች ይህ ሰው የትኞቹን ቋንቋዎች ሊያስተካክል እንደሚችል ይወስናል፤ የተርጓሚው ሚና ራሱ የማስተካከል ፈቃድ ይሰጣል።",
  "admin.translations.translator.empty": "ምንም የተርጓሚ ቋንቋ አልተመደበም።",
  "admin.translations.translator.noRole": "የቋንቋ መቆጣጠር እንዲኖር የተርጓሚ ሚና ይመድቡ።",
  "admin.translations.translator.checkError": "የተርጓሚ ፈቃዶችን ማረጋገጥ አልተቻለም — እንደገና ይሞክሩ።",
  "admin.translations.translator.none": "የተመደበ ቋንቋ የለም።",
  "admin.translations.translator.save": "ምድብ አስቀምጥ",
  "admin.translations.translator.saved": "ምድብ ተቀምጧል።",
  "admin.translations.translator.audit": "እዚህ የሚደረጉ ለውጦች ይመዘገባሉ።",
  // U4d — የይዘት (ዳታ) ትርጉሞች፦ የምድብና የቦታ ስሞች።
  "admin.translations.scope.label": "የትርጉም ወሰን",
  "admin.translations.scope.interface": "በይነገጽ",
  "admin.translations.scope.data": "ዳታ",
  "admin.translations.data.caption": "ለዚህ ቋንቋ የምድብና የቦታ ስሞች",
  "admin.translations.data.empty": "ከዚህ ማጣሪያ ጋር የሚዛመድ የይዘት ስም የለም።",
  "admin.translations.data.coverage": "ከ{total} የይዘት ስሞች ውስጥ {approved} ጸድቀዋል።",
  "admin.translations.data.aiNote": "የይዘት ስሞች የማሽን ትርጉሞች ጊዜያዊ ናቸው — ከመታተማቸው በፊት ያጽድቁ።",
  "admin.translations.data.gateNote": "የይዘት ሽፋን መለኪያ ብቻ ነው፤ ቋንቋን ማተም አሁንም በበይነገጽ ሐረጎች ላይ ይመሠረታል።",
  "admin.translations.col.entity": "ስም",
  "admin.translations.col.entityType": "ዓይነት",
  "admin.translations.entity.category": "ምድብ",
  "admin.translations.entity.location": "ቦታ",
  "admin.translations.entity.attribute": "ባህሪ",
  "admin.translations.history.open": "ታሪክ",
  "admin.translations.history.title": "የለውጥ ታሪክ",
  "admin.translations.history.loading": "ታሪክ በመጫን ላይ…",
  "admin.translations.history.error": "ታሪኩን መጫን አልተቻለም።",
  "admin.translations.history.empty": "እስካሁን ታሪክ የለም — ለውጦች እዚህ ይታያሉ።",
  "admin.translations.history.noValue": "(ምንም እሴት የለም)",
  "admin.translations.history.actor.system": "ሲስተም",
  "admin.translations.history.restore": "ይህን እሴት መልስ",
  "admin.translations.history.restored": "ተመልሷል።",
  "admin.translations.history.confirm": "ይህን ጽሑፍ እንደ የተስተካከለ እሴት ይመልሰዋል — ታሪኩ ሁሉንም ይይዛል",
  "admin.translations.history.clearInstead": "በምትኩ አጽዳ",
  "admin.translations.history.action.machine": "የማሽን ጽሑፍ",
  "admin.translations.history.action.save": "የሰው እርማት",
  "admin.translations.history.action.approve": "ጸድቋል",
  "admin.translations.approve.action": "ሁሉንም የተገመገሙ አጽድቅ ({count})",
  "admin.translations.approve.confirmTitle": "ሁሉንም የተገመገሙ ሐረጎች ማጽደቅ?",
  "admin.translations.approve.confirmBody":
    "{count} የተገመገሙ ሐረጎች ይጸድቃሉ። ያልተገመገመ የማሽን ጽሑፍ ለዚህ ቋንቋ ቀጥታ ይሆናል። የተለጠፉ ሐረጎች ይዘለላሉ።",
  "admin.translations.approve.confirmCta": "{count} አጽድቅ",
  "admin.translations.approve.cancel": "ሰርዝ",
  "admin.translations.approve.pending": "በማጽደቅ ላይ…",
  "admin.translations.approve.summary": "{approved} ጸድቀዋል፣ {skipped} ተለጥፈው ተዘለዋል።",
  "admin.translations.approve.none": "ማጽደቅ የሚጠብቅ ነገር የለም።",
  // U4k — የይዘት ስሞች የጅምላ ማጽደቅና ድርብ መለኪያዎች።
  "admin.translations.approve.entityAction": "ሁሉንም የይዘት ስሞች አጽድቅ ({count})",
  "admin.translations.approve.entityConfirmTitle": "ሁሉንም የተገመገሙ የይዘት ስሞች ማጽደቅ?",
  "admin.translations.approve.entityConfirmBody": "{count} የይዘት ስሞች ለዚህ ቋንቋ ቀጥታ ይሆናሉ።",
  "admin.translations.approve.entitySummary": "{approved} የይዘት ስሞች ጸድቀዋል።",
  "admin.translations.meters": "በይነገጽ {uiApproved}/{uiTotal} · ይዘት {dataApproved}/{dataTotal}",
  "admin.translations.order.up": "{language}ን ወደ ላይ አንቀሳቅስ",
  "admin.translations.order.down": "{language}ን ወደ ታች አንቀሳቅስ",
  "admin.translations.order.note": "ይህ ቅደም ተከተል የሕዝብ ቋንቋ መቀየሪያውን ይመራል፤ መሠረታዊው ቋንቋ መጀመሪያ ይቆያል።",
  "admin.translations.order.saved": "ቅደም ተከተል ተቀምጧል።",
  "admin.translations.orphaned.chip": "ወላጅ አልባ · {count}",
  "admin.translations.orphaned.note": "እነዚህ ቁልፎች በመተግበሪያው ካታሎግ ውስጥ የሉም፤ ከሽፋን ተገልለዋል እና ፈጽሞ አይላኩም።",
  "admin.translations.history.action.clear": "ጸድቷል",
  "admin.categories.title": "ምድቦች",
  "admin.categories.caption": "የምድብ ዛፍ",
  "admin.categories.loading": "ምድቦች በመጫን ላይ…",
  "admin.categories.error": "ምድቦችን መጫን አልተቻለም።",
  "admin.categories.empty": "ከዚህ ፍለጋ ጋር የሚዛመድ ምድብ የለም።",
  "admin.categories.searchPlaceholder": "ስም ወይም ስሌግ ይፈልጉ",
  "admin.categories.col.name": "ስም",
  "admin.categories.col.slug": "ስሌግ",
  "admin.categories.col.order": "ቅደም ተከተል",
  "admin.categories.col.flags": "ባህሪያት",
  "admin.categories.col.listings": "ዝርዝሮች",
  "admin.categories.col.exclusions": "የተገለሉ",
  "admin.categories.col.status": "ሁኔታ",
  "admin.categories.badge.catchall": "አጠቃላይ",
  "admin.categories.badge.inactive": "ጡረታ የወጣ",
  "admin.categories.badge.active": "ንቁ",
  "admin.categories.badge.listings": "ዝርዝሮችን ይቀበላል",
  "admin.categories.badge.price": "ዋጋ",
  "admin.categories.badge.window": "የቀን ክልል",
  "admin.categories.badge.scheduled": "መርሐ የተያዘለት",
  "admin.categories.badge.missingAssets": "ንብረቶች ይጎድላሉ",
  "admin.categories.tip.active": "በአሰሳ ውስጥ ንቁ ነው፦ ገዢዎች ሊደርሱበት እና ሻጮች ሊለጥፉበት ይችላሉ።",
  "admin.categories.tip.retired":
    "ተቋርጧል፦ ታሪኩንና የአሰሳ አገናኞቹን ይይዛል፣ ነገር ግን አዲስ ማስታወቂያ ሊለጠፍበት አይችልም እና እንደ መድረሻ አይታይም።",
  "admin.categories.tip.catchall": "ሁሉንም ተቀባይ፦ ከእህት ምድብ የተቋረጡ ማስታወቂያዎች ወደዚህ ይዛወራሉ።",
  "admin.categories.tip.listings": "ሻጮች በቀጥታ በዚህ ምድብ ውስጥ ማስታወቂያ መለጠፍ ይችላሉ።",
  "admin.categories.tip.price": "በዚህ ምድብ ውስጥ ያሉ ማስታወቂያዎች ዋጋ ይይዛሉ።",
  "admin.categories.tip.window": "ታይነቱ ተይዞለታል፦ ምድቡ በቀን ገደቡ ውስጥ ብቻ ይታያል።",
  "admin.categories.tip.scheduled": "መርሐ ተያዝሏል፦ ይህ ምድብ አሁን አይታይም — በሚጀምርበት ቀን ይታያል።",
  "admin.categories.tip.missingAssets":
    "ንብረቶች ይጎድላሉ፦ ይህ ምድብ አዶ ወይም ምስል የለውም፣ ስለዚህ በአሰሳ ላይ ባዶ ሆኖ ይታያል።",
  "admin.categories.filter.missingAssets": "ንብረቶች የሚጎድላቸው",
  "admin.categories.filter.pageSize": "በአንድ ገጽ ረድፎች",
  "admin.categories.create.slugPreview": "ስም-መለያ (በአገልጋዩ የሚመነጭ)፦",
  "admin.categories.action.edit": "አርትዕ",
  "admin.categories.action.window": "ታይነት",
  "admin.categories.action.exclusions": "አገሮች",
  "admin.categories.action.retire": "ጡረታ አውጣ",
  "admin.categories.action.reactivate": "እንደገና አንቃ",
  "admin.categories.action.delete": "ሰርዝ",
  "admin.categories.col.parent": "ወላጅ",
  "admin.categories.filter.root": "በዋና ምድብ አጣራ",
  "admin.categories.filter.allRoots": "ሁሉም ዋና ምድቦች",
  "admin.categories.filter.allCategories": "ሁሉም ምድቦች",
  "admin.categories.filter.retired": "ተቋርጧል",
  "admin.categories.paths.title": "የአሰሳ መንገዶች",
  "admin.categories.paths.hint":
    "ይህ ምድብ በአሰሳ ዛፉ ውስጥ የሚታይባቸው ሁሉም ቦታዎች። መንገድን ማንቀሳቀስ ወይም ማስወገድ ምድቡን አያባዛም።",
  "admin.categories.paths.loading": "የአሰሳ መንገዶች በመጫን ላይ…",
  "admin.categories.paths.error": "የአሰሳ መንገዶችን መጫን አልተቻለም።",
  "admin.categories.paths.empty": "ይህ ምድብ እስካሁን የአሰሳ መንገድ የለውም።",
  "admin.categories.paths.root": "ከፍተኛ ደረጃ (ዋና)",
  "admin.categories.paths.moveTo": "ይህን መንገድ ወደዚህ አንቀሳቅስ",
  "admin.categories.paths.remove": "መንገድ አስወግድ",
  "admin.categories.paths.primary": "ዋና",
  "admin.categories.paths.primaryTip": "በዝርዝሩና በዳሰሳ መስመሩ ውስጥ እንደ ወላጅ ይታያል።",
  "admin.categories.paths.add": "ሌላ የአሰሳ መንገድ ጨምር",
  "admin.categories.paths.addSubmit": "መንገድ ጨምር",
  "admin.categories.action.pointer": "ጠቋሚ ጨምር",
  "admin.categories.action.up": "ወደ ላይ አንቀሳቅስ",
  "admin.categories.action.down": "ወደ ታች አንቀሳቅስ",
  "admin.categories.create.open": "ምድብ ፍጠር",
  "admin.categories.create.title": "ምድብ ፍጠር",
  "admin.categories.create.name": "ስም (እንግሊዝኛ)",
  "admin.categories.create.icon": "አዶ",
  "admin.categories.create.parent": "ወላጅ",
  "admin.categories.create.parentRoot": "ወላጅ የለም (ሥር)",
  // C5b — category imagery + AI-assisted creation.
  "admin.categories.action.image": "ምስል",
  "admin.categories.image.title": "የምድብ ምስል",
  "admin.categories.image.review":
    "የተፈጠሩ ምስሎች ወዲያውኑ ይቀመጣሉ እና እዚህ ይገመገማሉ፤ ትክክል እስኪሆን ድረስ እንደገና ያመንጩ።",
  "admin.categories.image.empty": "ለዚህ ምድብ ገና ምስል የለም።",
  "admin.categories.image.generate": "ምስል አመንጭ",
  "admin.categories.image.regenerate": "ምስል እንደገና አመንጭ",
  "admin.categories.image.busy": "በማመንጨት ላይ…",
  "admin.categories.image.card": "ካርድ 512",
  "admin.categories.image.thumb": "አነስተኛ ምስል 128",
  "admin.categories.image.og": "የማኅበራዊ ምስል 1200x630",
  "admin.categories.image.loading": "የተቀመጠውን ምስል በመጫን ላይ…",
  "admin.categories.image.acceptToast": "ምስሉ ተቀብሏል።",
  "admin.categories.image.accept": "ይህን ምስል ተቀበል",
  "admin.categories.image.accepted": "ተቀብሏል",
  "admin.categories.error.acceptNoImage": "ከመቀበልዎ በፊት ምስል ያመንጩ።",
  "admin.categories.image.stageLabel": "ደረጃ:",
  "admin.categories.error.imageFailed": "ምስል ማመንጨት አልተሳካም",
  "admin.categories.bulk.generateMissing": "የጎደሉትን አመንጭ (ቢበዛ 25)",
  "admin.categories.bulk.generateHint":
    "በአንድ አሂድ ንብረት ለሌላቸው እስከ 25 ምድቦች የካርድ፣ የትንሽ ምስል እና የOG ምስሎችን ያመነጫል (DEC-032 የወጪ ወሰን)",
  "admin.categories.bulk.progress": "በማመንጨት ላይ",
  "admin.categories.bulk.generated": "ተመንጭተዋል",
  "admin.categories.bulk.failed": "አልተሳኩም",
  "admin.categories.bulk.more": "ለቀጣዮቹ 25 እንደገና ያስኪዱ።",
  "admin.categories.export.open": "ምድቦችን ላክ",
  "admin.categories.export.busy": "ልኩን በማዘጋጀት ላይ…",
  "admin.categories.export.error": "ልኩ ሊዘጋጅ አልቻለም።",
  "admin.categories.import.open": "ምድቦችን አስገባ",
  "admin.categories.import.title": "ምድቦችን ማስገባት",
  "admin.categories.import.hint": "ያወጡትን የምድቦች ፋይል ይምረጡ። ቅድመ-እይታ አይተው እስኪያረጋግጡ ድረስ ምንም አይጻፍም።",
  "admin.categories.import.file": "የምድቦች ፋይል",
  "admin.categories.import.preview": "ለውጦችን በቅድሚያ አሳይ",
  "admin.categories.import.busy": "ፋይሉን በመፈተሽ ላይ…",
  "admin.categories.import.counts":
    "{adds} ተጨምረዋል · {changes} ተቀይረዋል · {retires} ተቋርጠዋል · {reactivations} ተመልሰዋል · {deletes} ተሰርዘዋል · {unchanged} አልተቀየሩም · {refusals} ተከልክለዋል",
  "admin.categories.import.refusalRow": "ረድፍ {row}",
  "admin.categories.import.confirm": "ማስገባቱን አረጋግጥ",
  "admin.categories.import.discard": "ተው",
  "admin.categories.import.choose": "{file}ን ይምረጡ…",
  "admin.categories.import.noFile": "ምንም ፋይል አልተመረጠም",
  "admin.categories.import.discardHint": "ምንም ሳይጻፍ ይዘጋል",
  "admin.categories.import.applied": "ማስገባቱ ተፈጽሟል — {count} ለውጦች ተጽፈዋል",
  "admin.categories.import.close": "ዝጋ",
  "admin.categories.import.undo": "የመጨረሻውን ማስገባት መልስ",
  "admin.categories.import.undone": "{count} ምድቦች ተመልሰዋል።",
  "admin.categories.import.reason.missingSlug":
    "የምድብ አድራሻ የለም፦ category_slug ባዶ ነው (መንገድ “{category_path}”፣ ስም “{name_en}”)",
  "admin.categories.import.reason.duplicateSlug":
    "category_slug “{category_slug}” በዚህ ፋይል ሁለት ጊዜ ተጠቅሷል (መንገድ “{category_path}”)",
  "admin.categories.import.reason.badAction":
    "ያልታወቀ ትዕዛዝ “{action}” በ“{category_slug}” ላይ — upsert፣ create-root፣ retire፣ reactivate ወይም delete ይጠቀሙ",
  "admin.categories.import.reason.missingName":
    "አዲስ ምድብ ስም ያስፈልገዋል፦ name_en በ“{category_slug}” ላይ ባዶ ነው",
  "admin.categories.import.reason.unknownSlug":
    "ያልታወቀ ምድብ፦ category_slug “{category_slug}” በዝርዝሩ የለም (መንገድ “{category_path}”)",
  "admin.categories.import.reason.unknownParent":
    "ያልታወቀ ወላጅ ምድብ፦ parent_slug “{parent_slug}” በዝርዝሩም በዚህ ፋይልም የለም (የመስመሩ slug “{category_slug}”፣ መንገድ “{category_path}”)",
  "admin.categories.import.reason.catchallParent":
    "“ሌላ” ማቆያ ወላጅ መሆን አይችልም፦ parent_slug “{parent_slug}” ማቆያ ነው — ለ“{category_slug}” ሌላ ወላጅ ይምረጡ",
  "admin.categories.import.reason.cycle":
    "ይህ ዝውውር “{category_slug}”ን በራሱ ውስጥ ያደርገዋል — parent_slug “{parent_slug}” ከሱ በታች ነው",
  "admin.categories.import.reason.slugRename":
    "የምድብ አድራሻ መለያ ነው፤ እዚህ መቀየር አይቻልም፦ “{detail}” → “{category_slug}” (መንገድ “{category_path}”) — “{detail}”ን መልሰው ስሙን ብቻ ይቀይሩ",
  "admin.categories.import.reason.hasChildren":
    "በ“{category_slug}” ስር {detail} ምድቦች አሉ — መጀመሪያ ያንቀሳቅሱ ወይም ይሰርዙ",
  "admin.categories.import.reason.hasListings":
    "በ“{category_slug}” ውስጥ {detail} ማስታወቂያዎች አሉ — መጀመሪያ ወደ ሌላ ምድብ ያዛውሩ",
  "admin.categories.import.reason.deleteActive":
    "ከመሰረዝ በፊት “{category_slug}”ን ያቋርጡ (is_active “{is_active}” ነው)",
  "admin.categories.import.reason.unknownCountry":
    "ያልታወቀ የአገር ኮድ በ excluded_country_codes “{excluded_country_codes}” ላይ ({category_slug})",
  "admin.categories.import.reason.badDate":
    "ቀኑ ሊነበብ አልቻለም፦ visible_from “{visible_from}”፣ visible_until “{visible_until}” ({category_slug})",
  "admin.categories.import.reason.outOfScope":
    "“{category_slug}” (መንገድ “{category_path}”) ካጣሩት ምድብ ውጭ ነው",
  "admin.categories.import.reason.formula":
    "ሕዋሶች በ = + - @ መጀመር አይችሉም (የመስመሩ slug “{category_slug}”)",
  "admin.categories.import.reason.statusNeedsAction":
    "የ“{category_slug}” የገቢርነት ምልክት ከተመዘገበው ሁኔታ ይለያል — የተመዘገበው {stored}፣ ይህ ፋይል የሚጠይቀው {requested} ነው፤ action = reactivate ወይም retire ይጠቀሙ",
  "admin.categories.import.reason.alreadyRetired": "“{category_slug}” አስቀድሞ ተቋርጧል — የሚቀየር ነገር የለም",
  "admin.categories.import.reason.alreadyDeleted":
    "“{category_slug}” ከዝርዝሩ ውስጥ የለም — የሚሰረዝ ነገር የለም",
  "admin.categories.import.reason.unknown": "ይህ ረድፍ ተከልክሏል",

  "admin.categories.import.guidance":
    "“(read-only)” የተባሉ አምዶች በራስ-ሰር ይሰላሉ፤ በፋይሉ ውስጥ ቢቀይሯቸውም ተግባራዊ አይሆኑም።",
  "admin.categories.import.ignored": "ችላ የተባሉ (read-only)",
  "admin.categories.import.ignoredRow": "መስመር {row} — {column} ተግባራዊ አልሆነም",
  "admin.categories.import.ignoredColumn": "{column} — {count} መስመሮች ተግባራዊ አልሆኑም",
  "admin.categories.import.error.denied": "ምድቦችን ለማስገባት ፈቃድ የለዎትም።",
  "admin.categories.import.error.stepUp": "ኮድ አረጋግጠው እንደገና ይሞክሩ።",
  "admin.categories.import.error.fileChanged": "ከቅድመ-እይታው በኋላ ፋይሉ ተቀይሯል። እንደገና ይመልከቱ።",
  "admin.categories.import.error.busy": "ሌላ ማስገባት እየሄደ ነው።",
  "admin.categories.import.error.tooLarge": "ፋይሉ ከ1 ሜባ ይበልጣል።",
  "admin.categories.import.error.tooManyRows": "ፋይሉ ከ5,000 ረድፎች ይበልጣል።",
  "admin.categories.import.error.badHeader": "የአምዶቹ አርዕስት ከልኩ ጋር አይመሳሰልም።",
  "admin.categories.import.error.emptyFile": "ፋይሉ ባዶ ነው።",
  "admin.categories.import.error.wrongFile": "ይህ የባህሪያት ፋይል ነው — ከባህሪያት ገጽ ያስገቡት።",
  "admin.categories.import.error.failed": "ማስገባቱ ሊጠናቀቅ አልቻለም።",
  "admin.categories.import.error.reason": "ምክንያት፦ {reason}",
  "admin.categories.edit.title": "ምድብ አርትዕ",
  "admin.categories.field.name": "ስም (እንግሊዝኛ)",
  "admin.categories.field.icon": "አዶ",
  "admin.categories.field.order": "የማሳያ ቅደም ተከተል",
  "admin.categories.field.orderManaged": "በ«ወደ ላይ አንቀሳቅስ/ወደ ታች አንቀሳቅስ» ይተዳደራል።",
  "admin.categories.field.expiryDays": "የዝርዝር ማብቂያ (ቀናት)",
  "admin.categories.field.expiryNone": "ማብቂያ የለም",
  "admin.categories.field.allowListings": "ዝርዝሮችን ይቀበላል",
  "admin.categories.field.priceEnabled": "የዋጋ መስክ ነቅቷል",
  "admin.categories.field.iconChange": "ቀይር",
  // C5l — ሁለት-ደረጃ የመፍጠሪያ መስኮት: ዝርዝሮች (አገሮችና ቦታ ጋር)፣ ከዚያ ምስል።
  "admin.categories.field.iconHint": "ስም ካስገቡ በኋላ ይጠቆማል።",
  "admin.categories.create.position": "ቦታ",
  "admin.categories.create.positionEnd": "በመጨረሻ (ቦታ {n})",
  "admin.categories.create.positionBefore": "ከ{name} በፊት",
  "admin.categories.create.positionCount": "{n} ንዑስ ምድቦች በ{parent}",
  "admin.categories.create.countries": "በእነዚህ አገሮች ደብቅ",
  "admin.categories.create.countriesHint": "ምድቡ በመለሰው አገር ሁሉ ይደበቃል፤ በኋላ መቀየር ይችላሉ።",
  "admin.categories.error.createPartial": "ምድቡ ተፈጥሯል፣ ግን የተከተለው ደረጃ አልተሳካም። ማጠናቀቅ ከዝርዝሩ ውስጥ ክፈቱት።",
  "admin.categories.window.title": "የታይነት መስኮት",
  "admin.categories.window.hint": "ገደቡን ለማስወገድ ቀኑን ባዶ ይተዉት። ሰዓቶች በUTC ናቸው።",
  "admin.categories.window.from": "የሚታይ ከ",
  "admin.categories.window.until": "የሚታይ እስከ",
  "admin.categories.exclusions.title": "የአገር ገደቦች",
  "admin.categories.exclusions.hint": "ማስቀመጥ ለዚህ ምድብ ያለውን ሙሉ የገደብ ዝርዝር ይተካል።",
  "admin.categories.exclusions.current": "አሁን የተገለሉ",
  "admin.categories.retire.title": "ምድብ ጡረታ አውጣ",
  "admin.categories.retire.hint": "ምድቡ ይቦዝናል፤ ዝርዝሮቹ ወደ መረጡት ምድብ ይዛወራሉ።",
  "admin.categories.retire.reassign": "ዝርዝሮችን ወደ",
  "admin.categories.retire.reassignPlaceholder": "ምድብ ይምረጡ",
  "admin.categories.retire.reassignCount": "{count} ዝርዝሮችን ወደ አዛውር",
  "admin.categories.retire.noListings": "የሚዛወር ዝርዝር የለም — ምድቡ ከአሰሳና ከመለጠፍ ብቻ ይሰወራል።",
  "admin.categories.retire.confirm": "ጡረታ አውጣ",
  "admin.categories.delete.title": "ምድብ ሰርዝ",
  "admin.categories.delete.hint":
    "ይህ የተቦዘነውን ምድብ፣ የአሰሳ ቦታዎቹን፣ የአገር ገደቦቹን፣ ባህሪያቱንና ትርጉሞቹን በቋሚነት ይሰርዛል። መመለስ አይቻልም።",
  "admin.categories.delete.confirmLabel": "ለማረጋገጥ slug ይተይቡ",
  "admin.categories.delete.confirm": "በቋሚነት ሰርዝ",
  "admin.categories.delete.mismatch": "slug አይዛመድም።",
  "admin.categories.pointer.title": "የዛፍ ጠቋሚ ጨምር",
  "admin.categories.pointer.hint": "ጠቋሚ ይህን ምድብ ሳይባዛ በሁለተኛ ወላጅ ስር ያሳያል።",
  "admin.categories.pointer.parent": "ተጨማሪ ወላጅ",
  "admin.categories.pointer.parentPlaceholder": "ወላጅ ይምረጡ",
  "admin.categories.error.saveFailed": "ለውጡን ማስቀመጥ አልተቻለም።",
  "admin.categories.error.catchallParent": "የተረፈ-ሁሉ ምድብ ወላጅ ሊሆን አይችልም። ሌላ መድረሻ ይምረጡ።",
  "admin.categories.error.nameRequired": "ስም ያስፈልጋል።",
  "admin.categories.error.reassignRequired": "ዝርዝሮቹ የሚሄዱበትን ይምረጡ።",
  "admin.categories.error.parentRequired": "ወላጅ ይምረጡ።",
  "admin.audit.total.capped": "10,000+",
  "admin.attributes.caption": "የባህሪ ትርጓሜዎች",
  "admin.attributes.loading": "ባህሪያት በመጫን ላይ…",
  "admin.attributes.error": "የባህሪ ቤተ-መዝገቡ ሊጫን አልቻለም።",
  "admin.attributes.empty": "በዚህ ፍለጋ የተገኘ ባህሪ የለም።",
  "admin.attributes.searchPlaceholder": "በስም ወይም በቁልፍ ይፈልጉ",
  "admin.attributes.col.name": "ባህሪ",
  "admin.attributes.col.type": "ዓይነት",
  "admin.attributes.col.options": "አማራጮች",
  "admin.attributes.export.open": "ባህርያትን ላክ",
  "admin.attributes.export.busy": "ኤክስፖርቱ በመዘጋጀት ላይ…",
  "admin.attributes.export.error": "ኤክስፖርቱ ማዘጋጀት አልተቻለም።",
  "admin.attributes.import.open": "ባህርያትን አስገባ",
  "admin.attributes.import.title": "ባህርያትን አስገባ",
  "admin.attributes.import.hint": "ያወጡትን ሁለቱን ፋይሎች ይምረጡ። ቅድመ-እይታውን እስኪያረጋግጡ ድረስ ምንም አይመዘገብም።",
  "admin.attributes.import.definitionsFile": "የትርጓሜዎች ፋይል",
  "admin.attributes.import.linksFile": "የአገናኞች ፋይል",
  /* C3-UX-8 — የአገናኞች ፋይል አማራጭ ነው፤ መንገዱ ከጅምሩ እንደፈቀደው። */
  "admin.attributes.import.optionalFile": "አማራጭ — የምድብ አገናኞች ሲቀየሩ ብቻ ይምረጡት።",
  "admin.attributes.import.preview": "ለውጦችን ቀድመው ይመልከቱ",
  "admin.attributes.import.busy": "ፋይሎቹ በመፈተሽ ላይ…",
  "admin.attributes.import.counts":
    "{adds} ተጨመሩ · {changes} ተቀየሩ · {unlinks} ተነጠሉ · {deletes} ተሰረዙ · {unchanged} አልተቀየሩም · {refusals} ተከልክለዋል",
  "admin.attributes.import.refusalRow": "መስመር {row}",
  "admin.attributes.import.confirm": "ማስገባቱን አረጋግጥ",
  "admin.attributes.import.discard": "ተወው",
  "admin.attributes.import.choose": "{file}ን ይምረጡ…",
  "admin.attributes.import.noFile": "ምንም ፋይል አልተመረጠም",
  "admin.attributes.import.discardHint": "ምንም ሳይጻፍ ይዘጋል",
  "admin.attributes.import.applied": "ማስገባቱ ተፈጽሟል — {count} ለውጦች ተጽፈዋል",
  "admin.attributes.import.close": "ዝጋ",
  "admin.attributes.import.undo": "የመጨረሻውን ማስገባት መልስ",
  "admin.attributes.import.undone": "{count} መስመሮች ተመልሰዋል።",
  "admin.attributes.import.reason.missingKey": "የባህርይ ቁልፍ የለም፦ የቁልፉ ሕዋስ ባዶ ነው",
  "admin.attributes.import.reason.duplicateKey": "ይህ ቁልፍ በፋይሉ ሁለት ጊዜ ገብቷል",
  "admin.attributes.import.reason.badAction":
    "ያልታወቀ እርምጃ “{action}” — upsert፣ unlink ወይም delete ይጠቀሙ",
  "admin.attributes.import.reason.unknownType": "ያልታወቀ ዓይነት “{attr_type}”",
  "admin.attributes.import.reason.malformedOptions": "አማራጮቹ ሊነበቡ አልቻሉም፦ “{options}”",
  "admin.attributes.import.reason.badParent":
    "የወላጅ እሴት “{detail}” በሚመሠረትበት ዝርዝር “{depends_on}” ውስጥ የለም",
  "admin.attributes.import.reason.unknownParent":
    "በ“{detail}” ላይ ይመሠረታል፤ ነገር ግን በቤተ-መጻሕፍቱም ሆነ በዚህ ፋይል ውስጥ የለም",
  "admin.attributes.import.reason.dependsNotSelect":
    "በ“{detail}” ላይ ይመሠረታል፤ ነገር ግን ነጠላ-ምርጫ ባህርይ አይደለም",
  "admin.attributes.import.reason.dependsSelf": "አንድ ባህርይ በራሱ ላይ ሊመሠረት አይችልም",
  "admin.attributes.import.reason.dependsCycle":
    "ከ“{detail}” ጋር የጥገኝነት ዑደት — ከሁለቱ አንዱ ብቻውን መቆም አለበት",

  "admin.attributes.import.reason.blastRadius":
    "አሁንም ከእነዚህ ምድቦች ጋር ተያይዟል፣ ስለዚህ ሊሰረዝ አይችልም፦ {detail}",
  "admin.attributes.import.reason.unknownCategory":
    "ያልታወቀ ምድብ፦ category_slug “{category_slug}” በዝርዝሩ የለም (መንገድ “{category_path}”)",
  "admin.attributes.import.reason.unknownAttribute":
    "ያልታወቀ ባህርይ፦ attribute_key “{attribute_key}” በቤተ-መጻሕፍቱ የለም",
  "admin.attributes.import.reason.outOfScope": "“{category_slug}” ካጣሩት ምድብ ውጭ ነው",
  "admin.attributes.import.reason.inheritedRow":
    "የተወረሰ መስመር — ለ“{category_slug}” ቀጥተኛ መስመር ይጨምሩ ወይም በ“{detail}” ላይ ያስተካክሉ",
  "admin.attributes.import.reason.badCardRank":
    "የካርድ ቦታ 1፣ 2 ወይም 3 መሆን አለበት — ይህ መስመር “{card_rank}” ይላል",
  "admin.attributes.import.reason.badDisplayOrder":
    "ቅደም ተከተል ከ0 ወይም በላይ ሙሉ ቁጥር መሆን አለበት — ይህ መስመር “{detail}” ይላል",
  "admin.attributes.import.reason.formula": "ሕዋሶች በ = + - @ መጀመር አይችሉም",
  "admin.attributes.import.reason.keyRename":
    "ቁልፎች ማንነት ናቸው፦ “{detail}”ን መልሰው አዲስ ቁልፍ ከማከል ይልቅ label_enን ይቀይሩ",
  "admin.attributes.import.reason.unknown": "ይህ መስመር ተከልክሏል",

  /* DEC-050 L2b — የv2 ቃላት፦ በሩ የሚናገረው እያንዳንዱ መታወቂያና ዝርዝር። */
  "admin.attributes.import.reason.hasDependents":
    "ሌሎች ባህሪያት በዚህ ላይ ጥገኛ ናቸው፤ ስለዚህ መሰረዝ አይቻልም፦ {detail}",
  "admin.attributes.import.reason.badOption": "በዚህ መስመር ላይ ያለ አንድ አማራጭ ተቀባይነት አላገኘም",
  "admin.attributes.import.reason.badCell": "የ“{cell}” ሕዋስ ተቀባይነት አላገኘም",
  "admin.attributes.import.reason.boundsTargetNotNumber":
    "የአማራጭ ገደቦች “{detail}”ን ያመለክታሉ፤ እሱ ግን የቁጥር ባህሪ አይደለም",
  "admin.attributes.import.reason.boundsTargetNotColinked":
    "የአማራጭ ገደቦች “{detail}”ን ያመለክታሉ፤ እሱ ግን ከተመሳሳይ ምድቦች ጋር አልተገናኘም",
  "admin.attributes.import.reason.rankInherited":
    "በ“{category}” የካርድ ቦታ {rank} ከ“{origin_category}” በተወረሰው “{origin_key}” ተይዟል",
  "admin.attributes.import.reason.required": "የ“{detail}” ሕዋስ ያስፈልጋል",
  "admin.attributes.import.reason.tooLong": "የ“{detail}” ሕዋስ በጣም ረጅም ነው",
  "admin.attributes.import.reason.badSlug": "የ“{detail}” ሕዋስ ተገቢ ቁልፍ አይደለም፦ a-z፣ 0-9፣ - እና _ ይጠቀሙ",
  "admin.attributes.import.reason.badBoolean": "የ“{detail}” ሕዋስ true ወይም false መሆን አለበት",
  "admin.attributes.import.reason.badNumber": "የ“{detail}” ሕዋስ ሙሉ ቁጥር መሆን አለበት",
  "admin.attributes.import.reason.badDate": "የ“{detail}” ሕዋስ ቀን (YYYY-MM-DD) መሆን አለበት",
  "admin.attributes.import.reason.badValue": "የ“{detail}” ሕዋስ የማይፈቀድ ዋጋ ይዟል",
  "admin.attributes.import.reason.tooManyOptions": "ይህ መስመር በጣም ብዙ አማራጮችን ይዘረዝራል",
  "admin.attributes.import.reason.optionValueTooLong": "በዚህ መስመር ያለ የአማራጭ ዋጋ በጣም ረጅም ነው",
  "admin.attributes.import.reason.optionLabelTooLong": "በዚህ መስመር ያለ የአማራጭ ስም በጣም ረጅም ነው",
  "admin.attributes.import.reason.duplicateIdentity": "ይህ መስመር ፋይሉ ቀድሞ የተጠቀመውን ማንነት ይጠራል",
  "admin.attributes.import.reason.badHeader":
    "የፋይሉ አምዶች ከዚህ ማስገባት ጋር አይመሳሰሉም — አዲስ ፋይል አውርደው እሱን ያስተካክሉ",
  "admin.attributes.import.reason.wrongFile": "ይህ ፋይል የሌላ ማስገባት ነው",
  "admin.attributes.import.reason.unknownColumn": "ፋይሉ ይህ ማስገባት የማያውቀውን አምድ ይዟል",
  "admin.attributes.import.reason.duplicateColumn": "ፋይሉ አንድ አምድ ሁለት ጊዜ ይጠራል",
  "admin.attributes.import.reason.nulByte": "ፋይሉ ተራ ጽሑፍ አይደለም",
  "admin.attributes.import.reason.emptyFile": "ፋይሉ ባዶ ነው",
  "admin.attributes.import.reason.tooManyRows": "ፋይሉ በጣም ብዙ መስመሮች አሉት",
  "admin.attributes.import.reason.badBound":
    "የ“{detail}” ሕዋስ ቁጥር ወይም ዓመት (year፣ year+5፣ year-10) መሆን አለበት",
  "admin.attributes.import.reason.badPreset":
    "የ“{detail}” ሕዋስ ከተፈቀዱት ቅርጸቶች አንዱ አይደለም (vin፣ plate-et፣ digits:8፣ alnum:3-12፣ free:200)",
  "admin.attributes.import.reason.optionKey": "አማራጭ “{cell}” ይህ ማስገባት የማያውቀውን መስክ ይዟል",
  "admin.attributes.import.reason.optionShape": "አማራጭ “{cell}” በሚጠበቀው ቅርጽ አልተዘጋጀም",
  "admin.attributes.import.reason.optionShape.activeNotBoolean":
    "አማራጭ “{cell}”፦ የበራ/የጠፋ መስክ true ወይም false መሆን አለበት",
  "admin.attributes.import.reason.optionShape.boundsNotObject":
    "አማራጭ “{cell}”፦ ገደቦቹ በመስኮች ስብስብ መልክ መሰጠት አለባቸው",
  "admin.attributes.import.reason.optionShape.aliasesNotArray":
    "አማራጭ “{cell}”፦ ሌሎቹ አጻጻፎች በዝርዝር መልክ መሰጠት አለባቸው",
  /* badCell ዝርዝሮች */
  "admin.attributes.import.reason.badCell.onlyNumber":
    "“{cell}” ለቁጥር ባህሪያት ብቻ ይሠራል (ይህ መስመር “{value}” ይላል)",
  "admin.attributes.import.reason.badCell.onlyText":
    "“{cell}” ለጽሑፍ ባህሪያት ብቻ ይሠራል (ይህ መስመር “{value}” ይላል)",
  "admin.attributes.import.reason.badCell.badLength": "“{cell}” በጣም ረጅም ነው፦ “{value}”",
  "admin.attributes.import.reason.badCell.badBound":
    "“{cell}” ቁጥር ወይም ዓመት (year፣ year+5፣ year-10) መሆን አለበት — ይህ መስመር “{value}” ይላል",
  "admin.attributes.import.reason.badCell.minAboveMax": "አነስተኛው ዋጋ ከከፍተኛው በላይ ነው፦ {value}",
  "admin.attributes.import.reason.badCell.outOfRange": "“{cell}” ከተፈቀደው ክልል ውጭ ነው፦ “{value}”",
  "admin.attributes.import.reason.badCell.yearForcesZero":
    "ዓመት ሙሉ ቁጥር ነው፤ ስለዚህ decimals 0 መሆን አለበት — ይህ መስመር “{value}” ይላል",
  "admin.attributes.import.reason.badCell.unknown": "“{cell}” “{value}”ን አይቀበልም",
  "admin.attributes.import.reason.badCell.notAllowed": "“{cell}” ከተፈቀዱት ቅርጸቶች አንዱ አይደለም፦ “{value}”",
  "admin.attributes.import.reason.badCell.notANumber": "“{cell}” ሙሉ ቁጥር መሆን አለበት፦ “{value}”",
  "admin.attributes.import.reason.badCell.tooLong": "“{cell}” በጣም ረጅም ነው ({value} ቁምፊዎች)",
  /* badOption ዝርዝሮች */
  "admin.attributes.import.reason.badOption.unknownOptionKey":
    "አማራጭ “{value}” ይህ ማስገባት የማያውቀውን መስክ ይዟል፦ “{arg}”",
  "admin.attributes.import.reason.badOption.activeNotBoolean":
    "አማራጭ “{value}”፦ የበራ/የጠፋ መስክ true ወይም false መሆን አለበት",
  "admin.attributes.import.reason.badOption.aliasesNotArray":
    "አማራጭ “{value}”፦ ሌሎቹ አጻጻፎች በዝርዝር መልክ መሰጠት አለባቸው",
  "admin.attributes.import.reason.badOption.aliasesCount":
    "አማራጭ “{value}”፦ ከአንድ እስከ አምስት ሌሎች አጻጻፎች ይፈቀዳሉ፤ {arg} አይደለም",
  "admin.attributes.import.reason.badOption.aliasNotString":
    "አማራጭ “{value}”፦ እያንዳንዱ ሌላ አጻጻፍ ጽሑፍ መሆን አለበት",
  "admin.attributes.import.reason.badOption.aliasLength":
    "አማራጭ “{value}”፦ አጻጻፉ “{arg}” በጣም ረጅም ነው (ቢበዛ 32 ቁምፊዎች)",
  "admin.attributes.import.reason.badOption.aliasControlChar":
    "አማራጭ “{value}”፦ አንድ አጻጻፍ የማይፈቀዱ ቁምፊዎችን ይዟል",
  "admin.attributes.import.reason.badOption.aliasDuplicate":
    "አማራጭ “{value}”፦ አጻጻፉ “{arg}” ሁለት ጊዜ ተዘርዝሯል",
  "admin.attributes.import.reason.badOption.boundsNotObject":
    "አማራጭ “{value}”፦ ገደቦቹ በመስኮች ስብስብ መልክ መሰጠት አለባቸው",
  "admin.attributes.import.reason.badOption.boundsUnknownKey":
    "አማራጭ “{value}”፦ የ“{arg}” ገደቦች ከmin ወይም max ውጭ መስክ ይዘዋል",
  "admin.attributes.import.reason.badOption.boundsBadValue":
    "አማራጭ “{value}”፦ ገደቡ “{arg}” ቁጥር ወይም ዓመት መሆን አለበት",
  "admin.attributes.import.reason.badOption.boundsMinAboveMax":
    "አማራጭ “{value}”፦ ለ“{arg}” አነስተኛው ገደብ ከከፍተኛው በላይ ነው",
  /* DEC-057 L2 — በበሩ ላይ የ`allowed` ቅርጽ ፍርዶች */
  "admin.attributes.import.reason.optionShape.allowedNotObject":
    "አማራጭ “{cell}”፦ የተፈቀዱ እሴቶች በባህርያት ስብስብ መልክ መሰጠት አለባቸው",
  "admin.attributes.import.reason.optionShape.allowedValuesNotArray":
    "አማራጭ “{cell}”፦ ለ“{arg}” የተፈቀዱ እሴቶች በጽሑፍ ዝርዝር መልክ መሰጠት አለባቸው",
  /* DEC-057 L2 — የበሩ የቅርጽ ፍርዶች (attr_option_shape) */
  "admin.attributes.import.reason.badOption.allowedNotObject":
    "አማራጭ “{value}”፦ የተፈቀዱ እሴቶች በባህርያት ስብስብ መልክ መሰጠት አለባቸው",
  "admin.attributes.import.reason.badOption.allowedValuesNotArray":
    "አማራጭ “{value}”፦ ለ“{arg}” የተፈቀዱ እሴቶች በዝርዝር መልክ መሰጠት አለባቸው",
  "admin.attributes.import.reason.badOption.allowedTooMany":
    "አማራጭ “{value}”፦ የተፈቀዱ እሴቶች በጣም ብዙ ናቸው — ቢበዛ አምስት ባህርያት፣ በየአንዳንዱ አምሳ እሴቶች",
  "admin.attributes.import.reason.badOption.allowedEmpty":
    "አማራጭ “{value}”፦ ለ“{arg}” የተፈቀዱ እሴቶች ባዶ ናቸው — በምትኩ ባህርዩን ያንሱ",
  "admin.attributes.import.reason.badOption.allowedDuplicate":
    "አማራጭ “{value}”፦ ለ“{arg}” የተፈቀዱ እሴቶች አንድን እሴት ሁለት ጊዜ ይዘረዝራሉ",
  /* DEC-057 L2 — የዕቅዱ ፍርዶች በዒላማው ላይ (attr_allowed_check) */
  "admin.attributes.import.reason.allowedTargetNotSelect":
    "አማራጭ “{value}” “{target}”ን ያመለክታል፤ ይህ ግን የምረጥ ባህርይ አይደለም",
  "admin.attributes.import.reason.allowedTargetNotColinked":
    "አማራጭ “{value}” “{target}”ን ያመለክታል፤ ይህ ግን ይህ ባህርይ በሚሠራበት በማንኛውም ምድብ ውስጥ አይሠራም",
  "admin.attributes.import.reason.allowedTargetCircular":
    "አማራጭ “{value}” “{target}”ን ያመለክታል፤ ይህ ግን ይህ ባህርይ ራሱ ነው ወይም በእሱ ይመሠረታል",
  "admin.attributes.import.reason.allowedUnknownValue":
    "አማራጭ “{value}” “{target}” የማያቀርበውን እሴት ይዘረዝራል",

  "admin.attributes.import.guidance":
    "“(read-only)” የተባሉ አምዶች በራስ-ሰር ይሰላሉ፤ በፋይሉ ውስጥ ቢቀይሯቸውም ተግባራዊ አይሆኑም።",
  "admin.attributes.import.ignored": "ችላ የተባሉ (read-only)",
  "admin.attributes.import.ignoredRow": "መስመር {row} — {column} ተግባራዊ አልሆነም",
  "admin.attributes.import.ignoredColumn": "{column} — {count} መስመሮች ተግባራዊ አልሆኑም",
  "admin.attributes.import.error.denied": "ባህርያትን ለማስገባት ፈቃድ የለዎትም።",
  "admin.attributes.import.error.stepUp": "ኮድ አረጋግጠው እንደገና ይሞክሩ።",
  "admin.attributes.import.error.fileChanged": "ከቅድመ-እይታው በኋላ ፋይሉ ተቀይሯል። እንደገና ይመልከቱ።",
  "admin.attributes.import.error.busy": "ሌላ ማስገባት አስቀድሞ በሂደት ላይ ነው።",
  "admin.attributes.import.error.tooLarge": "ያ ፋይል ከ1 ሜባ ይበልጣል።",
  "admin.attributes.import.error.tooManyRows": "ያ ፋይል ከ5,000 መስመሮች ይበልጣል።",
  "admin.attributes.import.error.badHeader": "የአምድ ርዕሶቹ ከኤክስፖርቱ ጋር አይዛመዱም።",
  "admin.attributes.import.error.emptyFile": "ያ ፋይል ባዶ ነው።",
  "admin.attributes.import.error.wrongFile": "ይህ የምድቦች ፋይል ነው — ከምድቦች ገጽ ያስገቡት።",
  "admin.attributes.import.error.failed": "ማስገባቱ ሊጠናቀቅ አልቻለም።",
  "admin.attributes.import.error.reason": "ምክንያት፦ {reason}",
  "admin.roles.perm.action.import": "ማስገባት",
  "admin.attributes.inherited.badge": "ከ{origin} የተወረሰ",
  "admin.attributes.inherited.openOrigin": "የመነሻ ምድብ ክፈት",
  "admin.attributes.col.usage": "የሚጠቀሙት",
  "admin.attributes.type.text": "ጽሑፍ",
  "admin.attributes.type.number": "ቁጥር",
  "admin.attributes.type.boolean": "አዎ / አይደለም",
  "admin.attributes.type.single_select": "አንድ ምርጫ",
  "admin.attributes.type.multi_select": "ብዙ ምርጫ",
  "admin.attributes.type.date": "ቀን",
  "admin.attributes.type.range": "ክልል",
  "admin.attributes.create.open": "አዲስ ባህሪ",
  "admin.attributes.create.title": "አዲስ ባህሪ",
  "admin.attributes.edit.title": "ባህሪን አርትዕ",
  "admin.attributes.field.key": "ቁልፍ",
  "admin.attributes.field.keyHelp": "የማሽን ማንነት፦ snake_case፣ አይለወጥም፣ በወጪና በኮድ ያገለግላል",
  "admin.attributes.field.name": "ስም",
  "admin.attributes.field.nameHelp": "ሰዎች የሚያዩት፤ ሊተረገም ይችላል",
  "admin.attributes.field.type": "ዓይነት",
  "admin.attributes.field.options": "አማራጮች",
  "admin.attributes.field.optionsHelp": "በአንድ መስመር አንድ አማራጭ።",
  /* DEC-050 L3b — የአማራጭ መስመር አርታኢ (INC-188)። */
  "admin.attributes.options.rowsLabel": "አማራጮች",
  "admin.attributes.options.rowsHelp":
    "እያንዳንዱ አማራጭ የራሱን ስሞች፣ ተለዋጭ ስሞችና ገደቦች ይይዛል። የተቀመጠ አማራጭ ይጠፋል ሳይሆን ይዘጋል።",
  "admin.attributes.options.value": "እሴት",
  "admin.attributes.options.valuePlaceholder": "የማሽን እሴት",
  "admin.attributes.options.labelEn": "ስም (እንግሊዝኛ)",
  "admin.attributes.options.labelAm": "ስም (አማርኛ)",
  "admin.attributes.options.active": "በማስታወቂያ ጊዜ ይቀርባል",
  "admin.attributes.options.inactiveTag": "ዝግ",
  "admin.attributes.options.addRow": "አማራጭ ጨምር",
  "admin.attributes.options.removeRow": "አስወግድ",
  "admin.attributes.options.empty": "እስካሁን አማራጮች የሉም።",
  "admin.attributes.options.aliasesHelp": "እስከ አምስት የፍለጋ ተለዋጭ ስሞች፣ እያንዳንዱ 32 ቁምፊ።",
  "admin.attributes.options.aliasPlaceholder": "ተለዋጭ ስም",
  "admin.attributes.options.boundsHelp": "ገደቡ ይህ ባህርይና የቁጥር ባህርዩ አብረው በሚሠሩበት ቦታ ሁሉ ተግባራዊ ይሆናል።",
  "admin.attributes.options.boundsAdd": "ገደብ ጨምር",
  "admin.attributes.options.boundsAddExample": "ቁጥርን ገድብ (ለምሳሌ፣ የሞዴል ዓመታት 2008–2026)",
  "admin.attributes.options.boundsMin": "አነስተኛ",
  "admin.attributes.options.boundsMax": "ከፍተኛ",
  "admin.attributes.options.boundsRemove": "ገደብ አስወግድ",
  /* DEC-057 L3 — the allowed-values picker. */
  "admin.attributes.options.allowedHelp": "ገደቡ ይህ ባህርይና የዝርዝር ባህርዩ አብረው በሚሠሩበት ቦታ ሁሉ ተግባራዊ ይሆናል።",
  "admin.attributes.options.allowedAdd": "የተፈቀዱ ዋጋዎች መለያ ጨምር",
  "admin.attributes.options.allowedAddExample": "ዝርዝርን ገድብ (ለምሳሌ፣ ማከማቻ፦ 64 / 128 / 256 GB)",
  "admin.attributes.options.allowedRemove": "የተፈቀዱ ዋጋዎችን አስወግድ",
  "admin.attributes.options.allowedNoValues": "ይህ መለያ እስካሁን ዋጋዎች የለውም።",
  /* C3-UX-7 — ረጅም የአማራጭ ዝርዝርን መዘዋወር። */
  "admin.attributes.options.search": "አማራጭ ፈልግ",
  "admin.attributes.options.searchPlaceholder": "ዋጋ ወይም መለያ ስም",
  "admin.attributes.options.parentFilter": "የትኞቹን አማራጮች አሳይ",
  "admin.attributes.options.parentFilterAll": "ሁሉም የወላጅ ዋጋዎች",
  "admin.attributes.options.countLine": "ከ{total} ውስጥ {shown}",
  /* INC-195 — ዋጋ የሌላቸው የአማራጭ መስመሮች ከበሩ በፊት እዚህ ይከለከላሉ። */
  "admin.attributes.options.blankRows": "{count} አማራጮች ዋጋ የላቸውም — ይሙሉ ወይም ያስወግዱ",
  /* C3-UX-9 — ገደብ ወይም ክልከላ መያዝ የሚችሉት አብረው የሚሠሩ ባህርያት ብቻ ናቸው። */
  "admin.attributes.options.boundsNoTargets":
    "ከዚህ ጋር አብሮ የሚሠራ የቁጥር ባህርይ የለም፤ ስለዚህ የሚገደብ ምንም አይኖርም።",
  "admin.attributes.options.allowedNoTargets":
    "ከዚህ ጋር አብሮ የሚሠራ የዝርዝር ባህርይ የለም፤ ስለዚህ የሚከለከል ምንም አይኖርም።",

  "admin.attributes.field.help": "የእገዛ ጽሑፍ",
  /* DEC-050 L3a — የv2 መስኮች፤ የበሩ ፍተሻዎች ወሳኝ ናቸው። */
  "admin.attributes.field.helpAm": "የእገዛ ጽሑፍ (አማርኛ)",
  "admin.attributes.field.helpCounter": "{count}/240",
  "admin.attributes.field.numberGroup": "የቁጥር ቅንብሮች",
  "admin.attributes.field.unit": "መለኪያ",
  "admin.attributes.field.unitHelp": "ከእሴቱ በኋላ ይታያል፤ ከ16 ቁምፊ አይበልጥም (ኪሜ፣ ጊባ፣ ካሬ ሜትር)።",
  "admin.attributes.field.min": "አነስተኛ",
  "admin.attributes.field.max": "ከፍተኛ",
  "admin.attributes.field.boundHelp": "ቁጥር፣ ወይም የዓመት ቶከን፦ year፣ year+1፣ year-1።",
  "admin.attributes.field.format": "ቅርጸት",
  "admin.attributes.field.formatHelp": "ተራ ቁጥሮች ወይም ዓመት።",
  "admin.attributes.format.none": "አልተቀመጠም",
  "admin.attributes.format.plain": "ተራ",
  "admin.attributes.format.year": "ዓመት",
  "admin.attributes.field.decimals": "የአሥርዮሽ ቦታዎች",
  "admin.attributes.field.decimalsYear": "ዓመት 0 የአሥርዮሽ ቦታ ይይዛል።",
  "admin.attributes.decimals.none": "አልተቀመጠም",
  "admin.attributes.field.textGroup": "የጽሑፍ ቅንብሮች",
  "admin.attributes.field.preset": "የተፈቀደ ቅርጽ",
  "admin.attributes.field.presetHelp": "የሚጻፈውን ይገድቡ፤ ነጻ ጽሑፍ ከፈለጉ ይተዉት።",
  "admin.attributes.preset.none": "አልተቀመጠም",
  "admin.attributes.preset.digits": "አሃዞች",
  "admin.attributes.preset.vin": "VIN",
  "admin.attributes.preset.plateEt": "የኢትዮጵያ ታርጋ",
  "admin.attributes.preset.alnum": "ፊደሎችና አሃዞች",
  "admin.attributes.preset.free": "ነጻ ጽሑፍ",
  "admin.attributes.field.presetN": "የቁምፊ ብዛት",
  "admin.attributes.field.presetA": "አጭሩ ርዝመት",
  "admin.attributes.field.presetB": "ረጅሙ ርዝመት",
  "admin.attributes.field.maxLength": "ከፍተኛ ርዝመት",
  "admin.attributes.field.maxLengthHelp": "ከ1 እስከ 1000 ቁምፊ።",
  "admin.attributes.col.coverage": "የአማርኛ አማራጮች",
  "admin.attributes.coverage.tooltip": "ከ{total} አማራጮች {count} የአማርኛ ስም አላቸው",
  /* DEC-045 — ጥገኛ አማራጮች። */
  "admin.attributes.field.dependsOn": "የሚመሠረተው በ",
  "admin.attributes.field.dependsOnHelp":
    "አንድ-ምርጫ ባህርይ ይምረጡ፤ የዚህ ባህርይ አማራጮች ከዚያ ባህርይ እሴቶች በታች ይዘረዘራሉ።",
  "admin.attributes.dependsOn.none": "ጥገኝነት የለም",
  "admin.attributes.dependsOn.parentEmpty": "የተመረጠው ባህርይ እስካሁን አማራጮች የሉትም።",
  "admin.attributes.dependsOn.optionsFor": "የ{parent} አማራጮች",
  "admin.attributes.dependsOn.previewLabel": "ቅደም ተከተሉን ይመልከቱ",
  "admin.attributes.dependsOn.previewNone": "እሴት ይምረጡ",
  "admin.attributes.dependsOn.previewPrompt": "{parent}፡ የ{child} አማራጮችን ለመመልከት እሴት ይምረጡ",
  "admin.attributes.dependsOn.previewResults": "የ{child} አማራጮች",

  "admin.attributes.action.edit": "አርትዕ",
  "admin.attributes.action.delete": "ሰርዝ",
  "admin.attributes.action.assign": "ወደ ምድብ መድብ",
  "admin.attributes.filter.category": "በምድብ አጣራ",
  "admin.attributes.filter.allCategories": "ሁሉም ምድቦች",
  "admin.attributes.filter.clear": "ማጣሪያውን አጽዳ",
  "admin.attributes.filter.empty": "እስካሁን ከዚህ ምድብ ጋር የተገናኘ ባህሪ የለም።",
  "admin.attributes.assign.title": "ወደ ምድብ መድብ",
  "admin.attributes.assign.hint": "ባህሪው ከተመረጠው ምድብ ጋር ይገናኛል፤ በመነሻው አማራጭ እና የሚጣራ ይሆናል።",
  "admin.attributes.assign.category": "ምድብ",
  "admin.attributes.assign.pickNone": "ምድብ ይምረጡ",
  "admin.attributes.assign.searchPlaceholder": "ምድቦችን ይፈልጉ",
  "admin.attributes.error.assignNoCategory": "መጀመሪያ ምድብ ይምረጡ።",
  "admin.attributes.action.menu": "እርምጃዎች",
  "admin.attributes.action.remove": "ከምድብ አስወግድ",
  "admin.attributes.remove.title": "ከምድብ ማስወገድ",
  "admin.attributes.remove.hint": "ባህሪው በቤተ-መዝገቡ ይቀራል፤ ይህ ምድብ ብቻ መጠየቁን ያቆማል።",
  "admin.attributes.remove.category": "ምድብ",
  "admin.attributes.remove.pickNone": "ምድብ ይምረጡ",
  "admin.attributes.remove.none": "ይህ ባህሪ እስካሁን ከየትኛውም ምድብ ጋር አልተያያዘም።",
  "admin.attributes.remove.confirm": "{attribute} ከዚህ በኋላ በ{category} ውስጥ አይጠየቅም።",
  "admin.attributes.error.removeNoCategory": "መጀመሪያ ምድብ ይምረጡ።",
  "admin.attributes.usage.none": "እስካሁን በየትኛውም ምድብ አልተጠቀመም።",
  "admin.attributes.usage.count": "በ{count} ምድቦች ጥቅም ላይ ውሏል",
  "admin.attributes.action.merge": "አዋህድ",
  "admin.attributes.delete.title": "ባህሪን ሰርዝ",
  "admin.attributes.delete.hint": "ትርጓሜው ከቤተ-መዝገቡ ይጠፋል። የሚጠቀሙበት ምድቦች መጀመሪያ መነጠል አለባቸው።",
  "admin.attributes.delete.blastRadius": "{count} ምድቦች አሁንም ይህን ባህሪ ይጠቀማሉ።",
  "admin.attributes.delete.confirmLabel": "ለማረጋገጥ {key} ይተይቡ",
  "admin.attributes.merge.title": "ባህሪያትን አዋህድ",
  "admin.attributes.merge.hint": "የተመረጡት ድግግሞሾች አገናኞች ወደ ቀሪው ባህሪ ይዛወራሉ፤ ከዚያም ድግግሞሾቹ ይወገዳሉ።",
  "admin.attributes.merge.target": "ይህን ባህሪ አቆይ",
  "admin.attributes.merge.targetNone": "ቀሪውን ባህሪ ይምረጡ",
  "admin.attributes.merge.sources": "የሚዋሃዱ ድግግሞሾች",
  "admin.attributes.merge.confirm": "{n} አገናኞች ወደ {target} ይዛወራሉ፤ {m} ትርጓሜዎች ይወገዳሉ።",
  "admin.attributes.merge.result":
    "{moved} አገናኞች ተዛወሩ · {folded} ወደ ነባር አገናኝ ተጣመሩ · {removed} ትርጓሜዎች ተወገዱ።",
  "admin.attributes.links.error": "የተገናኙት ባህሪያት ሊጫኑ አልቻሉም።",
  "admin.attributes.links.empty": "እስካሁን ከዚህ ምድብ ጋር የተገናኘ ባህሪ የለም።",
  "admin.attributes.links.required": "ግዴታ",
  "admin.attributes.links.filterable": "የሚጣራ",
  "admin.attributes.links.saved": "ተቀምጧል",
  "admin.attributes.links.order": "ቅደም ተከተል {order}",
  "admin.attributes.links.done": "ተጨርሷል",
  "admin.attributes.links.autosave": "ለውጦች ወዲያውኑ ይቀመጣሉ",
  "admin.attributes.links.unlink": "አላቅቅ",
  "admin.attributes.links.add": "ባህሪ ጨምር",
  "admin.attributes.links.pickNone": "ባህሪ ይምረጡ",
  "admin.attributes.links.searchPlaceholder": "ቤተ-መዝገቡን ይፈልጉ",
  "admin.attributes.card.caption": "ካርዱ ያሳያል፦ {list}",
  "admin.attributes.card.none": "እስካሁን በዝርዝር ካርዱ ላይ የሚታይ ባህሪ የለም።",
  "admin.attributes.card.rank": "በካርድ ላይ ({rank})",
  "admin.attributes.card.rankNone": "በካርድ ላይ አሳይ",
  "admin.attributes.error.keyRequired": "ቁልፍ ያስፈልጋል።",
  "admin.attributes.error.nameRequired": "ስም ያስፈልጋል።",
  "admin.attributes.error.keyTaken": "ሌላ ባህሪ ይህን ቁልፍ ተጠቅሞበታል።",
  "admin.attributes.error.notFound": "ያ ባህሪ ከእንግዲህ የለም።",
  "admin.attributes.error.confirmMismatch": "የተተየበው ቁልፍ አይዛመድም።",
  "admin.attributes.error.deleteHasLinks": "{count} ምድቦች አሁንም ይህን ባህሪ ይጠቀማሉ። መጀመሪያ ከሁሉም ያላቅቁት።",
  "admin.attributes.error.inUse": "{count} ምድቦች አሁንም ይህን ባህሪ ይጠቀማሉ። መጀመሪያ ከሁሉም ያላቅቁት።",
  "admin.attributes.error.alreadyLinked": "ይህ ምድብ ያንን ባህሪ አስቀድሞ አገናኝቷል።",
  "admin.attributes.error.linkNotFound": "ያ አገናኝ ከእንግዲህ የለም።",
  "admin.attributes.error.cardLimit": "የዝርዝር ካርድ ቢበዛ ሦስት ባህሪያትን ያሳያል።",
  /* DEC-050 L2a/L2b — በሩ በተወረሰ ካርድ የተያዘውን ቦታ አይቀበልም። */
  "admin.attributes.error.rankInherited": "ይህ የካርድ ቦታ ከወላጅ ምድብ በተወረሰ ባህሪ ተይዟል። ሌላ ቦታ ይምረጡ።",
  "admin.attributes.error.orderMismatch": "የባህሪው ቅደም ተከተል ጊዜው አልፎበታል፤ እንደገና ከፍተው ይሞክሩ።",
  "admin.attributes.error.mergeNoTarget": "የሚቆየውን ባህሪ ይምረጡ።",
  "admin.attributes.error.mergeNoSources": "ቢያንስ አንድ ድግግሞሽ ይምረጡ።",
  "admin.attributes.error.mergeTargetInSources": "የሚቆየው ባህሪ በተመሳሳይ ጊዜ ድግግሞሽ ሊሆን አይችልም።",
  "admin.attributes.error.saveFailed": "ለውጡ ሊቀመጥ አልቻለም።",
  /* DEC-045 — የጥገኝነት እምቢታዎች። */
  "admin.attributes.error.dependsUnknown": "“{detail}” በሚል ቁልፍ ባህርይ የለም።",
  "admin.attributes.error.dependsNotSelect": "“{detail}” አንድ-ምርጫ ባህርይ ስላልሆነ በእሱ መመሥረት አይቻልም።",
  "admin.attributes.error.dependsSelf": "ባህርይ በራሱ ላይ መመሥረት አይችልም።",
  "admin.attributes.error.dependsCycle": "“{detail}” አስቀድሞ በዚህ ባህርይ ላይ ስለሚመሠረት ዙር ይፈጥራል።",
  "admin.attributes.error.parentWithoutDepends":
    "“{detail}” የወላጅ እሴት ይጠቅሳል፣ ነገር ግን የሚመሠረትበት ባህርይ አልተመረጠም።",
  "admin.attributes.error.parentNotInParent": "“{detail}” የሚመሠረትበት ባህርይ እሴቶች አንዱ አይደለም።",
  /* DEC-050 L3b — በበሩ የሚነሱ የአማራጭና የቅንብር እምቢታዎች። */
  "admin.attributes.error.badOption": "አንድ አማራጭ ተቀባይነት አላገኘም፦ {detail}",
  "admin.attributes.error.badCell": "አንድ ቅንብር ተቀባይነት አላገኘም፦ {detail}",
  "admin.attributes.error.boundsTargetNotNumber": "“{target}” የቁጥር ባህርይ አይደለም፤ ገደብ ማድረግ አይቻልም።",
  "admin.attributes.error.boundsTargetNotColinked":
    "“{target}” ይህ ባህርይ በሚሠራበት በማንኛውም ምድብ ውስጥ ስለማይሠራ፣ በዚህ ቦታ ገደብ ማድረግ አይቻልም።",
  /* DEC-057 L2 — በአማራጭ የተመሠረቱ የተፈቀዱ እሴቶች የበሩ ፍርዶች */
  "admin.attributes.error.allowedTargetNotSelect": "“{target}” የምረጥ ባህርይ ስላልሆነ እሴቶቹን መገደብ አይቻልም።",
  "admin.attributes.error.allowedTargetNotColinked":
    "“{target}” ይህ ባህርይ በሚሠራበት በማንኛውም ምድብ ውስጥ ስለማይሠራ፣ በዚህ ቦታ እሴቶቹን መገደብ አይቻልም።",
  "admin.attributes.error.allowedTargetCircular":
    "“{target}” ይህ ባህርይ ራሱ ነው ወይም በእሱ ስለሚመሠረት፣ እሴቶቹን መገደብ አይቻልም።",
  "admin.attributes.error.allowedUnknownValue":
    "ከተፈቀዱት እሴቶች አንዱ (“{target}”) በሚያመለክተው ባህርይ አይቀርብም።",

  "admin.attributes.error.deleteHasDependents": "እነዚህ ባህርያት በእሱ ስለሚመሠረቱ ሊሰረዝ አይችልም፦ {detail}",
  "admin.attributes.error.unlinkHasDependents": "በዚህ ምድብ ውስጥ እነዚህ ባህርያት በእሱ ይመሠረታሉ፦ {detail}",
  "admin.attributes.error.mergeAcrossDependency": "ጥገኝነት ስለሚያስተሳስራቸው ሊዋሃዱ አይችሉም፦ {detail}",
  "admin.categories.action.attributes": "ባህሪያት",
  "admin.categories.col.attributes": "ባህሪያት",
  "admin.categories.badge.needsCard": "የካርድ ባህሪያት ያስፈልጋሉ",
  "admin.categories.tip.needsCard": "ዝርዝሮችን የሚቀበል ንቁ ምድብ በዝርዝር ካርዶቹ ላይ ቢያንስ ሁለት ባህሪያት ማሳየት አለበት።",
  "admin.categories.flag.needsCard": "በዝርዝር ካርዶች ላይ የሚታዩ ቢያንስ ሁለት ባህሪያት ይምረጡ።",
  "admin.categories.parent.primary": "ዋና",
  "admin.categories.parent.also": "በተጨማሪ በ፦ {list}",
  "admin.locations.caption": "በተመረጠው ገበያ ውስጥ ያሉ ቦታዎች",
  "admin.locations.loading": "ቦታዎች እየተጫኑ ነው…",
  "admin.locations.error": "ቦታዎቹን መጫን አልተቻለም።",
  "admin.locations.empty": "በእነዚህ ማጣሪያዎች የሚስማማ ቦታ የለም።",
  "admin.locations.searchPlaceholder": "በስም፣ በአድራሻ ወይም በተለዋጭ ስም ፈልግ",
  "admin.locations.filter.country": "አገር",
  "admin.locations.filter.search": "ቦታዎችን ፈልግ",
  "admin.locations.filter.allCountriesOption": "አገር፦ ሁሉም አገሮች",
  "admin.locations.filter.allLevelsOption": "ደረጃ፦ ሁሉም ደረጃዎች",
  "admin.locations.filter.allStatusesOption": "ሁኔታ፦ ገባሪና የተቋረጡ",
  "admin.locations.filter.closedSuffix": "ዝግ",
  "admin.locations.filter.level": "ደረጃ",
  "admin.locations.filter.status": "ሁኔታ",
  "admin.locations.filter.activeOnly": "ገባሪ ብቻ",
  "admin.locations.filter.retiredOnly": "የተቋረጡ ብቻ",
  "admin.locations.filter.pageSize": "በአንድ ገጽ ያሉ ረድፎች",
  "admin.locations.level.country": "አገር",
  "admin.locations.level.region": "ክልል",
  "admin.locations.level.city": "ከተማ",
  "admin.locations.level.sub_city": "ክፍለ ከተማ",
  "admin.locations.badge.active": "ገባሪ",
  "admin.locations.badge.retired": "የተቋረጠ",
  "admin.locations.tip.active": "ንቁ — በመራጮችና በዝርዝር ፍሰቶች ውስጥ ይቀርባል።",
  "admin.locations.tip.retired": "ጡረታ የወጣ — የተደበቀ፣ ዝርዝሮች አሁንም ሊያመለክቱት ስለሚችሉ የተቀመጠ።",
  "admin.locations.market.open": "{country} — ክፍት ገበያ",
  "admin.locations.market.closed": "{country} — ዝግ ገበያ፦ እስኪከፈት ድረስ ቦታዎቹ ተደብቀው ይቆያሉ",
  "admin.locations.col.name": "ስም",
  "admin.locations.col.path": "መንገድ",
  "admin.locations.col.status": "ሁኔታ",
  "admin.locations.col.counts": "ዝርዝሮች · ሽፋን · ነባሪዎች",
  "admin.locations.col.order": "ቅደም ተከተል",
  "admin.locations.col.aliases": "ተለዋጭ ስሞች",
  "admin.locations.col.iso": "ISO",
  "admin.locations.counts.hint": "እዚህ የተለጠፉ ዝርዝሮች · የሻጭ ሽፋን ረድፎች · እዚህ በነባሪ የሚለጥፉ መገለጫዎች",
  "admin.locations.action.createChild": "ውስጡ ቦታ ጨምር",
  "admin.locations.action.edit": "አስተካክል",
  "admin.locations.action.activate": "አግብር",
  "admin.locations.action.retire": "አቋርጥ",
  "admin.locations.action.move": "አንቀሳቅስ",
  "admin.locations.action.reorder": "የአጎራባቾችን ቅደም ተከተል ቀይር",
  "admin.locations.action.delete": "አጥፋ",
  "admin.locations.action.floor": "ክፍለ ከተማ የመጨረሻው ደረጃ ነው።",
  "admin.locations.action.anchorFixed": "የአገር መልሕቅ ማንቀሳቀስ አይቻልም።",
  "admin.locations.action.deleteBlocked": "ይህን ቦታ አጥፋ ሳይሆን አቋርጥ — አንድ ነገር አሁንም ያመለክተዋል።",
  "admin.locations.create.open": "ቦታ ጨምር",
  "admin.locations.create.rootHint": "በመጀመሪያ ቦታ ምረጥ — አዲስ ቦታ ሁሌም በነባር ቦታ ውስጥ ይፈጠራል።",
  "admin.locations.create.title": "ውስጡ ቦታ ጨምር",
  "admin.locations.create.parent": "ውስጥ",
  "admin.locations.create.parentCountry": "{country} — መላው አገር",
  "admin.locations.create.level": "ደረጃ፦ {level}",
  "admin.locations.create.slugPreview": "መለያ (በአገልጋዩ የሚሠራ)፦",
  "admin.locations.create.bornRetired": "ተቋርጦ ተፈጥሯል — ዝግጁ ሲሆን አግብረው።",
  "admin.locations.edit.title": "ቦታ አስተካክል",
  "admin.locations.edit.translationsLink": "በሌሎች ቋንቋዎች ያሉ ስሞች በትርጉሞች → ዳታ ይስተካከላሉ።",
  "admin.locations.active.titleActivate": "ይህን ቦታ አግብር",
  "admin.locations.active.titleRetire": "ይህን ቦታ አቋርጥ",
  "admin.locations.active.hintActivate": "ከበላዩ ያሉት ሁሉ ገባሪ እንደሆኑ ወዲያውኑ በአሰሳ ላይ ይታያል።",
  "admin.locations.active.hintRetire": "እሱና ውስጡ ያሉት ከአሰሳ ይሰወራሉ። ምንም አይጠፋም።",
  "admin.locations.active.confirmActivate": "አግብር",
  "admin.locations.active.confirmRetire": "አቋርጥ",
  "admin.locations.move.title": "ይህን ቦታ አንቀሳቅስ",
  "admin.locations.move.parent": "አዲስ ወላጅ",
  "admin.locations.move.hint": "በተመሳሳይ ገበያ ውስጥ ካንድ ደረጃ በላይ ያሉ ቦታዎች ብቻ ሊይዙት ይችላሉ።",
  "admin.locations.move.noTargets": "በዚህ ገበያ ውስጥ ሊይዘው የሚችል ቦታ የለም።",
  "admin.locations.move.submit": "አንቀሳቅስ",
  "admin.locations.reorder.title": "የአጎራባቾችን ቅደም ተከተል ቀይር",
  "admin.locations.reorder.hint": "ከታች ያለው ቅደም ተከተል ገዢዎች የሚያዩት ነው።",
  "admin.locations.reorder.up": "ወደ ላይ አንቀሳቅስ",
  "admin.locations.reorder.down": "ወደ ታች አንቀሳቅስ",
  "admin.locations.reorder.submit": "ቅደም ተከተሉን አስቀምጥ",
  "admin.locations.delete.title": "ይህን ቦታ አጥፋ",
  "admin.locations.delete.hint": "ማጥፋት ዘላቂ ነው። ለማረጋገጥ የቦታውን አድራሻ ጻፍ።",
  "admin.locations.delete.confirmLabel": "አድራሻ",
  "admin.locations.delete.confirm": "አጥፋ",
  "admin.locations.delete.mismatch": "ይህ የዚህ ቦታ አድራሻ አይደለም።",
  "admin.locations.field.name": "ስም (እንግሊዝኛ)",
  "admin.locations.field.slug": "መለያ",
  "admin.locations.field.slugHint": "ባዶ ተው — አገልጋዩ ከስሙ ይሠራዋል።",
  "admin.locations.field.iso": "የISO 3166-2 ኮድ",
  "admin.locations.field.centerLat": "የመሃል ኬክሮስ",
  "admin.locations.field.centerLng": "የመሃል ኬንትሮስ",
  "admin.locations.field.centerRequired": "በዚህ ደረጃ ሁለቱም ቁጥሮች ያስፈልጋሉ።",
  "admin.locations.field.centerOptional": "በዚህ ደረጃ አማራጭ ነው።",
  "admin.locations.field.aliases": "ሰዎች የሚፈልጉባቸው ሌሎች ስሞች",
  "admin.locations.field.aliasesHint": "ከእነዚህ አንዱን መፈለግ ይህን ቦታ ያገኘዋል።",
  "admin.locations.field.aliasAdd": "ጨምር",
  "admin.locations.field.aliasRemove": "ይህን ስም አስወግድ",
  "admin.locations.field.order": "ቅደም ተከተል",
  "admin.locations.field.readOnlyLevel": "ደረጃ",
  "admin.locations.transfer.scope": "ለ{country}",
  "admin.locations.export.countries": "አገሮችን አውርድ",
  "admin.locations.export.countriesHint":
    "በአንድ አገር አንድ ረድፍ፦ ክፍት ወይም ዝግ፣ የመለኪያ ሥርዓት፣ ገንዘብ፣ የሐዲድ ቅደም ተከተል።",
  "admin.locations.export.locations": "ቦታዎችን አውርድ",
  "admin.locations.export.locationsHint":
    "የተመረጠው አገር የቦታ ዛፍ፦ ክልሎች፣ ከተሞች፣ ክፍለ ከተሞች፣ ተለዋጭ ስሞች፣ መጋጠሚያዎች።",
  "admin.locations.export.busy": "ፋይሉ እየተዘጋጀ ነው…",
  "admin.locations.export.error": "ያ ፋይል መዘጋጀት አልቻለም።",
  "admin.locations.error.reason": "ምክንያት፦ {reason}",
  "admin.locations.error.denied": "ይህን ለማድረግ ፈቃድ የለህም።",
  "admin.locations.error.stepUp": "ኮድ አረጋግጠህ እንደገና ሞክር።",
  "admin.locations.error.unknown": "ማስቀመጥ አልተቻለም።",
  "admin.locations.error.locationMissing": "ያ ቦታ ከዚህ በኋላ የለም።",
  "admin.locations.error.parentMissing": "ውስጡ መቀመጥ የነበረበት ቦታ ከዚህ በኋላ የለም።",
  "admin.locations.error.parentInactive": "በመጀመሪያ ከበላዩ ያለውን ቦታ አግብር።",
  "admin.locations.error.levelMismatch": "ቦታ ከያዘው ካንድ ደረጃ በታች ብቻ ሊቀመጥ ይችላል።",
  "admin.locations.error.crossCountry": "ቦታ ወደ ሌላ ገበያ መሄድ አይችልም።",
  "admin.locations.error.badSlug": "ያ አድራሻ ጥቅም ላይ የሚውል አይደለም።",
  "admin.locations.error.missingCoordinates": "ከተማም ክፍለ ከተማም የመሃል ኬክሮስና ኬንትሮስ ያስፈልጋቸዋል።",
  "admin.locations.error.rootMustBeCountry": "በላይ የሚቀመጠው የገበያ መልሕቅ ብቻ ነው።",
  "admin.locations.error.badLevel": "ያ ደረጃ ከአገር፣ ክልል፣ ከተማ ወይም ክፍለ ከተማ አንዱ አይደለም።",
  "admin.locations.error.isoOnRegionsOnly": "የISO 3166-2 ኮድ የሚይዘው ክልል ብቻ ነው።",
  "admin.locations.error.nameRequired": "ስም ያስፈልጋል።",
  "admin.locations.error.unknownCountry": "ያ ገበያ በመዝገብ ላይ የለም።",
  "admin.locations.error.useMoveDoor": "ቦታ የሚቀመጥበትን ለመቀየር አንቀሳቅስን ተጠቀም።",
  "admin.locations.error.cannotMoveCountry": "የገበያ መልሕቅ ማንቀሳቀስ አይቻልም።",
  "admin.locations.error.orderCountriesInProfile": "ገበያዎች የሚደረደሩት በራሳቸው ቅንብር ውስጥ ነው፣ እዚህ አይደለም።",
  "admin.locations.error.notAChild": "እነዚያ ቦታዎች በሙሉ በአንድ ውስጥ አይቀመጡም።",
  "admin.locations.error.slugMismatch": "የጻፍከው አድራሻ አይመሳሰልም።",
  "admin.locations.error.retireInstead": "ይህን ቦታ ከማጥፋት ይልቅ አቋርጠው።",
  "admin.locations.error.detail.hasChildren": "ቦታዎች ውስጡ ይቀመጣሉ",
  "admin.locations.error.detail.hasListings": "ዝርዝሮች እዚህ ተለጥፈዋል",
  "admin.locations.error.detail.hasCoverage": "ሻጮች ይሸፍኑታል",
  "admin.locations.error.detail.hasProfileDefaults": "ሰዎች በነባሪ እዚህ ይለጥፋሉ",
  "admin.locations.import.openPlaces": "ቦታዎችን አስገባ",
  "admin.locations.import.title": "ገበያዎችንና ቦታዎችን አስገባ",
  "admin.locations.import.titleScoped": "ወደ {country} አስገባ",
  "admin.locations.import.hint": "ፋይል ሙሉ ለሙሉ ወይም በጭራሽ ይጻፋል፦ አንድ የተከለከለ ረድፍ ሙሉውን ማስገባት ያቆማል።",
  "admin.locations.import.guidance": "አምዶቹ እንዲስማሙ ከወረደ ፋይል ጀምር። ለንባብ ብቻ የሆኑ አምዶች በጭራሽ አይተገበሩም።",
  "admin.locations.import.countriesFile": "የገበያዎች ፋይል",
  "admin.locations.import.locationsFile": "የቦታዎች ፋይል",
  "admin.locations.import.optionalFile": "አማራጭ — አንዱ ፋይል በራሱ ሙሉ ሥራ ነው።",
  "admin.locations.import.preview": "ለውጦችን ቅድመ እይታ",
  "admin.locations.import.busy": "ፋይሉ እየተመረመረ ነው…",
  "admin.locations.import.counts":
    "{adds} ተጨመሩ · {changes} ተቀየሩ · {retires} ተቋረጡ · {reactivations} እንደገና ገቡ · {deletes} ጠፉ · {unchanged} አልተቀየሩም · {refusals} ተከልክለዋል",
  "admin.locations.import.refusalRow": "ረድፍ {row}",
  "admin.locations.import.confirm": "ማስገባትን አረጋግጥ",
  "admin.locations.import.discard": "ጣል",
  "admin.locations.import.discardHint": "ምንም ሳይጻፍ ዝጋ",
  "admin.locations.import.choose": "{file} ምረጥ…",
  "admin.locations.import.noFile": "ፋይል አልተመረጠም",
  "admin.locations.import.applied": "ማስገባት ተፈጻሚ ሆነ — {count} ለውጦች ተጻፉ",
  "admin.locations.import.close": "ዝጋ",
  "admin.locations.import.undo": "የመጨረሻውን ማስገባት ቀልብስ",
  "admin.locations.import.undone": "{count} ረድፎች ተመልሰዋል።",
  "admin.locations.import.ignored": "ተዘለሉ (ለንባብ ብቻ)",
  "admin.locations.import.ignoredRow": "ረድፍ {row} — {column} አልተተገበረም",
  "admin.locations.import.ignoredColumn": "{column} — {count} ረድፎች አልተተገበሩም",
  "admin.locations.import.error.denied": "ቦታዎችን ለማስገባት ፈቃድ የለህም።",
  "admin.locations.import.error.stepUp": "ኮድ አረጋግጠህ እንደገና ሞክር።",
  "admin.locations.import.error.fileChanged": "ፋይሉ ከቅድመ እይታው በኋላ ተቀይሯል። እንደገና ቅድመ እይታ አድርግ።",
  "admin.locations.import.error.busy": "ሌላ ማስገባት አሁን በሥራ ላይ ነው።",
  "admin.locations.import.error.tooLarge": "ያ ፋይል ከ1 ሜባ ይበልጣል።",
  "admin.locations.import.error.tooManyRows": "ያ ፋይል ከ5,000 ረድፎች ይበልጣል።",
  "admin.locations.import.error.badHeader": "የአምድ አርዕስቶቹ ከወረደው ፋይል አይመሳሰሉም።",
  "admin.locations.import.error.emptyFile": "ያ ፋይል ባዶ ነው።",
  "admin.locations.import.error.wrongFile": "ያ ፋይል የሌላ ማስገባት ነው። የሚስማማውን ክፈት።",
  "admin.locations.import.error.failed": "ማስገባቱን ማጠናቀቅ አልተቻለም።",
  "admin.locations.import.error.reason": "ምክንያት፦ {reason}",
  "admin.locations.import.reason.badKey": "የዚህ ረድፍ አድራሻ ጥቅም ላይ የሚውል መንገድ አይደለም።",
  "admin.locations.import.reason.unknownParent": "ይህ ረድፍ የሚቀመጥበት ቦታ በመዝገብ ላይ የለም።",
  "admin.locations.import.reason.parentLaterInFile": "ይህ ረድፍ የሚቀመጥበት ቦታ ከፋይሉ በታች ይታያል።",
  "admin.locations.import.reason.levelMismatch": "የዚህ ረድፍ ጥልቀት ከያዘው ቦታ አይስማማም።",
  "admin.locations.import.reason.parentInactive": "ከበላዩ ያለው ቦታ ተቋርጧል።",
  "admin.locations.import.reason.crossCountry": "ይህ ረድፍ ሌላ ገበያን ያመለክታል።",
  "admin.locations.import.reason.duplicateKey": "ተመሳሳዩ አድራሻ በዚህ ፋይል ሁለት ጊዜ ይታያል።",
  "admin.locations.import.reason.missingCoordinates": "ከተማም ክፍለ ከተማም የመሃል ኬክሮስና ኬንትሮስ ያስፈልጋቸዋል።",
  "admin.locations.import.reason.isoOnRegionsOnly": "የISO 3166-2 ኮድ የሚይዘው ክልል ብቻ ነው።",
  "admin.locations.import.reason.nameRequired": "ስም ያስፈልጋል።",
  "admin.locations.import.reason.countryRowByActivation":
    "ገበያ የሚከፈተው በራሱ ፋይል ነው፣ እዚህ ረድፍ በማግበር አይደለም።",
  "admin.locations.import.reason.unknownKey": "በዚያ አድራሻ ላይ ቦታ የለም።",
  "admin.locations.import.reason.deleteBlocked": "አንድ ነገር እስካመለከተው ይህ ቦታ መጥፋት አይችልም።",
  "admin.locations.import.reason.hasChildren": "ቦታዎች አሁንም ውስጡ ይቀመጣሉ።",
  "admin.locations.import.reason.hasListings": "ዝርዝሮች አሁንም እዚህ ተለጥፈዋል።",
  "admin.locations.import.reason.hasCoverage": "ሻጮች አሁንም ይሸፍኑታል።",
  "admin.locations.import.reason.hasProfileDefaults": "ሰዎች አሁንም በነባሪ እዚህ ይለጥፋሉ።",
  "admin.locations.import.reason.statusNeedsAction": "ሁኔታውን መቀየር በተግባር አምድ ውስጥ የሚስማማ ተግባር ይፈልጋል።",
  "admin.locations.import.reason.badCountryCode": "ያ የሁለት ፊደል የገበያ ኮድ አይደለም።",
  "admin.locations.import.reason.badUnitSystem": "የመለኪያ ሥርዓቱ ሜትሪክ ወይም ኢምፔሪያል መሆን አለበት።",
  "admin.locations.import.reason.badCurrency": "ያ የሦስት ፊደል የገንዘብ ኮድ አይደለም።",
  "admin.locations.import.reason.notARoot": "በገበያ ቅደም ተከተል ውስጥ የሚዘረዘረው የላይኛው ደረጃ ምድብ ብቻ ነው።",
  "admin.locations.import.reason.duplicateCategory": "ተመሳሳዩ ምድብ በዚያ ቅደም ተከተል ሁለት ጊዜ ተዘርዝሯል።",
  "admin.locations.import.reason.scopedRolesExist": "ሰዎች አሁንም በዚህ ገበያ የተወሰኑ ሚናዎችን ይይዛሉ።",
  "admin.locations.import.reason.unknownCountry": "ያ ገበያ በመዝገብ ላይ የለም።",
  "admin.locations.import.reason.badAction": "ያ ተግባር ይህ ፋይል የሚቀበለው አይደለም።",
  "admin.locations.import.reason.outOfScope": "ይህ ረድፍ ከምታስገባበት ገበያ ውጪ ነው።",
  "admin.locations.import.reason.alreadyOpen": "ያ ገበያ አስቀድሞ ክፍት ነው።",
  "admin.locations.import.reason.alreadyClosed": "ያ ገበያ አስቀድሞ ዝግ ነው።",
  "admin.locations.import.reason.alreadyActive": "ያ ቦታ አስቀድሞ ገባሪ ነው።",
  "admin.locations.import.reason.alreadyRetired": "ያ ቦታ አስቀድሞ ተቋርጧል።",
  "admin.locations.import.reason.planHasRefusals": "ምንም አልተጻፈም፦ ፋይል ሙሉ ለሙሉ ወይም በጭራሽ ይተገበራል።",
  "admin.locations.import.reason.batchAlreadyUndone": "ያ ማስገባት አስቀድሞ ተቀልብሷል።",
  "admin.locations.import.reason.formula": "በ=፣ +፣ - ወይም @ የሚጀምር ሕዋስ አይቀበልም።",
  "admin.locations.import.reason.badNumber": "ያ ሕዋስ ቁጥር አይደለም።",
  "admin.locations.import.reason.badBoolean": "ያ ሕዋስ true ወይም false መሆን አለበት።",
  "admin.locations.import.reason.tooLong": "ያ ሕዋስ ይህ አምድ ከሚፈቅደው ይረዝማል።",
  "admin.locations.import.reason.required": "ይህ ሕዋስ ባዶ መሆን አይችልም።",
  "admin.locations.import.reason.unknown": "ይህ ረድፍ ተከልክሏል",
  "admin.locations.import.titleAll":
    "\u12c8\u12f0 \u1201\u1209\u121d \u12a0\u1308\u122e\u127d \u12a0\u1235\u1308\u1263",
  "admin.locations.transfer.scopeAll": "\u1208\u1201\u1209\u121d \u12a0\u1308\u122e\u127d",
  "admin.countries.caption":
    "\u1260\u1218\u12dd\u1308\u1265 \u120b\u12eD \u12eb\u1209 \u1231\u1209 \u1308\u1260\u12eb\u12ce\u127d",
  "admin.countries.loading": "\u12a0\u1308\u122e\u127d \u1260\u1218\u1320\u122d \u120b\u12ed\u2026",
  "admin.countries.error":
    "\u12a0\u1308\u122e\u1279 \u1218\u1320\u122d \u12a0\u120d\u1270\u1273\u1229\u121d\u1362",
  "admin.countries.empty":
    "\u1260\u12a5\u1290\u12da\u1205 \u121b\u1323\u122a\u12eb\u12ce\u127d \u12e8\u121a\u1218\u1233\u1230\u120d \u12a0\u1308\u122d \u12e8\u1208\u121d\u1362",
  "admin.countries.searchPlaceholder":
    "\u1235\u121d \u12c8\u12ed\u121d \u12ae\u12f5 \u12ed\u1348\u120d\u1309",
  "admin.countries.filter.status": "\u1201\u1294\u1273",
  "admin.countries.transfer.scope": "ለሁሉም ገበያዎች",
  "admin.countries.filter.search": "ገበያዎችን ፈልግ",
  "admin.countries.filter.allStatusesOption": "ሁኔታ፦ ክፉት እና ዝግ",
  "admin.countries.filter.openOnly": "\u12ad\u1349\u1275 \u1265\u127b",
  "admin.countries.filter.closedOnly": "\u12dd\u130d \u1265\u127b",
  "admin.countries.filter.pageSize":
    "\u1260\u12a0\u1295\u12f5 \u1308\u1343 \u1228\u12f5\u1444\u127d",
  "admin.countries.col.name": "\u12a0\u1308\u122d",
  "admin.countries.col.status": "\u1201\u1294\u1273",
  "admin.countries.col.places": "\u1265\u122a \u1263\u1275\u12ce\u127d \u1363 \u1260\u12e0\u1275",
  "admin.countries.col.units": "\u1218\u1208\u12ad\u12eb \u00b7 \u1308\u1295\u12d8\u1265",
  "admin.countries.col.rail": "\u12e8\u1213\u12f5\u12f5 \u1245\u12f0\u1275",
  "admin.countries.col.roles": "\u12e8\u1273\u1230\u1229 \u121a\u1293\u12ce\u127d",
  "admin.countries.col.updated": "\u12e8\u1270\u12d8\u1218\u1290\u1260\u1275",
  "admin.countries.badge.open": "\u12ad\u1349\u1275",
  "admin.countries.badge.closed": "\u12dd\u130d",
  "admin.countries.tip.open": "ክፍት — ሰዎች እዚህ መለጠፍና ማሰስ ይችላሉ።",
  "admin.countries.tip.closed": "ዝግ — አስተዳዳሪ እስኪከፍተው ድረስ አይቀርብም።",
  "admin.countries.places.hint":
    "\u1265\u122a \u1263\u1275\u12ce\u127d \u00b7 \u1260\u1218\u12dd\u1308\u1265 \u120b\u12ed \u12eb\u1209 \u1263\u1275\u12ce\u127d",
  "admin.countries.rail.hint":
    "\u12ed\u1205 \u1308\u1260\u12eb \u1208\u122b\u1231 \u1260\u121b\u12d8\u1308\u1300\u1275 \u12e8\u121a\u12eb\u12dB\u1278\u12cD \u12e8\u1225\u122d \u121d\u12f5\u1266\u127d \u1265\u12d9\u1290\u1275\u1361 0 \u12a0\u1208\u121d\u12a0\u1240\u134D \u1245\u12f0\u1275 \u12ed\u12a8\u1270\u120b\u120d\u1362",
  "admin.countries.roles.hint":
    "\u1208\u12a5\u1290\u12da\u1205 \u1308\u1260\u12eb \u12e8\u1270\u1230\u1229 \u121a\u1293\u12ce\u127d\u1362",
  "admin.countries.unit.metric": "\u121c\u1275\u122a\u12ad",
  "admin.countries.unit.imperial": "\u12a2\u121d\u1350\u122a\u12eb\u120d",
  "admin.countries.field.name": "\u1235\u121d (\u12a5\u1295\u130d\u120a\u12dD)",
  "admin.countries.field.unit": "\u12e8\u1218\u1208\u12ad\u12eb \u1235\u122d\u12d3\u1275",
  "admin.countries.field.currency": "\u12e8\u1308\u1295\u12d8\u1265 \u12ae\u12f5",
  "admin.countries.field.currencyHint":
    "\u12e8ISO 4217 \u12ae\u12f5\u1361 \u1218\u1233\u120b\u1361 ETB\u1362 \u12e8\u1208\u120d\u121d \u12ab\u1208 \u1263\u12f6 \u12ed\u1213\u1362",
  "admin.countries.field.order": "\u1245\u12f0\u1275",
  "admin.countries.action.edit": "\u12a0\u122d\u1275\u12d5",
  "admin.countries.action.open": "\u12ed\u1205\u1295 \u1308\u1260\u12eb \u12ad\u1348\u1275",
  "admin.countries.action.close": "\u12ed\u1205\u1295 \u1308\u1260\u12eb \u12dd\u130a",
  "admin.countries.action.railOrder": "\u12e8\u1213\u12f5\u12f5 \u1245\u12f0\u1275",
  "admin.countries.edit.title": "\u12a0\u1308\u122d \u12a0\u122d\u1275\u12d5",
  "admin.countries.edit.anchorActive":
    "\u12e8\u12a5\u1229 \u1218\u1230\u1228\u1275 \u1263\u1275 \u1265\u122a \u1290\u12cD\u1362",
  "admin.countries.edit.anchorInactive":
    "\u12e8\u12a5\u1229 \u1218\u1230\u1228\u1275 \u1263\u1275 \u12a0\u1208\u1260\u122a\u121d\u1362",
  "admin.countries.edit.translationsLink":
    "\u1260\u120c\u120e\u127d \u12ee\u1295\u12eb\u12ce\u127d \u12eb\u1209 \u1235\u121e\u127d \u1260\u1275\u122d\u1301\u121b\u1295 \u2192 \u12f3\u1275\u1293 \u12ed\u1230\u1290\u12f3\u1209\u1362",
  "admin.countries.open.title": "\u12ed\u1205\u1295 \u1308\u1260\u12eb \u12ad\u1348\u1275",
  "admin.countries.open.hint":
    "\u1308\u12dB\u12ce\u127d \u1218\u1218\u120d\u12a8\u1275 \u12ed\u1300\u121d\u122b\u1209\u1361 \u12e8\u12a5\u1229 \u1218\u1230\u1228\u1275 \u1263\u1275\u121d \u1265\u122a \u12ed\u1206\u1293\u120d\u1362",
  "admin.countries.open.confirm": "\u12ad\u1348\u1275",
  "admin.countries.close.title": "\u12ed\u1205\u1295 \u1308\u1260\u12eb \u12dd\u130a",
  "admin.countries.close.hint":
    "\u12a8\u1218\u1218\u120d\u12a8\u1275 \u12ed\u1230\u12c8\u122b\u120d\u1362 \u121d\u1295\u121d \u12a0\u12ed\u1230\u1228\u12dd\u121d\u1362",
  "admin.countries.close.confirm": "\u12dd\u130a",
  "admin.countries.close.scopedHint":
    "{count} \u12e8\u121a\u1293 \u121a\u1293\u12ce\u127d \u1208\u12a5\u1290\u12da\u1205 \u1308\u1260\u12eb \u1270\u1230\u1229 \u1290\u12cD\u1362 \u1218\u12dD\u130b\u1275 \u1308\u1260\u12eb\u12cD\u1295 \u12ed\u1230\u12ed\u121b\u120d \u121a\u1293\u12ce\u1279\u1295 \u130d\u1295 \u12eb\u1246\u12eb\u120d\u1362",
  "admin.countries.close.forceConfirm":
    "\u1260\u121d\u1295\u121d \u12dd\u130a \u2014 \u1308\u1260\u12eb\u12cD\u1295 \u12ed\u1230\u12ed\u121d\u1361 \u121a\u1293\u12ce\u1279 \u12ed\u1246\u12ede",
  "admin.countries.rail.title": "\u12e8\u1213\u12f5\u12f5 \u1245\u12f0\u1275",
  "admin.countries.rail.hintDialog":
    "\u12a8\u12dA\u1205 \u1260\u1273\u127d \u12eb\u1208\u12cD \u1245\u12f0\u1275 \u1260\u12a5\u1290\u12da\u1205 \u1308\u1260\u12eb \u1308\u12dB\u12ce\u127d \u12e8\u121a\u12eb\u12e9\u1275 \u1290\u12cD\u1362",
  "admin.countries.rail.globalNote":
    "\u12ed\u1205 \u1308\u1260\u12eb \u12a0\u1201\u1295 \u12a0\u1208\u121d\u12a0\u1240\u134D \u1245\u12f0\u1275 \u12ed\u12a8\u1270\u120b\u120d\u1362",
  "admin.countries.rail.up": "\u12c8\u12f0 \u120b\u12ed",
  "admin.countries.rail.down": "\u12c8\u12f0 \u1273\u127d",
  "admin.countries.rail.reset":
    "\u12c8\u12f0 \u12a0\u1208\u121d\u12a0\u1240\u134D \u1245\u12f0\u1275 \u1218\u120d\u1235",
  "admin.countries.rail.submit": "\u1245\u12f0\u1271\u1295 \u12a0\u1235\u1240\u121d\u1325",
  "admin.countries.rail.empty":
    "\u1308\u1295\u1276 \u12e8\u121a\u12eb\u12dB \u1265\u122a \u12e8\u1225\u122d \u121d\u12f5\u1265 \u12e8\u1208\u121d\u1362",
  "admin.countries.import.title": "\u12a0\u1308\u122e\u127d\u1295 \u12a0\u1235\u1308\u1263",
  "admin.countries.error.reason": "\u121d\u12ad\u1295\u12eb\u1275\u1361 {reason}",
  "admin.countries.error.denied":
    "\u12ed\u1205\u1295 \u1208\u121b\u12f5\u1228\u130d \u1348\u1243\u12f5 \u12e8\u120b\u12ce\u1275\u121d\u1362",
  "admin.countries.error.stepUp":
    "\u12ae\u12f5 \u12a0\u1228\u130b\u130d\u1320\u12cD \u12a5\u1293 \u12a5\u1290\u12f0\u1308\u1298 \u12ed\u121e\u12ad\u1229\u1362",
  "admin.countries.error.unknown":
    "\u12ed\u1205 \u1218\u1240\u1218\u1325 \u12a0\u120d\u1270\u1233\u12a8\u121d\u1362",
  "admin.countries.error.badCountryCode":
    "\u12e8\u12a0\u1308\u122d \u12ae\u12f5 \u1265\u1275\u12ad \u1201\u1208\u1275 \u1206\u1204 \u1290\u12cD\u1362",
  "admin.countries.error.badUnitSystem":
    "\u12e8\u1218\u1208\u12ad\u12eb \u1235\u122d\u12d3\u1271 \u121c\u1275\u122a\u12ad \u12c8\u12ed\u121d \u12a2\u121d\u1350\u122a\u12eb\u120d \u1265\u127b \u1290\u12cD\u1362",
  "admin.countries.error.badCurrency":
    "\u12eb \u1276\u1235\u1275 \u1206\u1204 \u12e8\u1308\u1295\u12d8\u1265 \u12ae\u12f5 \u12a0\u12ed\u12f0\u1208\u121d\u1362",
  "admin.countries.error.nameRequired": "\u1235\u121d \u12eb\u1235\u1348\u120d\u130b\u120d\u1362",
  "admin.countries.error.unknownCountry":
    "\u12eb \u1308\u1260\u12eb \u1260\u1218\u12dd\u1308\u1265 \u120b\u12ed \u12e8\u1208\u121d\u1362",
  "admin.countries.error.scopedRolesExist":
    "\u12e8\u121a\u1293 \u121a\u1293\u12ce\u127d \u12a0\u1201\u1295\u121d \u12a5\u1290\u12da\u1205\u1295 \u1308\u1260\u12eb \u12ed\u12ed\u12dB\u1209\u1362",
  "admin.countries.error.duplicateCategory":
    "\u1260\u12ed\u1205 \u1245\u12f0\u1275 \u12cD\u1235\u1325 \u121d\u12f5\u1265 \u1201\u1208\u1275 \u130a\u12dc \u12a0\u1208\u1362",
  "admin.countries.error.notARoot":
    "\u1260\u1213\u12f5\u12f5 \u120b\u12ed \u12e8\u1225\u122d \u121d\u12f5\u1265 \u1265\u127b \u12ed\u1245\u12f0\u121b\u120d\u1362",

  // L2b-C2 — \u12e8\u1215\u12f3\u1290\u1275 \u12ad\u1349\u120d
  "admin.coverage.caption":
    "\u1201\u1209\u121d \u12a5\u12eb\u1295\u12f3\u1295\u12f1 \u12a5\u1245\u12f5 \u12a5\u1293 \u12e8\u121a\u12f0\u122d\u1230\u12cd \u122d\u1240\u1275",
  "admin.coverage.loading":
    "\u12a5\u12eb\u1295\u12f3\u1295\u12f1 \u12a5\u1245\u12f5 \u1260\u1218\u1320\u122b\u1275 \u120b\u12ed\u1362",
  "admin.coverage.error":
    "\u12a5\u1245\u12f6\u1279 \u1218\u1320\u122d \u12a0\u120d\u1270\u1273\u1209\u121d\u1362",
  "admin.coverage.empty":
    "\u12a0\u1201\u1295 \u12e8\u1270\u1218\u12dd\u1308\u1260 \u12a5\u1245\u12f5 \u12e8\u1208\u121d\u1362",
  "admin.coverage.filter.search": "ዕቅዶችን ፈልግ",
  "admin.coverage.searchPlaceholder": "የዕቅድ ስም ፈልግ",
  "admin.coverage.note":
    "\u12a8U6 \u1300\u121d\u122e \u121b\u1235\u1273\u12c8\u1242\u12eb \u12a5\u1290\u12da\u1205\u1295 \u12c8\u1230\u1266\u127d \u12ed\u1270\u1308\u1265\u122b\u120d\u1362",
  "admin.coverage.col.plan": "\u12a5\u1245\u12f5",
  "admin.coverage.col.cities": "\u12a8\u1274\u121e\u127d",
  "admin.coverage.col.regions": "\u12ad\u120d\u120e\u127d",
  "admin.coverage.col.countries": "\u1300\u1260\u12ce\u127d",
  "admin.coverage.col.photos": "በእብዛቅ ፈቶዎች",
  "admin.coverage.col.everywhere": "\u1260\u1201\u1209\u121d \u1260\u12ad\u120d",
  "admin.coverage.col.updated": "\u12e8\u1273\u12f0\u1230\u12cd",
  "admin.coverage.everywhere.yes": "\u12a0\u12ce",
  "admin.coverage.everywhere.no": "\u12a0\u12ed\u12f0\u1208\u121d",
  "admin.coverage.everywhere.tipYes":
    "\u1260\u12ed\u1205 \u12a5\u1245\u12f5 \u120b\u12ed \u12eb\u1208 \u1230\u12cd \u1260\u12a0\u1295\u12f5 \u130a\u12dc \u1201\u1209\u1295\u121d \u12ad\u1349\u1275 \u1308\u1260\u12eb\u12ce\u127d \u120b\u12ed \u121b\u12f5\u1228\u1235 \u12ed\u127d\u120b\u120d\u1362",
  "admin.coverage.everywhere.tipNo":
    "\u1260\u12ed\u1205 \u12a5\u1245\u12f5 \u120b\u12ed \u12eb\u1208 \u1230\u12cd \u1260\u1308\u1260\u12ce\u1279 \u12c8\u1230\u1295 \u12cD\u1235\u1325 \u1235\u12ce\u127d\u1295 \u12ed\u1218\u122d\u1323\u120d\u1362",
  "admin.coverage.create.open": "\u12a5\u1245\u12f5 \u12a0\u12ad\u120d",
  "admin.coverage.create.title": "\u12a5\u1245\u12f5 \u12a0\u12ad\u120d",
  "admin.coverage.action.edit": "\u12a0\u122d\u12ad",
  "admin.coverage.edit.title": "\u12a5\u1245\u12f5 \u12a0\u122d\u121d",
  "admin.coverage.field.plan": "\u12e8\u12a5\u1245\u12f1 \u1235\u121d",
  "admin.coverage.field.planHint":
    "\u1235\u122f\u1295 \u1235\u12ce\u127d \u12a5\u1293 \u12a0\u1295\u12f0\u122d\u1235\u12ae\u122d\u127d \u1265\u127b\u1365 \u12ad2 \u12a5\u1235\u12ad 32 \u1241\u121d\u1345\u1362",
  "admin.coverage.field.cities": "\u1260\u12a5\u1265\u12dB\u1245 \u12a8\u1274\u121e\u127d",
  "admin.coverage.field.regions": "\u1260\u12a5\u1265\u12dB\u1245 \u12ad\u120d\u120e\u127d",
  "admin.coverage.field.countries": "\u1260\u12a5\u1265\u12dB\u1245 \u1300\u1260\u12ce\u127d",
  "admin.coverage.field.photos": "በእብዛቅ ፈቶዎች",
  "admin.coverage.field.photosHint": "በአንድ ማስታወቂያ ከ0 እስከ 30 ፎቶዎች።",
  "admin.coverage.field.everywhere":
    "\u1260\u1201\u1209\u121d \u1260\u12ad\u120d \u1218\u1348\u1240\u12f5",
  "admin.coverage.field.limitHint": "\u1262\u12eb\u1235 1\u1362",
  "admin.coverage.noDelete":
    "\u12a5\u1245\u12f5 \u121b\u1235\u1270\u12ab\u12a8\u120d \u12ed\u127b\u120b\u120d\u1365 \u1218\u12f0\u1218\u1235\u1235 \u130d\u1295 \u12a0\u12ed\u127b\u120d\u121d\u1362",
  "admin.coverage.error.reason": "\u121d\u12ad\u1295\u12eb\u1275\u1728 {reason}",
  "admin.coverage.error.denied":
    "\u12ed\u1205\u1295 \u1208\u121b\u12f5\u1228\u130d \u1348\u1243\u12f5 \u12e8\u120e\u1275\u121d\u1362",
  "admin.coverage.error.stepUp":
    "\u12ae\u12f5 \u12a0\u1228\u130b\u130d\u1320\u12cD \u12a5\u1293 \u12a5\u1290\u1290\u1230\u1295\u1362",
  "admin.coverage.error.unknown":
    "\u1218\u1240\u1218\u1325 \u12a0\u120d\u1270\u1233\u12ab\u121d\u1362",
  "admin.coverage.error.badPlan":
    "\u12e8\u12a5\u1245\u12f5 \u1235\u121d \u12ad2 \u12a5\u1235\u12ad 32 \u1235\u122f\u1295 \u1235\u12ce\u127d \u12c8\u12ed\u121d \u12a0\u1295\u12f0\u122d\u1235\u12ae\u122d\u127d \u1265\u127b \u1290\u12cD\u1362",
  "admin.coverage.error.belowMinimum":
    "\u1201\u1209\u121d \u12c8\u1230\u1295 \u1262\u12eb\u1235 1 \u1290\u12cD\u1362",

  // U6-C1a — THE POSTING WIZARD (D1: no literal UI string).
  "post.head.title": "ማስታወቂያ ይለጥፉ — ethio.com",
  "post.head.description": "የሚሸጡትን በጥቂት ደረጃዎች በethio.com ላይ ይለጥፉ።",
  "post.title": "ማስታወቂያ ይለጥፉ",
  "post.stepOf": "ደረጃ {step} ከ{total} — {name}",
  "post.progress.label": "እድገትዎ",
  "post.step.category": "ምድብ",
  "post.step.photos": "ፎቶዎች",
  "post.step.specifications": "ዝርዝሮች",
  "post.step.details": "አርዕስት እና መግለጫ",
  "post.step.price": "ዋጋ",
  "post.step.place": "ቦታ",
  "post.step.contact": "አድራሻ",
  "post.step.review": "ገምግም",
  "post.action.back": "ተመለስ",
  "post.action.next": "ቀጥል",
  "post.action.retry": "እንደገና ይሞክሩ",
  "post.optional": "አማራጭ",
  "post.loading": "ቅጹ እየተከፈተ ነው…",
  "post.error.load": "ይህ ረቂቅ መከፈት አልቻለም።",
  "post.error.notYours": "ይህ ረቂቅ የሌላ መለያ ነው።",
  "admin.coverage.error.badMaxPhotos": "የፎቶ ገደብ ከ0 እስከ 30 ነው።",
  "post.signIn.required": "ማስታወቂያ ለመለጥፍ ይግቡ።",
  "post.signIn.action": "ግባ",
  "post.stepLater": "ይህ ደረጃ ቀደሙት ከተጠናቀቁ በኋላ ይከፈታል።",
  "post.save.saving": "እየተቀመጠ ነው…",
  "post.save.saved": "ተቀምጧል",
  "post.save.unsaved": "እስካሁን አልተቀመጠም — መሞከራችንን እንቀጥላለን።",
  "post.save.retrying": "እስካሁን አልተቀመጠም — እንደገና እየሞከርን…",
  "post.category.why": "ምድቡ ገዢዎች በየትኞቹ ዝርዝሮች መፈለግ እንደሚችሉ ይወስናል።",
  "post.category.searchLabel": "ምን ይሸጣሉ?",
  "post.category.searchPlaceholder": "ለምሳሌ ስልክ",
  "post.category.searchHint": "አንድ ቃል ይጻፉ፣ ወይም ከታች ያስሱ።",
  "post.category.noHits": "ምንም አልተገኘም። ሌላ ቃል ይሞክሩ፣ ወይም ከታች ያስሱ።",
  "post.category.browseLabel": "ወይም ያስሱ",
  "post.category.folder": "ክፈት",
  "post.category.chosen": "የተመረጠ ምድብ",
  "post.category.change": "ቀይር",
  "post.category.expect": "{details} ዝርዝሮች፣ {required} አስፈላጊ",
  "post.category.expectNone": "ለዚህ ምድብ ተጨማሪ ዝርዝር የለም።",
  "post.category.illustrationAlt": "የምድብ ምስል",
  "post.category.treeError": "ምድቦቹ መጫን አልቻሉም።",
  "post.category.empty": "እስካሁን ክፍት ምድብ የለም።",
  "post.photos.why": "ገዢ በመጀመሪያ የሚያየው ፎቶ ነው። ስልክዎ ከመላክ በፊት ያሳንሳቸዋል።",
  "post.photos.add": "ፎቶዎች ጨምር",
  "post.photos.addHint": "እስከ {max} ፎቶዎች። የመጀመሪያው ሽፋን ነው።",
  "post.photos.count": "{count} ከ{max}",
  "post.photos.none": "እስካሁን ፎቶ የለም — አንድ እስኪጨምሩ የምድቡ ምስል ይተካል።",
  "post.photos.cover": "ሽፋን",
  "post.photos.makeCover": "ሽፋን አድርግ",
  "post.photos.remove": "አስወግድ",
  "post.photos.moveUp": "ወደ ፊት አንቀሳቅስ",
  "post.photos.moveDown": "ወደ ኋላ አንቀሳቅስ",
  "post.photos.preparing": "እየተዘጋጀ…",
  "post.photos.uploading": "እየተላከ… {percent}%",
  "post.photos.done": "ተልኳል",
  "post.photos.retry": "እንደገና ላክ",
  "post.photos.deviceFailed": "ይህ ፎቶ በመሣሪያዎ ላይ መዘጋጀት አልቻለም።",
  "post.photos.needOne": "ቢያንስ አንድ ፎቶ ይጨምሩ።",
  "post.photo.refusal.nudity": "ይህ ፎቶ ራቁትነት ያሳያል፣ ይህ አይፈቀድም።",
  "post.photo.refusal.violence": "ይህ ፎቶ ጥቃት ያሳያል፣ ይህ አይፈቀድም።",
  "post.photo.refusal.weapon": "የጦር መሣሪያ በዚህ መታየት አይችልም።",
  "post.photo.refusal.drugs": "አደንዛዥ ዕፅ በዚህ መታየት አይችልም።",
  "post.photo.refusal.hate_symbol": "ይህ ፎቶ የጥላቻ ምልክት ይዟል።",
  "post.photo.refusal.personal_document": "ይህ የመታወቂያ ሰነድ ይመስላል። የግል ወረቀቶችን አይጫኑ።",
  "post.photo.refusal.contact_in_image": "በፎቶው ላይ የተጻፈውን ስልክ ቁጥር ወይም አድራሻ ያንሱ።",
  "post.photo.refusal.stock_watermark": "ይህ የውሃ ምልክት ያለው የተከማየ ፎቶ ይመስላል። የራስዎን ፎቶ ይጠቀሙ።",
  "post.photo.refusal.not_a_photo": "ይህ የዕቃው ፎቶ አይደለም።",
  "post.photo.refusal.providerUnavailable": "የፎቶ ማረጋገጫ አሁን አይሠራም። እባክዎ እንደገና ይሞክሩ።",
  "post.photo.refusal.unsupportedFormat": "JPEG፣ PNG እና WebP ፎቶዎች ብቻ ይቀበላሉ።",
  "post.photo.refusal.corruptImage": "ይህ ፋይል ተበላሽቷል መነበብም አልቻለም።",
  "post.photo.refusal.tooLarge": "ይህ ፎቶ በጣም ትልቅ ነው።",
  "post.photo.refusal.stripFailed": "ከዚህ ፎቶ የተደበቀ መረጃ ማጽዳት አልተቻለም፣ ስለዚህ አልተቀመጠም።",
  "post.photo.refusal.tooManyPhotos": "ይህ ማስታወቂያ የተፈቀደውን ከፍተኛ ፎቶ ብዛት ደርሷል።",
  "post.photo.refusal.rateLimited": "በአጭር ጊዜ ብዙ ፎቶ ልከዋል። እባክዎ ጥቂት ይጠብቁ።",
  "post.photo.refusal.notYourListing": "ይህ ማስታወቂያ የሌላ መለያ ነው።",
  "post.photo.refusal.listingNotOpenForPhotos": "ይህ ማስታወቂያ ተጨማሪ ፎቶ መቀበል አይችልም።",
  "post.photo.refusal.unknown": "ይህ ፎቶ አልተቀበለም።",
  "post.refusal.required": "ይህ ያስፈልጋል።",
  "post.refusal.badValue": "ይህ ዋጋ አይቀበልም።",
  "post.refusal.badShape": "ይህ የሚጠበቀው ቅርጽ አይደለም።",
  "post.refusal.tooLong": "ይህ በጣም ረጅም ነው።",
  "post.refusal.mustBeEmpty": "ይህ ምድብ ያንን አይቀበልም።",
  "post.refusal.unknownKey": "ያ ዝርዝር በዚህ ምድብ ውስጥ የለም።",
  "post.refusal.unknownCurrency": "ያ ገንዘብ በሥራ ላይ የለም።",
  "post.refusal.unknownPlace": "ያ ቦታ አልተገኘም።",
  "post.refusal.periodLocked": "የዋጋው ጊዜ ለዚህ ምድብ ተወስኗል።",
  "post.refusal.categoryNotPostable": "በዚህ ምድብ መለጠፍ አይችሉም። የተለየ ይምረጡ።",
  "post.refusal.residencyUnknown": "ከየትኛው አገር እየለጠፉ እንደሆነ ማወቅ አልቻልንም። እባክዎ እንደገና ይሞክሩ።",
  "post.refusal.coverageExceedsPlan": "ይህ ዕቅድዎ ከሚፈቅደው በላይ ቦታዎች ነው።",
  "post.refusal.rateLimited": "በአጭር ጊዜ ብዙ ጊዜ አስቀምጠዋል። እባክዎ ጥቂት ይጠብቁ።",
  "post.refusal.messagesRequired": "ገዢዎች መልእክት መላክ መቻል አለባቸው።",
  "post.refusal.notYourListing": "ይህ ማስታወቂያ የሌላ መለያ ነው።",
  "post.refusal.unknown": "ያ መቀመጥ አልቻለም።",
  "post.refusal.badType": "ይህ መልስ ይህ ዝርዝር በሚጠብቀው ቅርጽ አይደለም።",
  "post.refusal.badPreset": "ይህ ይህ ዝርዝር በሚጠብቀው ቅርጸት አይመሳሰልም።",
  "post.refusal.badDecimals": "ያነሱ የአስርዮሽ ቁጥሮችን ይጠቀሙ።",
  "post.refusal.outOfBounds": "ይህ ቁጥር በተፈቀደው ክልል ውጭ ነው።",
  "post.refusal.badMulti": "ጥቂት ይምረጡ፤ አንድ ምርጫን አይድገሙ።",
  "post.refusal.dependentMissing": "በመጀመሪያ ከዚህ በላይ ያለውን ዝርዝር ይመልሱ።",
  "post.refusal.inactiveOption": "ያ ምርጫ አሁን አይቀርብም።",
  "post.refusal.otherNeedsText": "ምን እንደሆነ ይግለጹ።",
  "post.refusal.unknownOption": "ያ ምርጫ በዚህ ውስጥ አይቀርብም።",
  "post.refusal.unknownAttribute": "ያ ዝርዝር የዚህ ምድብ አካል አይደለም።",
  "post.refusal.notPositive": "ከዜሮ በላይ መጠን ያስገቡ።",
  "post.price.why": "ምን እየጠየቁ እንደሆነ ይናገሩ። ገዢዎች በዋጋ ያጣራሉ፤ ትክክለኛ ዋጋ ብዙ መልስ ያገኛል።",
  "post.price.modeLabel": "ዋጋውን እንዴት ያስቀምጣሉ?",
  "post.price.mode.fixed": "ቁርጥ ዋጋ",
  "post.price.mode.negotiable": "የሚደራደር",
  "post.price.mode.free": "ነፃ",
  "post.price.mode.contact": "ለዋጋ ያገኙኝ",
  "post.price.notAllowed": "ይህ ምድብ ያለ ዋጋ ይለጠፋል።",
  "post.price.amountLabel": "መጠን",
  "post.price.amountPlaceholder": "ለምሳሌ 25000",
  "post.price.amountShown": "ገዢዎች {amount} {currency} ያያሉ።",
  "post.price.currencyLabel": "ገንዘብ",
  "post.price.currencySearch": "በኮድ ወይም በስም ይፈልጉ",
  "post.price.currencyNone": "ገንዘብ ይምረጡ",
  "post.price.currencyFailed": "የገንዘብ ዝርዝሩ አልተጫነም። ከጥቂት ቆይታ በኋላ ይሞክሩ።",
  "post.price.periodLabel": "በ",
  "post.price.period.once": "አንድ ጊዜ",
  "post.price.period.hour": "በሰዓት",
  "post.price.period.day": "በቀን",
  "post.price.period.week": "በሳምንት",
  "post.price.period.month": "በወር",
  "post.price.period.year": "በዓመት",
  "post.price.periodFixed": "ይህ ምድብ ሁልጊዜ {period} ይተመናል።",
  "post.price.expiryLabel": "የሚነሳበት ቀን (አማራጭ)",
  "post.price.expiryHint": "የተለመደውን ጊዜ ለመጠቀም ባዶ ይተዉት። ከዛሬ ጀምሮ ቢበዛ {days} ቀናት።",
  "post.where.why": "ዕቃው የሚገኝበትን ቦታ ይናገሩ። ገዢዎች በመጀመሪያ የራሳቸውን አካባቢ ይመለከታሉ።",
  "post.where.marketLabel": "ገበያ",
  "post.where.marketChoose": "አንዱን ይምረጡ",
  "post.where.marketUnresolved": "በየትኛው ገበያ ውስጥ እንደሆኑ ማወቅ አልቻልንም። አንዱን ይምረጡ።",
  "post.where.marketLoading": "ገበያዎች እየተጫኑ ነው…",
  "post.where.marketFailed": "የገበያ ዝርዝሩ አልተጫነም። ከጥቂት ቆይታ በኋላ ይሞክሩ።",
  "post.where.treeLoading": "ቦታዎች እየተጫኑ ነው…",
  "post.where.treeFailed": "ቦታዎቹ አልተጫኑም። ከጥቂት ቆይታ በኋላ ይሞክሩ።",
  "post.where.level.region": "ክልል",
  "post.where.level.city": "ከተማ",
  "post.where.level.sub_city": "ክፍለ ከተማ",
  "post.where.levelNone": "አንዱን ይምረጡ",
  "post.where.noRegions": "በዚህ ገበያ እስካሁን ክልሎች የሉም — ገበያው ራሱ አካባቢዎ ነው።",
  "post.where.allOf": "ሙሉ {name}",
  "post.where.planCaption": "የእርስዎ ዕቅድ፦ {cities} ከተማ",
  "post.where.planFull": "የእርስዎ ዕቅድ አንድ ከተማ ይሸፍናል። ሌላ ከመጨመር በፊት አንዱን ያንሱ።",
  "post.where.chosenLabel": "ይህ ማስታወቂያ የሚታይበት",
  "post.where.addPlace": "ይህን ቦታ ጨምር",
  "post.where.removePlace": "አንሳ",
  "post.where.prefilled": "ከአካባቢዎ ተሞልቷል — ዕቃው በሌላ ቦታ ከሆነ ይቀይሩ።",
  "post.where.pinLater": "የካርታ ምልክት ለሚጠቀሙ ምድቦች ይቀርባል።",
  "post.pin.open": "የካርትአ ምልክት ይጥቀሙ (አማራጭ)",
  "post.pin.close": "ካርትኩን ደብቅ",
  "post.pin.title": "የካርትአ ምልክት",
  "post.pin.why": "ምልክት ሲኖር ገዢዎች በቅልሉ ይደርሱባታል። አማራጭ ነው፣ ያትነትኑም እሬስ ትወስናለህ።",
  "post.pin.searchLabel": "በስም ቀባብውን ይፈልጉ",
  "post.pin.searchPlaceholder": "መንገድ፣ ምልክት መስጥን፣ አካባቢ",
  "post.pin.searching": "እያፈለገ ነው…",
  "post.pin.noResults": "በዚህ ስም ቀባብ አልተገኘም።",
  "post.pin.locate": "አቅማቾን ተጠቀም",
  "post.pin.locating": "አቅማቾን እያገኙ ነው…",
  "post.pin.locateRefused": "አሳሽዎ አቅማቾን አላገነም። ባይሆን ካርትኡን መንኸት ይችሉ።",
  "post.pin.locateUnavailable": "ይህ መሳሪያ አቅም ላይላክ አይችልም። ካርትኡን ይን኱።",
  "post.pin.layerStreet": "መንገዶች",
  "post.pin.layerSatellite": "ሳᜠላይት",
  "post.pin.tapHint": "ምልክቱን ለማስቀመጥ ካርትኡን ይን኱ ወይም ምልክቱን ይጉትቱ።",
  "post.pin.streetLabel": "መንገድ ወይም ምልክት መስጥን",
  "post.pin.streetHint": "ካርትኩ ላይ መሰረት ተሞላ። ገዢ ሙቅ የሚሆንበት ማስተካከያ ይቀይሩት።",
  "post.pin.precisionLabel": "ገዢዎች ምን ይመለካታሉ",
  "post.pin.precisionExact": "ትክኍኑን ምልክት አሳይ",
  "post.pin.precisionApprox": "አቃማባዊ ሰፈር (500 ሜትር) አሳይ",
  "post.pin.save": "ምልክቱን አስቀምጥ",
  "post.pin.saving": "እያስቀመጠ ነው…",
  "post.pin.saved": "ምልክቱ ተቀምጧል።",
  "post.pin.remove": "ምልክቱን አስወጥድ",
  "post.pin.removed": "ምልክቱ ተነሷል።",
  "post.pin.saveFailed": "ምልክቱ አልተቀመጠም። እባክዎ እነብስ ይሞክሩ።",
  "post.pin.none": "አሁን ምልክት ከሌም።",
  "post.pin.at": "ምልክት በ {lat}, {lng}",
  "post.pin.geocodeRateLimited": "በበዝ መፈለገይ፣ በአሁኑ ካርትኡን ይን኱ ወይም በኺዛ ይሞክሩ።",
  "post.pin.geocodeUnavailable": "የቀባብ ፈላገው አይሰራም። ካርትኡን ይን኱።",
  "post.pin.geocodeSignedOut": "በሳት ለመፈለገ እነብስ ይቅኘ።",
  "post.pin.previewExact": "ገዢ የሚያየው ትክኍ ምልክት።",
  "post.pin.previewApprox": "ገዢ ይህን አካባቢ (በአቄሪቡ 500 ሜትር) ይመለካታል — ትክኍ መሰረታችውን አይመለክትም።",
  "post.refusal.priceNotAllowed": "ይህ ምድብ ዋጋ ሊይዝ አይችልም።",
  "post.refusal.posterExpiryTooSoon": "ከዛሬ ቢያንስ አንድ ቀን የሚርቅ ቀን ይምረጡ።",
  "post.refusal.posterExpiryTooLate": "ያ ቀን ይህ ምድብ ከሚፈቅደው በላይ ነው።",
  "post.refusal.multipleMarkets": "ሁሉም ቦታዎች በአንድ ገበያ ውስጥ መሆን አለባቸው።",
  "post.refusal.providerUnavailable": "የመጻፍ እገዛው አሁን አይገኝም።",
  "post.refusal.aliasReserved": "ያ ስም ለገበያው የተያዘ ነው። ሌላ ይምረጡ።",
  "post.refusal.aliasTaken": "ያ ስም አስቀድሞ በአገልግሎት ላይ ነው።",
  "post.refusal.badLength": "ይህ በጣም አጭር ወይም በጣም ረጅም ነው።",
  "post.refusal.badHandle": "ይህ የሚሠራ ቁጥር ወይም መለያ አይመስልም።",
  "post.refusal.showNeedsValue": "ለገዢዎች ከማሳየት በፊት ይሙሉት።",
  "post.refusal.unknownCountry": "ያ ገበያ በመዝገብ ላይ የለም።",
  "post.refusal.countryAlreadyConfirmed": "የመኖሪያ አገርዎ አስቀድሞ ተወስኗል፤ በዚህ ቦታ አይቀየርም።",
  "post.specs.why": "ገዢዎች የሚያጣሩትና የሚፈልጉት በዚህ ዝርዝር ነው።",
  "post.specs.needCategory": "በመጀመሪያ ምድብ ይምረጡ።",
  "post.specs.none": "ይህ ምድብ ተጨማሪ ዝርዝር አይጠይቅም።",
  "post.specs.loadFailed": "እነዚህ ዝርዝሮች መጫን አልቻሉም።",
  "post.specs.choose": "ይምረጡ",
  "post.specs.optionsLoading": "ምርጫዎቹ በመጫን ላይ…",
  "post.specs.optionsFailed": "ምርጫዎቹ መጫን አልቻሉም። እንደገና ይንኩ።",
  "post.specs.attest": "አዎ፣ ይህ በምሸጠው ላይ እውነት ነው።",
  "post.specs.otherPlaceholder": "ምን እንደሆነ ይግለጹ",
  "post.specs.multiHint": "የሚመለከቱትን ሁሉ ይምረጡ።",
  "post.specs.boundsHint": "ከ{min} እስከ {max} መካከል።",
  "post.specs.noBound": "ማንኛውም",
  "post.specs.unitHint": "በ{unit}።",
  "post.specs.lengthHint": "እስከ {max} ቁምፊዎች።",
  "post.specs.presetHint": "ይህ ዝርዝር በሚጠብቀው ቅርጸት በትክክል ያስገቡ።",
  "post.details.why": "ጥርት ያለ አርዕስትና እውነተኛ መግለጫ ከማንኛውም ማጋነን በላይ ይሸጣል።",
  "post.details.titleLabel": "አርዕስት",
  "post.details.titlePlaceholder": "ለምሳሌ፦ ሳምሰንግ ፍሪጅ፣ የሚሠራ፣ አንድ ባለቤት",
  "post.details.descriptionLabel": "መግለጫ",
  "post.details.descriptionPlaceholder": "ምን እንደሆነ፣ ሁኔታውና ገዢ ማወቅ ያለበት ሁሉ።",
  "post.details.count": "{count} ከ{max}",
  "post.details.videoLabel": "የዩቲዩብ አገናኝ",
  "post.details.videoPlaceholder": "https://www.youtube.com/watch?v=…",
  "post.details.videoHint": "አማራጭ። የዩቲዩብ አገናኝ ብቻ።",
  "post.assist.action": "ለእኔ ይጻፍልኝ",
  "post.assist.hint": "ያስገቡትን ዝርዝር ብቻ እንጠቀማለን — ምንም አይፈጠርም።",
  "post.assist.working": "በመጻፍ ላይ…",
  "post.assist.done": "ተጠቁሟል — በነፃነት ያስተካክሉ።",
  // U6-C2b — የፎቶ ዳግም መሞከር ሕግ (PP-10)።
  "post.photos.couldNotSend": "መላክ አልተቻለም — እንደገና ይሞክሩ።",
  "post.photos.refusedFinal": "ይህ ፎቶ መጠቀም አይቻልም። ሌላ ይምረጡ።",
  // U6-C2b — ደረጃ 7፦ ማን እየሸጠ ነው።
  "post.who.why": "ገዢዎች የሚያምኑት ስምና የሚያገኙበት መንገድ ያስፈልጋቸዋል።",
  "post.who.readFailed": "የተቀመጡ ዝርዝሮችዎ መነበብ አልቻሉም። ከታች እንደገና ያስገቡ።",
  "post.who.savedHint": "ገዢዎች የሚያዩት ስም ይህ ነው።",
  "post.who.edit": "እነዚህን ይቀይሩ",
  "post.who.aliasLabel": "የሻጭ ስምዎ",
  "post.who.aliasHint": "ትንንሽ ፊደሎች፣ ቁጥሮችና _ ብቻ፣ ከ3 እስከ 30 ቁምፊ።",
  "post.who.aliasChecking": "በመፈተሽ ላይ…",
  "post.who.aliasFree": "ይህ ስም የእርስዎ ነው።",
  "post.who.typeLabel": "የሚሸጡት እንደ",
  "post.who.typePerson": "ግለሰብ",
  "post.who.typeBusiness": "ድርጅት",
  "post.who.businessLabel": "የድርጅት ስም",
  "post.who.channelsLabel": "ገዢዎች የሚያገኙዎት መንገድ",
  "post.who.channel.messages": "በethio.com መልዕክቶች",
  "post.who.channel.phone": "የስልክ ቁጥር",
  "post.who.channel.phoneHint": "ከአገር ኮድ ጋር፣ ለምሳሌ +251911234567።",
  "post.who.channel.telegram": "ቴሌግራም",
  "post.who.channel.telegramHint": "በ@ የሚጀምር መለያዎ።",
  "post.who.channel.whatsapp": "ዋትስአፕ",
  "post.who.channel.whatsappHint": "ከአገር ኮድ ጋር፣ ለምሳሌ +251911234567።",
  "post.who.messagesAlways": "መልዕክቶች ሁሌም ክፍት ናቸው — ገዢ ማግኘት መቻል አለበት።",
  "post.who.showIt": "በዝርዝሩ ላይ ይታይ",
  "post.who.countryLabel": "የሚኖሩበት አገር",
  "post.who.countryHint": "የሚኖሩበት። አንድ ጊዜ ብቻ ይጠየቃል።",
  "post.who.countryConfirmed": "ተረጋግጧል። ለመቀየር ድጋፍን ያግኙ።",
  "post.who.countryNone": "አገር ይምረጡ",
  // U6-C2b — ደረጃ 8፦ ማረጋገጥና ማውጣት።
  "post.review.why": "ገዢ የሚያየው ይህ ነው። ይፈትሹ፣ ከዚያ ያውጡ።",
  "post.review.noTitle": "እስካሁን አርዕስት የለም",
  "post.review.noPrice": "እስካሁን ዋጋ የለም",
  "post.review.priceLine": "{amount} {currency} {period}",
  "post.review.channels": "{count} የሚያገኙዎት መንገዶች",
  "post.review.publish": "አውጣ",
  "post.review.publishing": "በማውጣት ላይ…",
  "post.review.fixStep": "ወደ ደረጃ {step} ይሂዱ",
  "post.review.reviewTitle": "በግምገማ ላይ",
  "post.review.reviewBody": "ዝርዝርዎ በመመርመር ላይ ነው። አብዛኛውን ጊዜ ጥቂት ደቂቃዎች ይወስዳል፤ ሲወጣ በ«ዝርዝሮቼ» ውስጥ ያዩታል።",
  "post.review.myListings": "ወደ ዝርዝሮቼ ይሂዱ",
  // U6-C1-R1
  "post.field.required": "ያስፈልጋል",
  "post.refusalSummary": "ለመቀጠል እነዚህን ይሙሉ፦",
  "post.refusalSummary.elsewhere": "{step} ያስተካክሉ፦ {label}",
  "post.category.filterLabel": "ምድብ ይፈልጉ",
  "post.category.filterHint": "ቃል ይጻፉ፣ ወይም ከታች ያለውን ቡድን ይክፈቱ።",
  "post.category.chipLabel": "ምድብ",
  "post.photos.standIn": "የራስዎን ፎቶ እስኪጨምሩ ይህ ምስል ይተካል።",
  "post.photos.rules":
    "JPG፣ PNG ወይም WebP \u00b7 እያንዳንዱ እስከ 6 ሜባ \u00b7 ቢያንስ 480 ፒክስል ስፋት \u00b7 ጥሩ ብርሃን፣ ንጹህ ዳራ፣ ሙሉ እቃው በምስሉ ውስጥ",
  "post.photos.skip": "ያለ ፎቶ ይቀጥሉ",
  "post.save.paused": "ማስቀመጥ ለ {seconds} ሰከንድ ቆሟል",
  "post.review.activeFromLabel": "ከመቼ ጀምሮ",
  "post.review.activeUntilLabel": "እስከ መቼ",
  "post.review.activeFromFact": "ግምገማውን እንዳለፈ ወዲያውኑ።",
  "post.review.activeWindowHint": "በዚህ ምድብ {days} ቀን መስኮት ውስጥ ያለ ማንኛውም ቀን።",

  // ---- U6-C1-R2 — የሁለተኛው ግምገማ እርማቶች ------------------------------------
  "post.photos.helper": "እስከ {max} ፎቶዎች · JPG፣ PNG ወይም WebP · እስከ 6 ሜባ · ቢያንስ 480 ፒክስል ስፋት",
  "post.specs.unitsLater": "በምድብ የተወሰኑ የመለኪያ ዝርዝሮችና ነባሪ መጠኖች ቆይተው ይመጣሉ።",
  "post.assist.triesLeft": "ከ{max} ሙከራዎች {left} ቀርተዋል",
  "post.assist.exhausted": "ለዚህ ማስታወቂያ ሁሉንም {max} የጽሑፍ ሙከራዎች ተጠቅመዋል።",
  "post.assist.historyLabel": "የቀረቡ ሐሳቦች",
  "post.assist.use": "እሱን ተጠቀም",
  "post.assist.again": "ሌላ አማራጭ ጻፍ",
  "post.assist.suggestionNumber": "አማራጭ {number}",
  "post.who.aliasSuggested": "የቀረበ፦ {alias}",
  "post.who.aliasUseIt": "ተጠቀምበት",
  "post.who.firstNameLabel": "የመጀመሪያ ስም",
  "post.who.lastNameLabel": "የአባት ስም",
  "post.who.nameHint": "ገዢዎች የሚያዩት የሻጭ ስምዎን ነው፤ ይህን አይደለም። ለመዝገብዎ እንይዘዋለን።",
  "post.who.nameBusinessHint": "ለንግድ አማራጭ ነው — ገዢዎች የሚያዩት የንግዱን ስም ነው።",
  "post.who.showOnListing": "በማስታወቂያው ላይ አሳይ",
  "post.refusal.aliasImitatesBrand": "ይህ ስም {name}ን ይመስላል። ሌላ ይምረጡ።",
  "post.refusal.assistBudgetSpent": "ለዚህ ማስታወቂያ ሁሉንም የጽሑፍ ሙከራዎች ተጠቅመዋል።",
  "post.where.defaultPlaceLabel": "ዕቃው የሚገኝበት ቦታ",
  "post.where.defaultPlaceHint": "ካላስወገዱት በስተቀር ማስታወቂያው የሚታየውም እዚህ ነው።",
  "post.where.addBack": "መልሰው ጨምሩት",
  "post.where.addAnother": "ሌላ ቦታ ጨምር",
  "post.where.planCount": "ከ{max} ከተሞች {used}",
  "post.review.summaryLabel": "የሚለጥፉት ነገር",
  "post.review.edit": "አስተካክል",
  "post.review.placesCount": "{count} ቦታ(ዎች)",
  "post.review.notGiven": "አልተሰጠም",
  "post.refusal.belowModelYear": "ይህ ይህ ሞዴል ከተሠራበት ዓመት በፊት ነው።",
  "post.refusal.aboveModelYear": "ይህ ይህ ሞዴል ከተሠራበት ዓመት በኋላ ነው።",
  "post.refusal.optionNotAllowed": "ይህ ምርጫ በዚህ ምድብ ውስጥ አይቀርብም።",
  "post.specs.parentFirst": "በመጀመሪያ {parent}ን ይምረጡ።",
  "post.specs.fromModel": "ከሞዴሉ የተወሰደ — የተለየ ከሆነ ያስተካክሉ።",
  "post.specs.modelDiffers": "ይህ ሞዴል የተለየ ነገር ይናገራል።",
  "post.specs.useModelValue": "የሞዴሉን ዋጋ ይጠቀሙ",
  "post.specs.noneForParent": "ከላይ ለመረጡት ምርጫ አማራጭ የለም።",
  "post.specs.resetForModel": "ዝርዝሮች ለ{model} ተዘምነዋል",
  "post.specs.resetUndo": "ቀልብስ",
  "post.where.addUnder": "በዚህ ስር ቦታ ጨምር",
  "post.where.planLevels": "{regions} ክልል፣ {cities} ከተማ፣ {subCities} ክፍለ ከተማ",
  "post.category.nextBlocked": "ለመቀጠል ምድብ ይምረጡ።",
  "post.category.chooseOne": "ከዝርዝሩ ውስጥ ምድብ ይምረጡ።",
  "post.price.moreCurrencies": "ተጨማሪ ገንዘቦች…",
  "post.action.backToReview": "ወደ ማጠቃለያው ተመለስ",
  "post.progress.stripLabel": "ደረጃዎች",
  "post.progress.stepNumber": "ደረጃ {step}",
  "post.category.changedCleared": "አንዳንድ መልሶች ለ{category} አይሠሩም፤ ተጠርገዋል፦ {fields}",
  "post.category.changedDismiss": "ዝጋ",
  "post.category.changedPhotos": "ከመለጠፍዎ በፊት ፎቶዎችዎን ከአዲሱ ምድብ ጋር ያመሳክሩ።",
  "post.specs.setByModel": "በሞዴሉ የተወሰነ — ሞዴሉ የሚፈቅደው መልስ ይህ ብቻ ነው።",
  "post.specs.lockedByChoice": "በምርጫዎ የተወሰነ",
  "post.specs.lockedChange": "ቀይር",
  "post.specs.helpMore": "ስለዚህ ዝርዝር ተጨማሪ",
  "post.specs.helpMoreMark": "(i)",
  "post.specs.moreDetails": "ተጨማሪ ዝርዝሮች ({count})",
  "post.specs.moreLess": "ተጨማሪ ዝርዝሮችን ደብቅ ({count})",
  "post.specs.factHint": "ይህ ሞዴል፦ {value}",
  "post.category.allRoots": "ሁሉም ምድቦች",
  "post.category.trailLabel": "የአሁኑ ደረጃ",
  "post.review.previewAsBuyer": "ገዢዎች እንደሚያዩት ይመልከቱ",
  "post.preview.title": "ገዢዎች እንደሚያዩት",
  "post.preview.close": "ቅድመ-እይታን ዝጋ",
  "post.preview.detailsLabel": "ዝርዝሮች",
  "post.preview.descriptionLabel": "መግለጫ",
  "post.preview.sellerLabel": "ሻጭ",
  "post.preview.mapPlaceholder": "የዚህ ቦታ ካርታ ከማስታወቂያው ጋር ይከፈታል።",
  "post.preview.photoCount": "ፎቶ {index} ከ{total}",
  "admin.attributes.link.allowedOptions": "የተፈቀዱ አማራጮች",
  "admin.attributes.link.allowedOptionsHint": "ሁሉንም አማራጭ ለማቅረብ ባዶ ይተዉ። እሴቶችን በ| ይለያዩ።",
  "admin.attributes.link.defaultValue": "ነባሪ እሴት",
  "admin.attributes.link.defaultValueHint": "ሻጭ ይህን ምድብ ሲከፍት አስቀድሞ ይሞላል።",
  "admin.attributes.link.none": "ነባሪ የለም",
  "admin.attributes.link.visibleWhen": "የሚታይበት ጊዜ",
  "admin.attributes.link.saveFailed": "ይህ ለውጥ አልተቀመጠም። እንደገና ይሞክሩ።",
  "admin.attributes.link.alwaysVisible": "ሁልጊዜ ይታይ",
  "post.review.yes": "አዎ",
  "post.review.no": "አይ",
  "admin.coverage.maxPhotos": "ፎቶዎች",
};

export default am;
